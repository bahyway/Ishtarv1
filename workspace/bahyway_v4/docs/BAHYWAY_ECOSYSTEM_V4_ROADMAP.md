...
DataCleansing Station  — PII vault, null imputation, type coercion
    ↓
DataSteward Station    — human-review queue for flagged records
    ↓
BlackBox Station       — anomaly detection (orbital trust probe)
    ↓
GOLD Station           — finalise particles, assign KAKI, write to EnkiDB
```
...
| Agent | Role | Crates Required |
|-------|------|-----------------|
| DataSteward Agent | Review flagged particles, approve/reject | `data-steward-station`, `enkidullm-core` |
...
| Crate | Port | Type |
|-------|------|------|
| `enkidb-engine` | 7001 | EnkiDB — primary OLTP particle store |
| `enkidb-dw` | 7002 | EnkiDW — analytical data warehouse |
| `enkidb-sdb` | 7003 | EnkiSDB — schema/structure DB |
| `enkidb-odb` | 7004 | EnkiODB — orbital/temporal DB |
| `enkidb-qdb` | 7005 | EnkiQDB — quantum/probabilistic |
| `enkidb-quantdb` | 7006 | EnkiMDB — measurement DB |
| `enkidb-recovery` | 7007 | EnkiDDB — disaster-recovery |
