...
| **Where** | `crates/anu-governor/src/gate_review.rs`, `domain_review.rs`, `pb_dependency_review.rs` (suggest-then-approve scanners); `bin/anu_governor_web.rs` (`/api/gates/*`, `/api/playbooks/search`, `/api/dependencies/*`); `bin/web_assets/gate_orbits.js` + vendored `three.module.js`/`OrbitControls.js` (the Shala4 tab itself). |
...
| **Where** | Planned: `crates/anu-governor/src/pb_compare_review.rs` (mirrors `gate_review.rs`/`domain_review.rs`/`pb_dependency_review.rs`'s exact suggest-then-approve shape), a new `WriteNode::tag_compare_state`, new Shala `/api/compare/*` endpoints. Not yet implemented. |
