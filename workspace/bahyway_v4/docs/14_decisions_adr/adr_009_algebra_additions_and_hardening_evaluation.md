...
| **Scheduling (EriduOS / eridu-scheduler)** | 4, 8 | Orbits Calculus for tick sequencing, Markov Chains for optimal interval derivation | **B+** | Layer 8 addition enables optimal snapshot interval derivation from mean first-passage time — currently snapshot intervals are configurable constants, not mathematically derived. |
