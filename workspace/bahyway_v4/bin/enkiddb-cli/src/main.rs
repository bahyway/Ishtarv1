...
use enkiddb::{
    check_authorship, ingest, readnode, scan_markdown_directory, TeamAllowlist, WriteNode,
};
...
fn main() -> ExitCode {
    ...
    if !empty_files.is_empty() {
        ...
        for path in &empty_files {
            ...
            println!(
                "  {BYLW}SKIP{RST}     {:<40} -- empty, nothing to ingest",
                rel.display()
            );
        }
        println!();
    ...
    let allowlist = match &args.team_allowlist {
        Some(p) => match TeamAllowlist::from_file(p) {
            ...
            Err(e) => {
                eprintln!(
                    "  {BRED}error reading --team-allowlist {}: {e}{RST}",
                    p.display()
                );
                return ExitCode::FAILURE;
            }
    ...
    for path in &files {
        ...
        let scan = std::fs::read(path)
            .ok()
            .map(|bytes| enkiddb::scan_document(&bytes));
            ...
            if !security_ok {
                ...
                println!(
                    ...
                    check
                        .author
                        .as_deref()
                        .unwrap_or("<unknown -- no git history>"),
                );
            }
        }
    }
    ...
    println!(
        ...
        rejected,
        if rejected == 0 {
            format!(" {DIM}(allowlist: {} member(s)){RST}", allowlist.len())
        } else {
            String::new()
        },
    );
    ...
    let kakis = match write_node.ingest_directory_categorized_checked(
        &args.dir,
        args.epoch_start,
        &allowlist,
    ) {
    ...
    if let Err(e) = std::fs::create_dir_all(&current) {
        eprintln!(
            "  {BRED}error creating data dir {}:{RST} {e}",
            current.display()
        );
        return ExitCode::FAILURE;
    }
...
/// Particles per PU (Particle Unit) -- a whimsical but real rollup of the
/// raw EAV-triple count this run actually journaled, so "how much data did
/// this ingest" reads as one comparable number instead of a raw particle
/// count that's meaningless on its own. 7 is arbitrary (no schema
/// significance), chosen once and fixed here so PU is stable across runs.
const PARTICLES_PER_PU: f64 = 7.0;

/// Parse `OK:<kaki_hex>:<section_count>:<particle_count>` and return
/// `particle_count`, or `None` if the response isn't that exact shape (an
/// `ERR:` response, or an older `enkiddb-write-server` that hasn't been
/// rebuilt with this field yet and still replies
/// `OK:<kaki_hex>:<section_count>`) -- callers treat `None` as "unknown",
/// never as zero. Requires exactly 3 fields after `OK:` on purpose: taking
/// "whatever the last field is" would silently misread an old server's
/// `section_count` as `particle_count` instead of recognizing the response
/// shape is stale.
fn parse_particle_count(resp: &str) -> Option<usize> {
    let fields: Vec<&str> = resp.strip_prefix("OK:")?.split(':').collect();
    match fields.as_slice() {
        [_kaki_hex, _section_count, particle_count] => particle_count.parse().ok(),
        _ => None,
    }
}
...
fn run_remote(files: &[PathBuf], base: &Path, host: &str, port: u16) -> ExitCode {
    ...
    let mut unknown_particle_counts = 0usize;
    ...
    for path in files {
        ...
        let body = match std::fs::read_to_string(path) {
            ...
            Err(e) => {
                println!(
                    "  {BRED}ERR{RST}      {:<40} -> {collection:<20} read error: {e}",
                    rel.display()
                );
                failed += 1;
                continue;
        ...
        match remote::send_document(host, port, &collection, &body) {
            Ok(resp) if resp.starts_with("OK:") => {
                println!(
                    "  {GRN}OK{RST}       {:<40} -> {collection:<20} {DIM}{resp}{RST}",
                    rel.display()
                );
                ok += 1;
                match parse_particle_count(&resp) {
                    Some(n) => total_particles += n,
                    None => unknown_particle_counts += 1,
                }
            }
            Ok(resp) => {
                println!(
                    "  {BRED}ERR{RST}      {:<40} -> {collection:<20} {resp}",
                    rel.display()
                );
                failed += 1;
            }
            Err(e) => {
                println!(
                    "  {BRED}ERR{RST}      {:<40} -> {collection:<20} {e}",
                    rel.display()
                );
                failed += 1;
            }
    ...
    println!();
    if unknown_particle_counts > 0 {
        println!(
            "  {BYLW}note:{RST} {unknown_particle_counts} response(s) didn't carry a particle \
             count (older server build?) -- PU total below is a floor, not exact."
        );
    }
    let pu = total_particles as f64 / PARTICLES_PER_PU;
    println!(
        "  {BCYN}particles ingested: {total_particles}{RST}  {DIM}({PARTICLES_PER_PU:.0} particles = 1 PU){RST}  {BCYN}= {pu:.2} PU{RST}"
    );
    println!();
    if failed == 0 {
    ...
    while i < rest.len() {
        match rest[i].as_str() {
            ...
            "--tribe" => {
                ...
                tribe_id = u16::from_str_radix(hex.trim_start_matches("0x"), 16)
                    .map_err(|e| e.to_string())?;
            }
            "--team-allowlist" => {
                i += 1;
                team_allowlist = Some(PathBuf::from(
                    rest.get(i).ok_or("--team-allowlist needs a value")?,
                ));
            }
            "--epoch-start" => {
                i += 1;
                epoch_start = rest
                    .get(i)
                    .ok_or("--epoch-start needs a value")?
                    .parse()
    ...
    for doc in graph.documents() {
        ...
        match &doc.placement {
            Some(p) if p.bridges_to.is_empty() => {
                println!(
                    "  {GRN}·{RST} {title:<50} {DIM}affinity={:.2} radius={:.2}{RST}",
                    p.affinity, p.radius
                );
            }
            Some(p) => {
...
fn print_usage() {
    ...
    eprintln!(
        "  --data-dir <dir>         where to materialize Data Files (default ~/.bahyway/enkiddb)"
    );
    eprintln!(
        "  --tribe <hex>            KAKI tribe id (default 0x7160, matches enkiddb-write-server)"
    );
