//! enkidw-read-server — EnkiDW (Data Warehouse) Read Node, sovereign TCP
//! server. Wraps `enkidb_readnode::CachedReadNode` over the Journal that
//! `enkidw-write-server` materializes from its `enkidw::EtlPipeline`'s
//! `PersistedDb` (`EnkiDb::journal()` -- the same real `enkidb_journal::
//! Journal` type every other EnkiDB Type's materialize/CachedReadNode path
//! consumes, so this is a drop-in application of that pattern, not a
//! special case) behind the same real, hand-rolled **binary** wire
//! protocol every other EnkiDB Type Read Node in this fleet uses — no
...
//! read-only. `enkidw-write-server` is the only process that materializes
...
fn bind_addr() -> String {
    env::var("BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:7002".to_string())
}
fn data_dir() -> PathBuf {
...
fn main() {
    eprintln!("𒁾 enkidw-read-server — EnkiDW (Data Warehouse) Read Node [in-memory, binary wire]");
