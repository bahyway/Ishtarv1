...
fn ingest_lines<R: BufRead>(
    ...
    for line in reader.lines() {
        ...
        if parts.len() < 2 { skip += 1; continue; }

        let epoch = parts[1].trim().parse::<u32>().unwrap_or(1);
        let state = if parts.len() >= 3 { parse_state(parts[2]) } else { ParticleState::Fuzzy };
        ...
        let gr = match gate.ingest(record) {
            Ok(r)  => r,
            Err(e) => { eprintln!("[skip] ingest error: {e}"); skip += 1; continue; }
        };
        ...
        if !vr.is_valid() {
            eprintln!("[skip] validation failed — missing: {:?}", vr.missing_required);
            skip += 1;
            continue;
        }

        if let Err(e) = pdb.register_particle(&gr.particle) {
            eprintln!("[skip] register error: {e}"); skip += 1; continue;
        }
        if let Err(e) = pdb.commit(gr.event_kaki, gr.particle, gr.epoch, gr.eav) {
            eprintln!("[skip] commit error: {e}"); skip += 1;
        } else {
            ok += 1;
        }
    }

    eprintln!("𒁾 ingested={ok}  skipped={skip}  on-disk={}", pdb.stats().entries_written);
}
...
fn main() {
    ...
    if pdb.stats().entries_replayed > 0 {
        eprintln!("  replayed : {} existing entries", pdb.stats().entries_replayed);
    }
