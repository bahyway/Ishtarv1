//! enkiodb-read-server — EnkiODB (Operational Database) Read Node,
//! sovereign TCP server. Wraps `enkidb_readnode::CachedReadNode` over the
//! ODB Journal that `enkisdb-write-server` materializes (`SdbValidationPass`
//! for particles drained from EnkiSDB, `EnkidbTransactionCommit` for
//! particles committed via the EnkiDB GUI path, `ArchiveMove` once
//! retired — see that server's own module doc comment; it colocates the
//! EnkiSDB→EnkiODB/EnkiQDB pipeline in one process because `OdbStore::
...
fn bind_addr() -> String {
    env::var("BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:7004".to_string())
}
fn data_dir() -> PathBuf {
