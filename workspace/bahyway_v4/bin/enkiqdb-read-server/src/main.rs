//! enkiqdb-read-server — EnkiQDB (Quarantine Archive) Read Node, sovereign
//! TCP server. Wraps `enkidb_readnode::CachedReadNode` over the QDB
//! Journal that `enkisdb-write-server` materializes (one `QuarantineMove`
//! entry per permanently-quarantined particle, drained from EnkiSDB —
//! see that server's own module doc comment; it colocates the
//! EnkiSDB→EnkiODB/EnkiQDB pipeline in one process because `QdbStore::
//! drain_from_sdb` takes `&SdbStore` by reference, an in-process design)
//! behind the same real, hand-rolled **binary** wire protocol every other
//! EnkiDB Type Read Node in this fleet uses — no JSON, ever, per the
//! Architect's standing law for EnkiDB Types communications. Frame layout
//! is identical to `enkidb-read-server`'s own module doc comment; not
//! repeated here.
//!
//! ## Where its data comes from
//! This server never writes — it only opens `DATA_DIR/current/{entities,eav}`
//! read-only. `enkisdb-write-server` is the only process that materializes
//! into that path (the CQRS split ADR-012 already establishes elsewhere).
//!
//! ## Live reload
//! A background thread re-opens `DATA_DIR/current` every `RELOAD_SECS`
//! (default 30), same concurrency model as `enkidb-read-server` (`RwLock`,
//! never blocks an in-flight query on a reload's O(n) rebuild).
#![forbid(unsafe_code)]
...
fn bind_addr() -> String {
    env::var("BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:7005".to_string())
}
fn data_dir() -> PathBuf {
    PathBuf::from(env::var("DATA_DIR").unwrap_or_else(|_| "/data/qdb".to_string()))
}
fn reload_secs() -> u64 {
...
fn main() {
    eprintln!("𒁾 enkiqdb-read-server — EnkiQDB (Quarantine Archive) Read Node [in-memory, binary wire]");
...
fn handle(mut stream: TcpStream, state: &RwLock<Option<Arc<Live>>>) -> io::Result<()> {
    ...
    let Some(live) = live else {
        return send_error(&mut stream, "not ready -- Data Files not materialized yet");
    };
