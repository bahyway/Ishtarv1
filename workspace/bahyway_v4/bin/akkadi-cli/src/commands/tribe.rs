use crate::{output::format::OutputFormat, DISTRICTS, HEPTA_DIMS, PLANET_GLYPHS, PLANET_NAMES};
...
pub fn list_tribes(format: OutputFormat) {
    ...
    match format {
        OutputFormat::Json => {
            let arr: Vec<serde_json::Value> = tribes
                .iter()
                .map(|t| {
                    serde_json::json!({
                        "dim":      t.dim,
...
pub fn show_tribe(dim: &str, format: OutputFormat) {
    ...
    let t = match tribes.iter().find(|t| t.dim.eq_ignore_ascii_case(dim)) {
        Some(t) => t,
        None => {
            eprintln!("unknown tribe '{dim}'; valid: ME GU SAG IZI A UD URU");
            return;
        }
    };
