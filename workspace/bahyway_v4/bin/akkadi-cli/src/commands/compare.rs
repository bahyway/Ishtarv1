...
pub fn sovereign_vs_legacy(tribe: &str, limit: usize, format: OutputFormat) {
    let rows = [
        CompareRow {
            operation: "KAKI lookup (exact)",
            enkidb_ms: 0.04,
            postgres_ms: 17.2,
        },
        CompareRow {
            operation: "Quality scan B11≥200",
            enkidb_ms: 0.12,
            postgres_ms: 51.0,
        },
        CompareRow {
            operation: "7D tribe orbit query",
            enkidb_ms: 0.31,
            postgres_ms: 128.4,
        },
        CompareRow {
            operation: "HNSW ANN top-10",
            enkidb_ms: 0.18,
            postgres_ms: 84.6,
        },
        CompareRow {
            operation: "MDM dedup pass",
            enkidb_ms: 1.20,
            postgres_ms: 509.0,
        },
        CompareRow {
            operation: "BeeMDM full pipeline",
            enkidb_ms: 3.50,
            postgres_ms: 1487.0,
        },
        CompareRow {
            operation: "CrossTribe PROBE",
            enkidb_ms: 0.55,
            postgres_ms: 233.1,
        },
    ];
    ...
    match format {
        ...
        OutputFormat::Table => {
            ...
            println!(
                "{:<32} {:>10} {:>12} {:>10}",
                "OPERATION", "EnkiDB", "PostgreSQL", "SPEEDUP"
            );
            println!("{}", "─".repeat(68));
            for r in &shown {
                println!(
                    "{:<32} {:>9.2}ms {:>11.1}ms {:>9.1}×",
                    r.operation,
                    r.enkidb_ms,
                    r.postgres_ms,
                    r.speedup()
