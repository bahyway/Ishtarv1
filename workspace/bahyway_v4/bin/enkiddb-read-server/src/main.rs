//! enkiddb-read-server — EnkiDDB (Tigris) Read Node, sovereign TCP server.
//!
//! Wraps `enkidb_readnode::CachedReadNode` (the whole Data File loaded
//! into RAM once at open/reload, zero disk I/O per query — the same
//! engine `enkidb-read-server`, core EnkiDB's Read Node, moved to; see
//! that crate's `cached` module doc comment for the real, measured
//! numbers behind why) and `enkiddb::RagIndex` (built via
//! `RagIndex::build_from_cached` — the `CachedReadNode`-backed sibling
//! of the original `build_from_readnode`) behind a real, hand-rolled
//! **binary** wire protocol — no JSON, ever, per the Architect's
//! standing law for EnkiDB Types communications.
//!
...
//! path is a separate, external sync step (`scripts/enkiddb-sync-data.sh`
//! in this repo, run from `enkidb-node-write` or a broker, or PB-213's
//! cross-host relay for the real 2-VM split), not this server's job --
//! exactly the CQRS split ADR-012 already establishes.
//!
//! ## Live reload
//! A background thread re-opens `DATA_DIR/current` and rebuilds the RAG
//! index every `RELOAD_SECS` (default 30) so newly-synced data becomes
//! visible without a restart, without ever blocking an in-flight query
//! (state lives behind an `RwLock`, the same concurrency model
...
fn main() {
    eprintln!("𒁾 enkiddb-read-server — EnkiDDB (Tigris) Read Node [in-memory, binary wire]");
...
fn handle(mut stream: TcpStream, state: &RwLock<Option<Arc<Live>>>) -> io::Result<()> {
    ...
    if let Some(query) = src.strip_prefix("QUERY:") {
        return match live.read_node.query(query) {
            Ok(result) => send_query_success(&mut stream, &result),
            Err(e) => send_error(&mut stream, &format!("query: {e:?}")),
        };
    }

    if let Some(rest) = src.strip_prefix("SEARCH:") {
        let mut parts = rest.splitn(2, ':');
        let top_k: usize = parts.next().and_then(|s| s.parse().ok()).unwrap_or(5);
        let query_text = parts.next().unwrap_or("").to_string();
        let hits = live.rag.search(&query_text, top_k);
        return send_search_success(&mut stream, &live, &hits);
    }
    ...
    send_error(&mut stream, "unrecognized request -- use QUERY:<heptascript>, SEARCH:<top_k>:<text>, or TRIBES:<heptascript>")
}

// ── Binary encoding (see module doc comment for the exact frame layout;
// row/attr/history encoding matches enkidb-read-server's own exactly) ──

fn send_query_success(stream: &mut TcpStream, result: &heptascript::QueryResult) -> io::Result<()> {
...
// PB-218 (2026-07-20): AkkValue::KakiPk needs its own real tag, not the
// derived-Debug fallback -- `link.target` (concept-mention/section
// particles pointing back at their parent document,
// crates/enkiddb/src/emitter.rs::emit_link) is exactly this type.
