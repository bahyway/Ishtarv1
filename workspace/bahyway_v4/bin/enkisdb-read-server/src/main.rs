//! enkisdb-read-server — EnkiSDB (Stage Database) Read Node, sovereign TCP
//! server. Wraps `enkidb_readnode::CachedReadNode` over the SDB Journal
//! that `enkisdb-write-server` materializes (KakiBorn on arrival, plus
//! SdbValidationPass/SdbValidationFail once the ValidationSweep decides —
//! see that server's own module doc comment for the full pipeline this
//! type sits at the head of) behind the same real, hand-rolled **binary**
//! wire protocol every other EnkiDB Type Read Node in this fleet uses — no
//! JSON, ever, per the Architect's standing law for EnkiDB Types
//! communications. Frame layout is identical to `enkidb-read-server`'s own
//! module doc comment; not repeated here.
//!
...
fn bind_addr() -> String {
    env::var("BIND_ADDR").unwrap_or_else(|_| "0.0.0.0:7003".to_string())
}
fn data_dir() -> PathBuf {
    PathBuf::from(env::var("DATA_DIR").unwrap_or_else(|_| "/data/sdb".to_string()))
}
fn reload_secs() -> u64 {
...
fn main() {
    eprintln!("𒁾 enkisdb-read-server — EnkiSDB (Stage Database) Read Node [in-memory, binary wire]");
