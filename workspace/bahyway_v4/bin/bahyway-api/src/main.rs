...
fn parse_args() -> Config {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--port" => {
                i += 1;
                port = args[i].parse().unwrap_or(8082);
            }
            "--help" | "-h" => {
...
fn main() {
    ...
    eprintln!(
        "[bahyway-api]   GET http://localhost:{}/api/v1/live",
        cfg.port
    );
    eprintln!(
        "[bahyway-api]   GET http://localhost:{}/api/v1/batches",
        cfg.port
    );
    eprintln!(
        "[bahyway-api]   GET http://localhost:{}/api/v1/tribes",
        cfg.port
    );
