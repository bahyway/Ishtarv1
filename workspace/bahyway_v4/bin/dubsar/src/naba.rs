...
fn holo_bright()  -> String { fg(100, 240, 255) }   // bright cyan
fn holo_mid()     -> String { fg(0,   180, 230) }   // mid cyan
fn holo_deep()    -> String { fg(0,   100, 180) }   // deep blue
fn holo_dim_c()   -> String { fg(0,    60, 120) }   // dim blue
fn holo_eye()     -> String { fg(255, 255, 200) }   // warm white
fn holo_gold()    -> String { fg(255, 220,  40) }   // golden
fn holo_green()   -> String { fg(0,   255, 160) }   // sovereign green
fn holo_orange()  -> String { fg(255, 160,   0) }   // warning orange
fn holo_red()     -> String { fg(255,  60,  60) }   // critical red
fn holo_particle()-> String { fg(0,    80, 140) }   // dim particle dots
...
impl NabaMetrics {
    ...
    fn golden_pct(&self) -> f32 {
        if self.total == 0 { 0.0 } else { self.golden as f32 / self.total as f32 * 100.0 }
    }

    fn entropy_pct(&self) -> f32 {
        if self.total == 0 { 0.0 } else { (self.total - self.golden) as f32 / self.total as f32 * 100.0 }
    }

    fn coherence_pct(&self) -> f32 { 100.0 - self.entropy_pct() }
...
fn metrics_lines(m: &NabaMetrics) -> Vec<String> {
    ...
    let pct_bar = |n: usize, total: usize| -> String {
        ...
        format!("{}{}{}{}{}",
            holo_mid(), "█".repeat(filled),
            holo_dim_c(), "░".repeat(10 - filled),
            RST)
    };
    ...
    lines.push(format!("{DIM}Tribe      :{RST}  {}{:#06x}{RST}",
        holo_bright(), m.tribe_id));
    lines.push(sep.clone());
    lines.push(format!("{DIM}Particles  :{RST}  {BOLD}{}{:>9}{RST}",
        holo_bright(), m.total));
    lines.push(format!("{DIM}Golden     :{RST}  {}{:>9}{RST}  {DIM}{:>6.1}%{RST}",
        holo_gold(), m.golden, m.golden_pct()));
    lines.push(format!("  {}", pct_bar(m.golden, m.total.max(1))));
    lines.push(format!("{DIM}Fuzzy      :{RST}  {}{:>9}{RST}",
        holo_orange(), m.fuzzy));
    lines.push(format!("{DIM}Dead       :{RST}  {}{:>9}{RST}",
        holo_dim_c(), m.dead));
    lines.push(sep.clone());
    lines.push(format!("{DIM}Entropy    :{RST}  {}{:>9.3}%{RST}",
        if m.entropy_pct() < 5.0 { holo_green() } else { holo_orange() },
        m.entropy_pct()));
    lines.push(format!("{DIM}Coherence  :{RST}  {}{:>9.3}%{RST}",
        if m.coherence_pct() > 95.0 { holo_green() } else { holo_orange() },
        m.coherence_pct()));
    lines.push(sep.clone());
    lines.push(format!("{DIM}Epoch      :{RST}  {}{:>9}{RST}",
        holo_mid(), m.epoch));
    lines.push(format!("{DIM}Replayed   :{RST}  {DIM}{:>9}{RST}", m.replayed));
    lines.push(format!("{DIM}Written    :{RST}  {DIM}{:>9}{RST}", m.written));
    lines.push(sep.clone());
    ...
    while lines.len() < FIGURE.len() { lines.push("".to_string()); }
...
fn big_ring() -> String {
    ...
    let p = holo_particle();
    let h = holo_dim_c();
    let row: String = ring_chars.iter().enumerate().map(|(i, c)| {
        if i % 5 == 0 {
            format!("{h}●{RST}")
        } else {
            format!("{p}{c}{RST}")
...
fn banner_title(m: &NabaMetrics) -> String {
    format!("{}║{RST}  {BOLD}{}𒀭  N A B A  ─  Sovereign Hologram Agent{RST}  \
             {DIM}BahyWay v4.0  ·  tribe {}{:#06x}{RST}{}  ║{RST}",
        holo_deep(), holo_bright(), holo_mid(), m.tribe_id, holo_deep())
}
...
fn speech_bubble(lines: &[&str]) -> Vec<String> {
    ...
    for line in lines {
        ...
        rows.push(format!("  {border}  {BOLD}{}{}{RST}{}  {border}",
            holo_bright(), line, " ".repeat(pad + 2)));
    }
    rows.push(bot);
    rows
}
...
pub fn render(m: &NabaMetrics) {
    ...
    // Big Ring orbit
    println!("  {}", big_ring());
    println!("  {}  {}  N A B A  {}  Sovereign Hologram Agent  {}  BahyWay v4.0{RST}",
        holo_dim_c(), holo_bright(), holo_dim_c(), holo_dim_c());
    println!();
    println!("{}", banner_sep());
...
pub fn naba_hint(m: &NabaMetrics) {
    ...
    println!(
        "  {}𒀭{RST} NABA  {}◉ {status}{RST}  {DIM}│{RST}  \
         ...
         {}entropy {:.1}%{RST}",
        holo_bright(), sc,
        holo_gold(),   RST, m.golden,
        holo_orange(), RST, m.fuzzy,
        holo_dim_c(),  RST, m.dead,
        if m.entropy_pct() < 5.0 { holo_green() } else { holo_orange() },
        m.entropy_pct(),
    );
}
