...
impl HeptaSector {
    pub fn new(primary: u8, sub1: u8, sub2: u8) -> Result<Self, HeptaError> {
        if primary > 7 { return Err(HeptaError::SectorOutOfRange(primary)); }
        if sub1    > 7 { return Err(HeptaError::SectorOutOfRange(sub1)); }
        if sub2    > 7 { return Err(HeptaError::SectorOutOfRange(sub2)); }
        Ok(Self { primary, sub1, sub2 })
    }
    ...
    pub fn zoom(&self, idx: u8) -> Result<Self, HeptaError> {
        if idx > 7 { return Err(HeptaError::SectorOutOfRange(idx)); }
        if self.sub1 == 0 {
...
impl Kaki7dPk {
    ...
    pub fn new(
        ...
        let packed: u128 =
            ((d1_lon   as u128) << (18 + 18 + 18 + 8 + 10 + 6))
...
mod tests {
    ...
    fn planet_round_trip() {
        for (label, idx) in [("SUN",0u8),("MOON",1),("MERCURY",2),("VENUS",3),
    ...
    fn coord_system_from_str() {
        assert_eq!(CoordSystem::from_str("KAKI_7D").unwrap(),        CoordSystem::Kaki7d);
        assert_eq!(CoordSystem::from_str("KAKI_7D_SACRED").unwrap(), CoordSystem::Kaki7dSacred);
        assert_eq!(CoordSystem::from_str("WGS84").unwrap(),          CoordSystem::Wgs84);
        assert_eq!(CoordSystem::from_str("PROCEDURAL").unwrap(),     CoordSystem::Procedural);
    ...
    fn kaki7d_roundtrip_coordinates() {
        let sector = HeptaSector::root(0).unwrap();
        let pk = Kaki7dPk::new(
            44.3148, 31.9957, 30.0, 1_700_000_000, 1, sector, 5, "NajafWay",
        ).unwrap();
        assert!((pk.longitude() - 44.3148).abs() < 0.002,
            "lon roundtrip: {} ≠ 44.3148", pk.longitude());
        assert!((pk.latitude() - 31.9957).abs() < 0.001,
            "lat roundtrip: {} ≠ 31.9957", pk.latitude());
    }
