...
impl<'a> Lexer<'a> {
    pub fn new(input: &'a str) -> Self {
        Self { src: input.as_bytes(), pos: 0, line: 1 }
    }
    ...
    fn peek_at(&self, d: usize) -> Option<u8> { self.src.get(self.pos + d).copied() }
    ...
    fn peek_word(&self, s: &[u8]) -> bool {
        if self.src.len() < self.pos + s.len() { return false; }
        if &self.src[self.pos..self.pos + s.len()] != s { return false; }
    ...
    fn scan_bracket(&mut self) -> Result<Token, HeptaError> {
        ...
        loop {
            match self.peek() {
                ...
                Some(b) => { inner.push(b as char); self.advance(); }
            }
        }
        let s = inner.trim();
        if let Some(rest) = s.strip_prefix("H7-") {
            // [H7-NN:LABEL]
            let colon = rest.find(':').ok_or_else(|| self.err(format!("invalid node ref '[{}]'", s)))?;
            let idx: u8 = rest[..colon].parse()
                .map_err(|_| self.err(format!("bad sector index in '[{}]'", s)))?;
            let label = rest[colon + 1..].trim().to_string();
            Ok(Token::NodeRef { sector_index: idx, label })
        } else if let Some(rest) = s.strip_prefix("CHORD-7/") {
            // [CHORD-7/N]
            let k: u8 = rest.trim().parse()
    ...
    fn scan_number(&mut self, negative: bool) -> Result<Token, HeptaError> {
        ...
        if self.peek() == Some(b'.') && self.peek_at(1).map(|b| b.is_ascii_digit()).unwrap_or(false) {
            ...
            if self.peek() == Some(b'.') && self.peek_at(1).map(|b| b.is_ascii_digit()).unwrap_or(false) {
                ...
                let s = core::str::from_utf8(&self.src[start..self.pos]).unwrap().to_string();
    ...
    fn apply_unit_or_percent(&mut self, f: f64, _is_float: bool) -> Result<Token, HeptaError> {
        ...
        if self.peek_word(b"km")  { self.pos += 2; return Ok(Token::NumberWithUnit(f, Unit::Km)); }
        if self.peek_word(b"ms")  { self.pos += 2; return Ok(Token::NumberWithUnit(f, Unit::Ms)); }
        if self.peek_word(b"deg") { self.pos += 3; return Ok(Token::NumberWithUnit(f, Unit::Deg)); }
        if self.peek_word(b"m")   { self.pos += 1; return Ok(Token::NumberWithUnit(f, Unit::M)); }
        Ok(Token::Float(f))
    }
    ...
    fn apply_unit_or_percent_i(&mut self, n: i64) -> Result<Token, HeptaError> {
        ...
        if self.peek_word(b"km")  { self.pos += 2; return Ok(Token::NumberWithUnit(n as f64, Unit::Km)); }
        if self.peek_word(b"ms")  { self.pos += 2; return Ok(Token::NumberWithUnit(n as f64, Unit::Ms)); }
        if self.peek_word(b"deg") { self.pos += 3; return Ok(Token::NumberWithUnit(n as f64, Unit::Deg)); }
        if self.peek_word(b"m")   { self.pos += 1; return Ok(Token::NumberWithUnit(n as f64, Unit::M)); }
        Ok(Token::Int(n))
    }
}
...
mod tests {
    ...
    fn node_ref() {
        ...
        assert_eq!(t[0], Token::NodeRef { sector_index: 1, label: "MOON".into() });
    }
    ...
    fn attrib_key() {
        ...
        assert_eq!(t[0], Token::AttribKey { domain: "emergency".into(), key: "level".into() });
    }

    #[test]
    fn attrib_key_with_underscore() {
        let t = tok("[enki:last_cleansed]");
        assert_eq!(t[0], Token::AttribKey { domain: "enki".into(), key: "last_cleansed".into() });
    }
    ...
    fn percent() {
        ...
        if let Token::Percent(p) = t[1] { assert!((p - 0.0085).abs() < 1e-9); }
    ...
    fn block_keywords() {
        ...
        assert_eq!(t, vec![
            Token::KwAnchor, Token::KwSector, Token::KwNode,
            Token::KwBeam, Token::KwPath, Token::KwAttrib,
        ]);
    }
