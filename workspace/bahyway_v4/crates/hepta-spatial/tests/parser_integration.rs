...
fn baghdad_all_sector_planet_indices_are_sequential() {
    ...
    for (i, sector) in f.sectors.iter().enumerate() {
        assert_eq!(
            sector.node.sector_index as usize,
            i + 1,
            "sector {} has wrong index: {}",
            i,
            sector.node.sector_index
        );
    }
}
...
fn baghdad_sadr_city_beam_72_targets_venus() {
    ...
    let beam_72 = sadr
        .beams
        .iter()
        .find(|b| b.chord == ChordType::Chord72)
        .unwrap();
...
fn baghdad_sadr_city_beam_73_targets_jupiter() {
    ...
    let beam_73 = sadr
        .beams
        .iter()
        .find(|b| b.chord == ChordType::Chord73)
        .unwrap();
...
fn najaf_saturn_chord_73_has_high_hidden_path_probability() {
    ...
    assert!(
        prob > 0.90,
        "Saturn {{7/3}} beam should have prob > 0.90, got {}",
        prob
    );
}
