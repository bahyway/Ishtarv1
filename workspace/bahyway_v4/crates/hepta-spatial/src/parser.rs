...
impl Parser {
    ...
    fn expect_node_ref(&mut self) -> Result<NodeRef, HeptaError> {
        match self.advance() {
            Some(Token::NodeRef {
                sector_index,
                label,
            }) => Ok(NodeRef::new(sector_index, label)),
    ...
    fn parse_file(&mut self) -> Result<HeptaFile, HeptaError> {
        ...
        loop {
            match self.peek() {
                ...
                Some(Token::KwAnchor) => {
                    self.advance();
                    anchors.push(self.parse_anchor()?);
                }
                Some(Token::KwSector) => {
                    self.advance();
                    sectors.push(self.parse_sector()?);
                }
                Some(Token::KwNode) => {
                    self.advance();
            ...
            match self.peek() {
                Some(Token::DirVersion) => {
                    ...
                    header.version = match self.advance() {
                        ...
                        Some(t) => {
                            return Err(self.err(format!("expected version string, got {:?}", t)))
                        }
    ...
    fn parse_anchor(&mut self) -> Result<AnchorBlock, HeptaError> {
        ...
        loop {
            match self.peek() {
                ...
                Some(t) => {
                    return Err(self.err(format!("unexpected token in ANCHOR body: {:?}", t)))
                }
            }
        }
    ...
    fn parse_sector(&mut self) -> Result<SectorBlock, HeptaError> {
        ...
        loop {
            match self.peek() {
                ...
                Some(t) => {
                    return Err(self.err(format!("unexpected token in SECTOR body: {:?}", t)))
                }
            }
        }
    ...
    fn parse_beam(&mut self) -> Result<BeamStmt, HeptaError> {
        let is_path = matches!(self.advance(), Some(Token::KwPath));

        let chord = match self.advance() {
            Some(Token::Chord(k)) => ChordType::from_step(k)?,
            Some(t) => {
                return Err(self.err(format!("expected [CHORD-7/2] or [CHORD-7/3], got {:?}", t)))
            }
    ...
    fn parse_attrib(&mut self) -> Result<AttribStmt, HeptaError> {
        ...
        let (domain, key) = match self.advance() {
            ...
            Some(t) => {
                return Err(self.err(format!("expected attrib key [domain:key], got {:?}", t)))
            }
    ...
    fn parse_distance(&mut self) -> Result<Distance, HeptaError> {
        match self.advance() {
            Some(Token::NumberWithUnit(v, Unit::Km)) => Ok(Distance {
                value: v,
                unit: DistanceUnit::Km,
            }),
            Some(Token::NumberWithUnit(v, Unit::M)) => Ok(Distance {
                value: v,
                unit: DistanceUnit::M,
            }),
            Some(Token::Float(f)) => Ok(Distance {
                value: f,
                unit: DistanceUnit::M,
            }),
            Some(Token::Int(n)) => Ok(Distance {
                value: n as f64,
                unit: DistanceUnit::M,
            }),
...
"#;
    ...
    fn parses_anchor() {
        ...
        assert!(
            matches!(a.center, Some(Coord::LatLon { lat, lon }) if (lat - 33.33).abs() < 1e-6 && (lon - 44.39).abs() < 1e-6)
        );
        assert!(
            matches!(&a.radius, Some(d) if d.unit == DistanceUnit::Km && (d.value - 15.0).abs() < 1e-9)
        );
    ...
    fn parses_attribs() {
        ...
        assert_eq!(
            moon.attribs[0].value,
            AttribValue::Symbol("ELEVATED".into())
        );
        ...
        assert_eq!(
            moon.attribs[4].value,
            AttribValue::Text("2026-03-01".into())
        );
    }
    ...
    fn hex_attrib() {
        ...
        let f = parse(src).unwrap();
        assert_eq!(
            f.sectors[0].attribs[0].value,
            AttribValue::HexLiteral(0xFF00)
        );
    }
}
