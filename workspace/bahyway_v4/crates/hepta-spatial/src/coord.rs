...
pub fn assign_sector(anchor_lon: f64, anchor_lat: f64, point_lon: f64, point_lat: f64) -> u8 {
...
/// Each parameter is a distinct required input (the coordinate, its anchor,
/// and the tier/radius/project context) -- not artificial padding, so this
/// is a real API-design tradeoff rather than a mechanical fix.
...
pub fn wgs84_to_kaki7d(
    ...
    Kaki7dPk::new(
        lon, lat, elev_m, epoch_sec, tier, sector, density, project_id,
    )
}
...
mod tests {
    ...
    fn wgs84_to_kaki7d_produces_valid_pk() {
        let pk = wgs84_to_kaki7d(
            44.3148,
            31.9957,       // Najaf lon, lat
            30.0,          // elev_m
            1_700_000_000, // epoch
            1,             // tier
            44.3148,
            31.9957, // anchor = same point
            500.0,   // radius_m
            "NajafWay",
        )
        .unwrap();
