...
impl FieldType {
    pub fn parse_field_type(s: &str) -> Option<Self> {
...
impl FieldMeta {
    ...
    pub fn parse(s: &str) -> Result<Self, String> {
        ...
        if parts.len() < 6 {
            return Err(format!(
                "need 6 colon-separated parts, got {} in: {s}",
                parts.len()
            ));
        }
        ...
        let data_type = FieldType::parse_field_type(parts[1])
        ...
        let max_length = {
            ...
            if l == "-" || l == "0" {
                ...
                Some(
                    l.parse::<u16>()
                        .map_err(|_| format!("bad length '{l}' for field '{name}'"))?,
                )
            }
        };
        ...
        Ok(FieldMeta::new(
            name, data_type, max_length, position, mandatory, nullable,
        ))
    }
}
