...
impl SchemaMismatchAlert {
    pub fn from_report(report: &CompareReport) -> Self {
        ...
        let message = format!(
            ...
            if priority == AlertPriority::High { "HIGH" } else { "NORMAL" },
            report.file_name,
            report.v1_version,
            report.v2_version,
...
impl DeadVerdict {
    pub fn new(particle_count: usize, report: CompareReport) -> Self {
        ...
        alert.message = alert.message.replace(" 0 particles", &format!(" {particle_count} particles"));
        DeadVerdict {
            file_name: report.file_name.clone(),
            particle_count,
            report,
...
mod tests {
    ...
    fn mismatch_report() -> CompareReport {
        let v1 = SchemaVersion::new(1, "civil.registry", 1000, vec![
    ...
    fn dead_verdict_message_contains_particle_count() {
        ...
        assert!(verdict.alert.message.contains("99 particles"), "{}", verdict.alert.message);
    }
