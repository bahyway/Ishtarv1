...
impl ReplicationConsumer {
    ...
    pub fn consume(&mut self, now_epoch: u32) -> ReplicationResult<usize> {
        ...
        while pos < new_bytes.len() {
            ...
            if &new_bytes[pos..pos+4] != &FRAME_MAGIC { break; }
            let frame_len = u32::from_le_bytes(
                new_bytes[pos+4..pos+8].try_into().unwrap()
            ) as usize;
            let total = 8 + frame_len;
            if pos + total > new_bytes.len() { break; }
    ...
    fn verify_event(
        ...
        if ev.seq != expected {
            ...
            return Err(ReplicationError::SequenceGap { expected, got: ev.seq });
        }
    ...
    fn read_new_delta_bytes(&self) -> ReplicationResult<Vec<u8>> {
        let mut file = File::open(&self.delta_path)
    ...
    pub fn stats(&self) -> ConsumerStats { self.stats }
}
...
mod tests {
    ...
    fn temp_path(tag: &str) -> PathBuf {
        std::env::temp_dir().join(format!("enkwal_consumer_{}_{}.enkwal", tag,
    ...
    fn make_pipeline(kaki: u32) -> (ReplicationEmitter, ReplicationBroker, ReplicationConsumer, Arc<Mutex<Vec<Vec<u8>>>>) {
    ...
    fn full_pipeline_emitter_broker_consumer() {
        ...
        em.emit(ReplEventKind::ParticleInsert, b"hello-enkidb".to_vec(), 1_000).unwrap();
