...
impl ReplicationEmitter {
    ...
    pub fn emit(
        ...
        let frame_digest: [u8; 32] = frame[frame.len() - 32..]
            .try_into()
...
mod tests {
    ...
    fn log_file_starts_with_frame_magic() {
        let path = temp_path();
        let mut em = make_emitter(&path);
        em.emit(ReplEventKind::Checkpoint, b"chk".to_vec(), 1_000)
            .expect("emit");
    ...
    fn chained_digest_links_events() {
        ...
        let mut em = make_emitter(&path);
        let ev1 = em
            .emit(ReplEventKind::ParticleInsert, b"first".to_vec(), 1_000)
            .expect("emit");
        let ev2 = em
            .emit(ReplEventKind::ParticleInsert, b"second".to_vec(), 1_001)
            .expect("emit");
