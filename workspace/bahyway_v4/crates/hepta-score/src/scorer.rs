...
use crate::domain::{HeptaScore, HeptaVector, QualityLane, TribeIdealPoint};
...
impl HeptaScorer {
    pub fn new(tribe: TribeIdealPoint) -> Self {
        Self { tribe }
    }

    /// Create with sovereign Arabic MDM weights (BahyWay default).
    pub fn sovereign() -> Self {
        Self::new(TribeIdealPoint::sovereign_arabic_mdm())
    }
    ...
    pub fn filter_lane(
        scored: &[ScoredParticle],
        lane: QualityLane,
    ) -> impl Iterator<Item = &ScoredParticle> {
...
mod tests {
    ...
    fn scored_particles_preserve_kaki() {
        let kaki = [
            0x01u8, 0xAB, 0xCD, 0xEF, 0, 0, 0, 0, 0, 0, 0x10, 0xD4, 0, 0, 0, 0,
        ];
    ...
    fn gem_rate_matches_manual_count() {
        ...
        for _ in 0..40 {
            batch.push(make_particle(1.0));
        }
        for _ in 0..60 {
            batch.push(make_particle(0.1));
        }
