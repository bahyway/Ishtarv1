...
use crate::{QUALITY_DIVISOR, GEM_B11, TRIBE_B11, ACTIVE_B11, FUZZY_DEAD_BOUNDARY};
...
impl HeptaDimension {
    pub const ALL: [Self; 7] = [
        Self::Accuracy, Self::Completeness, Self::Consistency, Self::Validity,
        Self::Uniqueness, Self::Timeliness, Self::Integrity,
    ];
...
impl HeptaVector {
    ...
    pub fn try_new(
        accuracy:     f32, completeness: f32, consistency: f32, validity:  f32,
        uniqueness:   f32, timeliness:   f32, integrity:   f32,
    ) -> Result<Self, HeptaError> {
        let dims = [accuracy, completeness, consistency, validity,
    ...
    pub fn perfect() -> Self { Self { dims: [1.0; 7] } }

    /// Zero particle — all dimensions = 0.0 (worst possible quality).
    pub fn zero() -> Self { Self { dims: [0.0; 7] } }
    ...
    pub fn dimensions(&self) -> &[f32; 7] { &self.dims }

    pub fn get(&self, dim: HeptaDimension) -> f32 { self.dims[dim as usize] }

    pub fn set(&mut self, dim: HeptaDimension, value: f32) -> Result<(), HeptaError> {
        if !(0.0..=1.0).contains(&value) {
            return Err(HeptaError::DimensionOutOfRange { dim: dim as usize, value });
        }
    ...
    pub fn to_b11(&self, tribe: &TribeIdealPoint) -> u8 {
        (self.health_score(tribe) * QUALITY_DIVISOR).round().clamp(0.0, 240.0) as u8
    }
    ...
    pub fn weakest_dimension(&self) -> HeptaDimension {
        let (idx, _) = self.dims.iter().enumerate()
    ...
    pub fn raw_distance(&self, tribe: &TribeIdealPoint) -> f32 {
        self.dims.iter().zip(tribe.ideal().iter())
            .map(|(p, t)| (p - t).powi(2))
            .sum::<f32>()
            .sqrt()
    }
...
impl std::ops::Index<usize> for HeptaVector {
    type Output = f32;
    fn index(&self, i: usize) -> &Self::Output { &self.dims[i] }
}
...
impl TribeIdealPoint {
    ...
    pub fn new(
        ...
        Ok(Self { ideal, weights, name: name.into() })
    }
    ...
    pub fn ideal(&self)   -> &[f32; 7] { &self.ideal }
    pub fn weights(&self) -> &[f32; 7] { &self.weights }
...
impl QualityLane {
    pub fn from_b11(b11: u8) -> Self {
        if b11 >= GEM_B11         { Self::Gem         }
        else if b11 >= TRIBE_B11  { Self::TribeMember }
        else if b11 >= ACTIVE_B11 { Self::Active      }
        else if b11 >= FUZZY_DEAD_BOUNDARY { Self::Fuzzy }
        else                      { Self::Dead        }
    }
...
impl HeptaScore {
    pub fn compute(p: &HeptaVector, tribe: &TribeIdealPoint) -> Self {
        ...
        let weakest = dim_contributions.iter().enumerate()
            .max_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, _)| HeptaDimension::ALL[i])
            .unwrap_or(HeptaDimension::Accuracy);

        let strongest = dim_contributions.iter().enumerate()
            .min_by(|a, b| a.1.partial_cmp(b.1).unwrap())
            .map(|(i, _)| HeptaDimension::ALL[i])
            .unwrap_or(HeptaDimension::Integrity);

        Self {
            health, b11, lane, weighted_dist,
            dimensions: dims,
            dim_contributions,
            weakest, strongest,
    ...
    pub fn is_gem(&self)  -> bool { self.lane == QualityLane::Gem  }
    pub fn is_dead(&self) -> bool { self.lane == QualityLane::Dead }

    /// Quality percentage (0–100 %)
    pub fn quality_pct(&self) -> f32 { self.b11 as f32 / QUALITY_DIVISOR * 100.0 }
...
mod tests {
    ...
    fn tribe() -> TribeIdealPoint { TribeIdealPoint::sovereign_arabic_mdm() }
    ...
    fn lane_orbital_radius_ordering() {
        assert!(QualityLane::Gem.orbital_radius_factor() <
                QualityLane::TribeMember.orbital_radius_factor());
        assert!(QualityLane::Dead.orbital_radius_factor() >
                QualityLane::Active.orbital_radius_factor());
    }
    ...
    fn dimension_labels_non_empty() {
        for d in HeptaDimension::ALL { assert!(!d.label().is_empty()); }
    }
