...
pub struct DubSarApp {
    ...
    pub bus:             bee_mdm_bus::BusHandle, // real BeeMDM ETL gate state (Nergal + EriduOS panels)
    pub eridu:           crate::panels::eridu_layout::EriduState,
...
impl DubSarApp {
    pub fn new(cc: &CreationContext) -> Self {
        ...
        DubSarApp {
            ...
            bus,
            eridu:          crate::panels::eridu_layout::EriduState::default(),
            ...
            .show(ctx, |ui| {
                ui.horizontal_centered(|ui| {
                    ...
                    let ent_r = ui.add(egui::Button::new(
                        RichText::new(if ent_active { "Enterprise Apps *" } else { "Enterprise Apps v" })
                            .color(if ent_active { theme::GOLD } else { theme::TEXT_DIM })
                            .size(13.0))
                        .fill(if ent_active { egui::Color32::from_rgb(24, 22, 10) } else { egui::Color32::TRANSPARENT })
                        ...
                        egui::Window::new("ent_menu_win")
                            ...
                            .show(ui.ctx(), |ui| {
                                ...
                                for (panel, label) in ent_panels {
                                    ...
                                    let col = if active { theme::GOLD } else { egui::Color32::from_rgb(180, 190, 205) };
                                    if ui.add(egui::Button::new(RichText::new(*label).color(col).size(13.0))
                                        .fill(if active { egui::Color32::from_rgb(28, 26, 14) } else { egui::Color32::TRANSPARENT })
                    ...
                    let ai_r = ui.add(egui::Button::new(
                        RichText::new(if ai_active { "AI Agents *" } else { "AI Agents v" })
                            .color(if ai_active { theme::GOLD } else { theme::TEXT_DIM })
                            .size(13.0))
                    ...
                    // it becomes alive" -- the Architect's own framing).
                    {
                        ...
                        if ui.add(egui::Button::new(
                            RichText::new("𒂍𒁾 Glossary")
                                ...
                                .size(13.0))
                            .fill(if active { egui::Color32::from_rgb(24, 22, 10) } else { egui::Color32::TRANSPARENT })
                            .rounding(egui::Rounding::same(4.0))
                        ).clicked() {
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        ...
                        if let Ok(b) = self.bus.read() {
                            ...
                            ui.label(RichText::new(
                                format!("B11:{}/240  Gold:{}  Proc:{}", b11, b.total_golden, b.total_processed))
                                .color(egui::Color32::from_rgb(100, 110, 130)).size(10.0));
                            ui.separator();
                            ui.label(RichText::new(format!("B11:{b11}")).color(b11_col).size(11.0).strong());
                            ui.separator();
                            ui.label(RichText::new("𒁾 BahyWay.Ecosystem v4.0")
                                .color(egui::Color32::from_rgb(160, 130, 50)).size(11.0));
                        }
                    });
                });
            });
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            match self.panel {
                ActivePanel::EriduOS => crate::panels::eridu_layout::draw(
                    ui, &mut self.eridu, &self.bus, &self.particles,
                    &self.identities, &self.journal, &self.snapshots,
                ),
                ActivePanel::Hepta => crate::panels::hepta::draw(
                    ui, &self.particles, &mut self.selected,
                ),
...
#[rustfmt::skip]
const PARTICLE_SEED: &[(&str, &str, &[u8], f32, [f32;7], [u8;3], &str, &str, f32)] = &[
...
-> (Vec<ParticleDemo>, Vec<IdentityKaki>, Journal, Vec<SnapshotRecord>)
{
    ...
    for &(pid, label, state_b, hps, dims, rgb, seg_ref, sector_name, defect) in PARTICLE_SEED {
        ...
        let identity = IdentityKaki::try_from_kaki(minter.identity(KakiRole::Zikru))
            .expect("identity mint");

        // ── Epoch 100: REGISTERED ─────────────────────────────────────
        journal.append(event(&minter, &identity, 100, vec![
        ...
        journal.append(event(&minter, &identity, 200, vec![
        ...
        journal.append(event(&minter, &identity, 300, vec![
        ...
        match state_b {
            b"GOLDEN" => {
                ...
                journal.append(event(&minter, &identity, 350, vec![
                    eav(story_engine::projection::ATTR_SNAPSHOT_DATE, b"1748563200"),
                    eav(ATTR_EVENT_TYPE, b"SNAPSHOT_TAKEN"),
                ])).unwrap();
                snapshots.push(SnapshotRecord::at_birth(identity.clone(), 1_748_563_200));

                // Epoch 400: VERIFIED
                journal.append(event(&minter, &identity, 400, vec![
                    eav(ATTR_QUALITY,    quality_str.as_bytes()),
            ...
            b"FUZZY" => {
                ...
                // Epoch 400: MONITORED
                journal.append(event(&minter, &identity, 400, vec![
                    eav(ATTR_EVENT_TYPE, b"MONITORED"),
                ])).unwrap();
            }
            _ => {
                ...
                journal.append(event(&minter, &identity, 400, vec![
                    eav(ATTR_EVENT_TYPE, b"QUARANTINED"),
                ])).unwrap();
            }
        }

        // ── Epoch 500: LIVE (current heartbeat) ───────────────────────
        journal.append(event(&minter, &identity, 500, vec![
            eav(ATTR_FRESHNESS,  freshness_str.as_bytes()),
...
fn event(
    ...
    let ek = EventKaki::try_from_kaki(minter.event(KakiRole::Zikru))
        .expect("event mint");
    JournalEntry::new(ek, identity.clone(), epoch, triples)
}
