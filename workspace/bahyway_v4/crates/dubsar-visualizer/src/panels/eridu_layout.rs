//! 𒁾 EriduOS v4.0 — Sovereign IDE Layout
//!
//! Dual-sidebar layout:
    ...
    fn default() -> Self {
        ...
        Self {
            ...
            nisaba_orch: NisabaOrchestrator::new(
                ...
                LAMASSU_MAX_EPSILON, LAMASSU_R_MAX, LAMASSU_H_MAX,
            ),
        }
    }
}
...
pub fn draw(
    ...
    egui::TopBottomPanel::top("eridu_top")
        ...
        .show_inside(ui, |ui| {
            ui.horizontal_centered(|ui| {
                ui.label(RichText::new("𒁾  EriduOS v4.0")
                ...
                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                    let nisaba_col = if state.float.nisaba { theme::GOLD } else { theme::TEXT_DIM };
                    if ui.button(RichText::new("𒀭 NISABA").color(nisaba_col).strong()).clicked() {
    ...
    // ── Floating panels ─────────────────────────────────────────────
    draw_floats(ui.ctx(), state, &bus_state, particles, identities, journal, snapshots);
}
...
fn draw_left_sidebar(
    ...
    mini_panel(ui, "⬡  BeeMDM", "Pipeline Gate Monitor",
    ...
    if let Some(bus) = bus_state {
        ...
        stat_mini(ui, "Jailed (IRKALLA)", &fmt_n(bus.total_quarantined), Color32::from_rgb(200, 40, 40));
        ...
        ui.painter().rect_filled(bar, Rounding::same(2.0), Color32::from_rgb(20, 20, 40));
...
fn draw_centre(ui: &mut Ui, state: &mut EriduState) {
    ...
    ui.horizontal(|ui| {
        ...
        for tab in tabs {
            ...
            let fill   = if active { theme::BG_HOVER } else { Color32::TRANSPARENT };
        ...
        ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
            ui.label(RichText::new(ext).color(theme::TEXT_DIM).size(10.0).monospace());
        });
    });
    ...
    egui::Frame::none()
        ...
        .show(ui, |ui| {
            ...
            ui.label(RichText::new("── Output (scripted preview, not live execution) ──")
                .color(theme::TEXT_DIM).size(10.0));
            ui.add_space(2.0);
            egui::ScrollArea::both()
...
fn draw_floats(
    ...
    if state.float.nisaba {
        Window::new("𒀭 NISABA — Sovereign Hologram Agent")
            ...
            .show(ctx, |ui| {
                draw_nisaba(ui, &mut state.nisaba, &mut state.nisaba_orch,
...
fn draw_nisaba(
    ...
    ui.label(RichText::new("𒀭 NISABA — Internal Interface & TDA Sentinel")
        .color(Color32::from_rgb(0, 220, 255)).size(13.0).strong());
    ui.label(RichText::new("Observes, orchestrates, proposes — Architect ratifies (CSR-08)")
    ...
    ui.horizontal(|ui| {
        stat_mini(ui, "Steward queue", &digest.steward_queue_len.to_string(),
            if digest.steward_queue_len > 0 { Color32::from_rgb(220, 80, 80) } else { theme::TEXT_DIM });
        ui.add_space(10.0);
        stat_mini(ui, "Orbit scans", &digest.tribe_readings_count.to_string(), theme::CYAN);
        ui.add_space(10.0);
        stat_mini(ui, "Notes", &digest.note_count.to_string(), theme::GOLD);
        ui.add_space(10.0);
        stat_mini(ui, "Self-resolved", &digest.autonomous_resolutions_count.to_string(),
            Color32::from_rgb(0, 220, 120));
    });
    if let Some((env, _class)) = orch.active_autonomy() {
        ui.label(RichText::new(format!("𒀭 Autonomy grant active: {env:?}, Watch-band only"))
            .color(Color32::from_rgb(0, 220, 120)).size(9.0));
    } else {
        ui.label(RichText::new("𒀭 No autonomy grant — fully propose-only")
            .color(theme::TEXT_DIM).size(9.0));
    }
    ui.add_space(4.0);
...
fn nisaba_respond(question: &str, orch: &NisabaOrchestrator) -> String {
    ...
    if q.contains("alert") || q.contains("steward") || q.contains("queue") || q.contains("digest") {
        ...
        if digest.steward_queue_len == 0 {
            "𒀭 The Data Steward queue is empty — no high-priority findings raised. \
             Try \"Scan StoryEngine (Alerts)\" to run a fresh pass.".into()
        } else {
            format!(
    ...
    } else if q.contains("orbit") || q.contains("pattern") || q.contains("topology") || q.contains("golden") && q.contains("signature") {
        ...
        "KAKI is the 16-byte sovereign primary key.\n\
         ...
         Minted via KakiMinter — never manually assigned.".into()
    ...
    } else if q.contains("nergal") || q.contains("threat") || q.contains("malware") || q.contains("irkalla") {
        format!(
        ...
        "BeeMDM ETL — 7 Hepta gates (G1..G7), tracked live via\n\
         bee-mdm-bus::GateLoads. G7 is the Nergal threat gate.".into()
        ...
        "Way v2.0 (.way) — Sovereign Security Policy Language.\n\
         ...
         Hepta weights must sum to 1.0 across 7 dimensions.".into()
    } else {
        format!("𒀭 NISABA: I received your query about \"{question}\".\n\
    ...
    match tab {
        LangTab::HeptaScript => {
            ...
            "✓ HeptaScript (.hepta) — query parsed (preview, not live execution)\n\
             → This panel does not yet run against a live EnkiDB journal.".into()
        }
        LangTab::AkkadianAol => {
        ...
        LangTab::Way => {
            if source.contains("sovereign") && source.contains("hepta") {
                "✓ Way v2.0 (.way) — policy parsed (preview)\n\
                 → Weight sum check requires live SemanticValidator (not wired here).".into()
            } else {
    ...
    ui.columns(cols.len(), |colui| {
        for (i, (label, val, col)) in cols.iter().enumerate() {
            colui[i].vertical_centered(|ui| {
                ui.label(RichText::new(fmt_n(*val as u64)).color(*col).size(22.0).strong());
                ui.label(RichText::new(*label).color(Color32::from_rgb(140, 140, 160)).size(10.0));
            });
        }
    });
    ui.add_space(8.0);
...
fn beemdm_float_view(ui: &mut Ui, bus: &BeeMdmBusState) {
    ...
    for (i, &label) in labels.iter().enumerate() {
        ...
        let col  = if i == 6 { Color32::from_rgb(200, 40, 60) } else { Color32::from_rgb(0, 180, 100) };
        ui.horizontal(|ui| {
...
fn nergal_float_view(ui: &mut Ui, bus: &BeeMdmBusState) {
    ...
    ui.separator();
    let cats = ["Viruses", "Ransomware", "Trojans", "Spyware", "Worms", "Adware", "Rootkits"];
    ...
    ui.columns(4, |cols| {
        for (i, &cat) in cats.iter().enumerate() {
            let n = bus.gates.counts[i];
            cols[i % 4].group(|ui| {
                ui.label(RichText::new(cat).color(Color32::from_rgb(160, 60, 80)).size(10.0));
                ui.label(RichText::new(format!("{n}"))
                    .color(if n > 0 { Color32::from_rgb(220, 40, 60) } else { Color32::from_rgb(60, 60, 80) })
                    .size(18.0).strong());
            });
        }
    });
