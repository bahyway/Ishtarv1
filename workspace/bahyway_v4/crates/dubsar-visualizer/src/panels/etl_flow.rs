...
use egui::{
    Align2, Color32, FontId, Painter, Pos2, Rect, RichText, Rounding, Stroke, Ui, Vec2,
};
...
pub fn draw(ui: &mut Ui, state: &EtlFlowState) {
    ...
    ui.horizontal(|ui| {
        ...
        let color = if remaining < 100 { Color32::YELLOW } else { Color32::WHITE };
        ...
        let bar = format!("[{}{}]", "█".repeat(filled.min(12)), "░".repeat(12 - filled.min(12)));
    ...
    let (response, painter) = ui.allocate_painter(
        Vec2::new(available.x, panel_height),
        egui::Sense::hover(),
    );
    ...
    // ── Draw tiers ────────────────────────────────────────────────────────
    draw_tier(
        &painter, sdb_cx, sdb_cy, box_w, box_h,
        "EnkiSDB", "Stage DB",
        &format!("Pending: {}\nPromoted: {}\nQuarantined: {}\nMalware hits: {}",
            state.sdb_pending, state.sdb_promoted, state.sdb_quarantined, state.sdb_malware_hits),
        EtlStageColor::Pending, state.sdb_tribal_rgb,
    );

    draw_tier(
        &painter, odb_cx, odb_cy, box_w, box_h,
        "EnkiODB", "Operational DB",
        &format!("Active: {}", state.odb_total),
        EtlStageColor::Active, state.odb_tribal_rgb,
    );

    draw_tier(
        &painter, dw_cx, dw_cy, box_w, box_h,
        "EnkiDW", "Datawarehouse",
        &format!("Archived: {}", state.dw_total),
        EtlStageColor::Archived, state.dw_tribal_rgb,
    );

    draw_tier(
        &painter, qdb_cx, qdb_cy, box_w, box_h,
        "EnkiQDB", "Quarantine",
        &format!("Quarantined: {}\nMalware: {}", state.qdb_total, state.qdb_malware_count),
        EtlStageColor::Quarantined, state.qdb_tribal_rgb,
    );

    draw_tier(
        &painter, db_cx, db_cy, box_w, box_h,
        "EnkiDB", "Transactional",
        &format!("GUI commits: {}", state.db_total),
        EtlStageColor::Processing, state.db_tribal_rgb,
    );
...
fn draw_tier(
    ...
    painter.rect_stroke(rect, Rounding::same(8.0), Stroke::new(2.0, stage_col));
    ...
    painter.circle_stroke(tribal_dot, 6.0, Stroke::new(1.0, Color32::WHITE));
...
mod tests {
    ...
    fn stage_colors_distinct() {
        ...
        ].iter().map(|s| s.to_color32()).collect();
    ...
    fn etl_stage_labels_non_empty() {
        for stage in [
            EtlStageColor::Pending, EtlStageColor::Processing,
            EtlStageColor::Active, EtlStageColor::Archived, EtlStageColor::Quarantined,
        ] {
