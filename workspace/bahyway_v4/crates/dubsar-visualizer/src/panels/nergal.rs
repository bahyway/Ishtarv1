...
pub fn draw(ui: &mut Ui, bus: &Arc<RwLock<BeeMdmBusState>>) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(
            RichText::new(format!("Malware hits: {}", state.etl.sdb_malware_hits))
                .color(if state.etl.sdb_malware_hits > 0 { COL_RED } else { COL_DIM })
                .size(12.0),
        );
        ui.separator();
        ui.label(
                    ...
                    .show(ui, |ui| {
                        ...
                        ui.label(
                            RichText::new(format!("{n}"))
                                .color(if n > 0 { col } else { Color32::from_rgb(40, 20, 25) })
                                .size(24.0)
                                .strong(),
                        );
                    });
    ...
    ui.horizontal(|ui| {
        if threats > 0
            && ui
                ...
                .clicked()
        {
            let _ = bus.write().map(|s| s.send_cmd(bee_mdm_bus::BusCmd::ForceSweep));
        }
        if ui
            .add(
                egui::Button::new(RichText::new("Accelerate Scan").color(Color32::WHITE).size(12.0))
            ...
            .clicked()
        {
            let _ = bus.write().map(|s| s.send_cmd(bee_mdm_bus::BusCmd::AccelerateIntake));
        }
    });
    ...
    ui.label(RichText::new("Gate Loads (G1..G7)").color(COL_DIM).size(11.0).strong());
    ui.add_space(4.0);
    ...
    for (i, &label) in CAT_NAMES.iter().enumerate() {
        ...
        let col = CAT_COLS[i];
        ui.horizontal(|ui| {
            ui.label(RichText::new(format!("{label:<12}")).color(COL_DIM).size(10.0).monospace());
            let (bar, _) = ui.allocate_exact_size(egui::Vec2::new(200.0, 12.0), egui::Sense::hover());
            ui.painter().rect_filled(bar, Rounding::same(2.0), Color32::from_rgb(20, 8, 10));
            if frac > 0.0 {
                let fill = egui::Rect::from_min_size(bar.min, egui::Vec2::new(200.0 * frac, 12.0));
                ui.painter().rect_filled(fill, Rounding::same(2.0), col);
            }
            ui.label(RichText::new(format!("{n:>3}")).color(Color32::from_rgb(180, 140, 150)).size(10.0).monospace());
        });
    }
