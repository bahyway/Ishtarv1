...
impl AgentDecision {
    ...
    pub fn new(
        ...
        let content = format!("{}:{}:{}", kind.as_str(), epoch, parties.join(","));
...
mod tests {
    ...
    fn two_decisions_have_distinct_kakis() {
        let d1 = AgentDecision::new(
            DecisionKind::AlgebraSolved,
            DecisionResult::Value(2.0, "x=2".into()),
            1,
            vec![],
        );
        let d2 = AgentDecision::new(
            DecisionKind::AlgebraSolved,
            DecisionResult::Value(3.0, "x=3".into()),
            2,
            vec![],
        );
