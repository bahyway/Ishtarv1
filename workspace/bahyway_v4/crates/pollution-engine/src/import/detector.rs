...
impl DomainConfidence {
    ...
    fn compute_combined(&mut self) {
        self.combined =
            self.field_score * 0.60 + self.source_score * 0.25 + self.heuristic_score * 0.15;
    }
}
...
impl DetectionResult {
    ...
    pub fn scores_summary(&self) -> String {
        format!(
            "AIR={:.2} WATER={:.2} OIL={:.2}",
            self.air.combined, self.water.combined, self.oil.combined,
        )
    }
...
impl SchemaDetector {
    ...
    fn match_field_table(&self, fields: &[String], table: &[(&str, f32)]) -> (f32, Vec<String>) {
        ...
        for (pattern, weight) in table {
            if fields
                .iter()
                .any(|f| f.contains(pattern) || pattern.contains(f.as_str()))
            {
    ...
    fn score_value_heuristics(
        ...
        for row in &sample {
            ...
            if let Some(v) =
                self.parse_any_of(row, &["aqi", "air_quality_index", "us_aqi", "european_aqi"])
            {
