...
impl SourceAdapter for NoaaAdiosAdapter {
    fn name(&self)        -> &'static str { "noaa_adios" }
    fn description(&self) -> &'static str { "NOAA ADIOS Oil DB — Chemical fingerprint data (JSON)" }
...
impl NoaaAdiosAdapter {
    fn row_to_reading(
        ...
        let lat = row.get("lat").and_then(|v| v.parse().ok()).unwrap_or(0.0f64);
        let lon = row.get("lon").and_then(|v| v.parse().ok()).unwrap_or(0.0f64);

        // PAH ratio from SARA fractions: aromatics / (saturates + aromatics + resins + asphaltenes)
        let aromatics  = row.get("sara_aromatics")
            .or_else(|| row.get("sara_fraction_aromatics"))
            .and_then(|v| v.parse::<f64>().ok());
        let saturates  = row.get("sara_saturates")
            .or_else(|| row.get("sara_fraction_saturates"))
            .and_then(|v| v.parse::<f64>().ok());
        let resins     = row.get("sara_resins").and_then(|v| v.parse::<f64>().ok());
        let asphaltenes= row.get("sara_asphaltenes").and_then(|v| v.parse::<f64>().ok());

        let pah_ratio = match (aromatics, saturates) {
            (Some(a), Some(s)) => {
                let total = s + a
                    + resins.unwrap_or(0.0)
                    + asphaltenes.unwrap_or(0.0);
                if total > 0.0 { Some((a / total).clamp(0.0, 1.0)) } else { None }
            }
        ...
        let weathering_index = row.get("weathering")
            .and_then(|v| v.parse::<f32>().ok())
            .map(|w| (w / 100.0).clamp(0.0, 1.0));

        // TPHC proxy from API gravity: lower API = heavier = higher TPHC
        let api_gravity = row.get("physical_properties_api_gravity")
        ...
        let sensor_id = row.get("adios_oil_id")
