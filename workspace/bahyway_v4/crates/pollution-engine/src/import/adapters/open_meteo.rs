...
impl SourceAdapter for OpenMeteoAdapter {
    ...
    fn description(&self) -> &'static str { "Open-Meteo AQ — Hourly air quality forecast/history (JSON)" }
    fn supported_formats(&self) -> &[PayloadFormat] {
        &[PayloadFormat::JsonArray, PayloadFormat::NdJson, PayloadFormat::Csv]
    }
    ...
    fn normalize(
        ...
        for (loc_idx, row) in payload.rows.iter().enumerate() {
            ...
            let (lat, lon) = match (lat, lon) {
                ...
                _ => {
                    failed.push((loc_idx, ImportError::FieldMappingFailed {
            ...
            for t in 0..n {
                ...
                if pm2_5.is_none() && pm10.is_none() { continue; }
...
impl OpenMeteoAdapter {
    ...
    fn parse_array(&self, raw: &str) -> Vec<Option<f64>> {
        raw.split(',')
           .map(|s| {
               let t = s.trim();
               if t.is_empty() || t == "null" { None } else { t.parse().ok() }
           })
           .collect()
    }
