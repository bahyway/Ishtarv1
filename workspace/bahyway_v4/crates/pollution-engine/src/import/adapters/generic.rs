...
impl SourceAdapter for GenericAdapter {
    ...
    fn description(&self) -> &'static str { "Generic heuristic — unknown source, field-pattern fallback" }
...
enum RowDomain { Air, Water, Oil, Unknown }
...
impl GenericAdapter {
    ...
    fn classify_row(&self, row: &HashMap<String, String>) -> RowDomain {
        ...
        let air_score: usize = [
            "pm2_5","pm25","pm10","aqi","no2","o3","so2","co","ozone",
            "particulate","european_aqi","us_aqi","pollutant"
        ].iter().filter(|&&f| has(f)).count();

        let water_score: usize = [
            "turbidity","tds","wqi","dissolvedoxygen","do_mg","ph","bod","cod",
            "conductivity","nitrate","phosphate","salinity","ntu"
        ].iter().filter(|&&f| has(f)).count();

        let oil_score: usize = [
            "tphc","pah","sheen","spill","hydrocarbon","crude","petroleum",
            "weathering","tar","dispersant","api_gravity","tpah"
        ].iter().filter(|&&f| has(f)).count();
    ...
    fn extract_coords(
        ...
        let lat_keys = ["lat","latitude","y","coord_lat","point_lat","decimallatitude"];
        let lon_keys = ["lon","longitude","x","coord_lon","point_lon","decimallongitude"];

        let lat = lat_keys.iter()
    ...
    fn extract_sensor_id(&self, row: &HashMap<String, String>, i: usize) -> String {
        let id_keys = ["id","sensor_id","stationid","monitoringsiteid","locationid","name","station"];
        id_keys.iter()
    ...
    fn extract_origin_hint(&self, row: &HashMap<String, String>, domain: &str) -> String {
        ...
        let raw = hint_keys.iter()
            .find_map(|&k| row.get(k).cloned())
            .unwrap_or_default()
            .to_uppercase();

        if raw.contains("NATURAL")       { return "NATURAL".into(); }
        if raw.contains("SPILL")         { return "SPILL".into(); }
        if raw.contains("CHEMICAL")      { return "CHEMICAL".into(); }
        if raw.contains("ANTHROPOGENIC") { return "ANTHROPOGENIC".into(); }
    ...
    fn normalize_air_row(
        ...
        let aqi = self.fuzzy_f64(row, &["aqi","air_quality_index","european_aqi","us_aqi"])
            .map(|v| v as u16);
        ...
        Ok(RawSensorReading {
            ...
            lat, lon,
            altitude_m:   self.fuzzy_f64(row, &["altitude","elevation","alt"]).map(|v| v as f32),
            pm2_5, pm10,
            ...
            aqi,
            turbidity_ntu: None, tds_mg_l: None, do_mg_l: None,
            ph: None, bod_mg_l: None, wqi: None,
            tphc_ug_l: None, pah_ratio: None, spill_area_km2: None,
            sheen_detected: None, weathering_index: None,
        })
    }
    ...
    fn normalize_water_row(
        ...
        if turbidity_ntu.is_none() && tds_mg_l.is_none()
            && do_mg_l.is_none() && ph.is_none()
        {
            return Err(ImportError::FieldMappingFailed {
        ...
        let wqi = self.fuzzy_f64(row, &["wqi","water_quality"]).map(|v| v as f32);
        ...
        Ok(RawSensorReading {
            ...
            lat, lon,
            altitude_m:   None,
            pm2_5: None, pm10: None, no2_ppb: None,
            o3_ppb: None, so2_ppb: None, co_ppm: None, aqi: None,
            turbidity_ntu,
    ...
    fn normalize_oil_row(
        ...
        Ok(RawSensorReading {
            ...
            weathering_index: self.fuzzy_f64(row, &["weathering","evaporation"]).map(|v| v as f32),
        })
    }
}
