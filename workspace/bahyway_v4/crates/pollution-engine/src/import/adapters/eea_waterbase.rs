...
impl SourceAdapter for EeaWaterbaseAdapter {
    fn name(&self)        -> &'static str { "eea_waterbase" }
    fn description(&self) -> &'static str { "EEA Waterbase ICM — European water quality (CSV)" }
    ...
    fn normalize(
        ...
        for (i, row) in payload.rows.iter().enumerate() {
            ...
            if let Some(quality) = row.get("resultqualitybadge") {
                if quality.to_lowercase().contains("bad") { continue; }
            }

            let site_id = row.get("monitoringsiteid")
                .or_else(|| row.get("stationid"))
                .or_else(|| row.get("id"))
                .cloned()
                .unwrap_or_else(|| format!("EEA-{}", i));

            let time_key = row.get("phenomenontimestart")
...
impl EeaWaterbaseAdapter {
    fn row_to_reading(
        ...
        Ok(RawSensorReading {
            sensor_id:        row.get("sensor_id").cloned().unwrap_or_else(|| format!("EEA-{}", i)),
            domain:           "WATER".into(),
