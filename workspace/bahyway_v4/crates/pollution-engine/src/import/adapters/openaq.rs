...
impl SourceAdapter for OpenAqAdapter {
    fn name(&self)        -> &'static str { "openaq" }
    fn description(&self) -> &'static str { "OpenAQ v3 — Global air quality measurements (JSON)" }
    ...
    fn normalize(
        ...
        for (i, row) in payload.rows.iter().enumerate() {
            ...
            match param.as_str() {
                "pm25" | "pm2.5" | "pm2_5" => { entry.insert("pm2_5".into(), value); }
                "pm10"                       => { entry.insert("pm10".into(),  value); }
                "no2"                        => { entry.insert("no2_ppb".into(), value); }
                "o3"  | "ozone"              => { entry.insert("o3_ppb".into(),  value); }
                "so2"                        => { entry.insert("so2_ppb".into(), value); }
                "co"                         => { entry.insert("co_ppm".into(),  value); }
...
impl OpenAqAdapter {
    fn row_to_reading(
        ...
        Ok(RawSensorReading {
            sensor_id:        row.get("sensor_id").cloned().unwrap_or_else(|| format!("OAQ-{}", i)),
            domain:           "AIR".into(),
            origin_hint:      row.get("origin_hint").cloned().unwrap_or_else(|| "ANTHROPOGENIC".into()),
            lat, lon,
            altitude_m:       None,
            pm2_5,
            pm10,
