...
impl AzugaEngine {
    pub fn new() -> Self {
        Self {
            catalogue: ThermalSignature::catalogue(),
        }
    }
...
mod tests {
    ...
    fn cold_torso_triggers_security_overlap_and_alert() {
        let scan = BodyScan::from_deviations(
            BodyType::Man,
            &[(ZoneId::Chest, -1.5), (ZoneId::Abdomen, -1.8)],
            0,
        );
        let result = engine().infer(&scan);
        assert!(
            result.security_overlap,
            "cold torso must trigger security_overlap"
        );
    ...
    fn med_score_bounded_zero_to_one() {
        for dev in [-2.0f32, -0.5, 0.0, 1.0, 3.0] {
            ...
            let result = engine().infer(&scan);
            assert!(
                result.med_score >= 0.0 && result.med_score <= 1.0,
                "med_score={} for dev={dev}",
                result.med_score
            );
        }
    }
