...
impl SymmetryAnalysis {
    pub fn compute(scan: &BodyScan) -> Self {
        ...
        let shoulder_diff = (scan.zone(ZoneId::LShoulderZ).temperature
            - scan.zone(ZoneId::RShoulderZ).temperature)
            .abs();
        ...
        let arm_ul_diff = (scan.zone(ZoneId::LUpperArm).temperature
            - scan.zone(ZoneId::RUpperArm).temperature)
            .abs();
        let arm_ll_diff = (scan.zone(ZoneId::LForearm).temperature
            - scan.zone(ZoneId::RForearm).temperature)
            .abs();
        let arm_symmetry = (1.0 - (arm_ul_diff + arm_ll_diff) / 4.0).clamp(0.0, 1.0);

        // ── Bilateral leg symmetry ─────────────────────────────────────────────
        let hip_diff =
            (scan.zone(ZoneId::LHip).temperature - scan.zone(ZoneId::RHip).temperature).abs();
        let thigh_diff =
            (scan.zone(ZoneId::LThigh).temperature - scan.zone(ZoneId::RThigh).temperature).abs();
        ...
        let torso_mean =
            (scan.zone(ZoneId::Chest).temperature + scan.zone(ZoneId::Abdomen).temperature) / 2.0;
        ...
        if shoulder_diff > 1.0 {
            notes.push(format!(
                "Shoulder asymmetry: {:.2}°C — investigate",
                shoulder_diff
            ));
        }
...
mod tests {
    ...
    fn uniform_scan_gives_perfect_symmetry() {
        ...
        assert!(
            (sym.overall_symmetry - 1.0).abs() < 0.01,
            "overall_symmetry={}",
            sym.overall_symmetry
        );
    }
    ...
    fn torso_arm_inversion_detected_when_arms_hotter() {
        ...
        let sym = SymmetryAnalysis::compute(&scan);
        assert!(
            sym.torso_arm_inversion > 0.0,
            "torso_arm_inversion={}",
            sym.torso_arm_inversion
        );
    }
    ...
    fn fever_pattern_gives_positive_head_torso_gradient() {
        ...
        let sym = SymmetryAnalysis::compute(&scan);
        assert!(
            sym.head_torso_gradient > 1.5,
            "gradient={}",
            sym.head_torso_gradient
        );
