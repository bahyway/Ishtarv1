...
use nusku_engine::{BodyTribe, ZoneId};
    ...
    pub fn catalogue() -> Vec<Self> {
        vec![
            ...
            Self {
                ...
                zone_deltas: vec![(ZoneId::Neck, 1.6)],
                symmetric: true,
                dominant_tribe: BodyTribe::HeadNeck,
                min_trigger_delta: 1.0,
                max_total_delta: 2.0,
...
mod tests {
    ...
    fn all_display_names_non_empty() {
        for c in ALL_CONDITIONS {
            ...
            assert!(
                !c.display_arabic().is_empty(),
                "{c:?} display_arabic is empty"
            );
        }
    }
    ...
    fn fever_escalates_to_medical() {
        assert_eq!(
            MedicalCondition::SystemicFever.escalation(),
            EscalationLevel::Medical
        );
        assert_eq!(
            MedicalCondition::RespiratoryInfection.escalation(),
            EscalationLevel::Medical
        );
    }
    ...
    fn normal_variation_escalates_to_none() {
        assert_eq!(
            MedicalCondition::NormalVariation.escalation(),
    ...
    fn adrenaline_escalates_to_security_monitor() {
        assert_eq!(
            MedicalCondition::AdrenalineOnly.escalation(),
            EscalationLevel::SecurityMonitor
        );
    }
