...
use web_sys::{
    ...
    CanvasRenderingContext2d, InputEvent, Event,
};
...
#[cfg(target_arch = "wasm32")]
use tribes::{TribesState, TRIBES, PARTICLES, MAX_ORBIT_PX, particle_pos, status_css_color};
#[cfg(target_arch = "wasm32")]
use story_engine::{StoryEngineState, StewardState, STORIES};
#[cfg(target_arch = "wasm32")]
...
fn render_notebook() {
    NB.with(|nb| {
        ...
        let tok_html: String = nb.tokens().iter().map(|t| {
        ...
        set_text("result-count", &format!("{} particle(s) matched", results.len()));

        // Results table — CSS classes for status and tribe
        let rows: String = results.iter().map(|p| {
            let [r, g, b] = p.rgb;
            ...
            format!(
                ...
                html_escape(p.name), p.tribe_id, p.epoch,
                html_escape(p.status), p.tags.join(", ")
            )
        }).collect();
...
fn setup_notebook_events() {
    ...
    for &(label, query) in PRESETS {
        ...
        let cb = Closure::wrap(Box::new(move |_: Event| {
            ...
            if let Some(ta) = doc().get_element_by_id("nb-query")
    ...
    if let Some(ta) = d.get_element_by_id("nb-query") {
        ta.add_event_listener_with_callback("input", input_cb.as_ref().unchecked_ref()).ok();
    }
    input_cb.forget();
}
...
fn render_etl_log() {
    ETL.with(|etl| {
        let etl = etl.borrow();
        let html: String = etl.log.iter().rev().map(|e| {
...
fn setup_tabs() {
    switch_tab("notebook");
    attach_click("tab-hubble",    || { switch_tab("hubble");    render_hubble(); let _ = try_draw_hubble_canvas(); });
    attach_click("tab-dashboard", || { switch_tab("dashboard"); render_dashboard(); });
    attach_click("tab-notebook",  || switch_tab("notebook"));
    attach_click("tab-pipeline",  || switch_tab("pipeline"));
    attach_click("tab-probe",     || { switch_tab("probe");     render_probe(); let _ = try_draw_probe_canvas(); });
    attach_click("tab-tribes",    || { switch_tab("tribes");    fetch_tribe_stats(); render_tribes_overview(); });
    attach_click("tab-schema",    || { switch_tab("schema");    render_schema_view(); let _ = try_draw_particles_model_canvas(); });
}

#[cfg(target_arch = "wasm32")]
fn switch_tab(name: &'static str) {
    let d = doc();
    for &p in &["hubble", "dashboard", "notebook", "pipeline", "probe", "tribes", "schema"] {
    ...
    for &t in &["tab-hubble", "tab-dashboard", "tab-notebook", "tab-pipeline", "tab-probe", "tab-tribes", "tab-schema"] {
        if let Some(el) = d.get_element_by_id(t) {
            let cls = if t == format!("tab-{name}") { "tab-btn active" } else { "tab-btn" };
...
fn setup_tour() {
    ...
    if let Some(modal) = doc().get_element_by_id("tour-modal") {
        ...
        modal.add_event_listener_with_callback("click", stop_cb.as_ref().unchecked_ref()).ok();
        stop_cb.forget();
    }
}
...
fn start_loops() {
    ...
    let anim_cb = Closure::wrap(Box::new(|| {
        if doc().get_element_by_id("panel-pipeline")
    ...
    win.set_interval_with_callback_and_timeout_and_arguments_0(
        anim_cb.as_ref().unchecked_ref(), 250,
    ).ok();
    anim_cb.forget();
    ...
    win.set_interval_with_callback_and_timeout_and_arguments_0(
        tick_cb.as_ref().unchecked_ref(), 800,
    ).ok();
    tick_cb.forget();
    ...
    let tribes_cb = Closure::wrap(Box::new(|| {
        ...
        if doc().get_element_by_id("panel-tribes")
            .and_then(|e| e.get_attribute("style"))
            .map(|s| s.contains("none"))
            .unwrap_or(false)
        {
    ...
    win.set_interval_with_callback_and_timeout_and_arguments_0(
        tribes_cb.as_ref().unchecked_ref(), 33,
    ).ok();
    tribes_cb.forget();

    // Live MDM data auto-refresh: every 10 seconds when dashboard is visible
    let live_cb = Closure::wrap(Box::new(|| {
        let is_visible = doc().get_element_by_id("panel-dashboard")
            .and_then(|e| e.get_attribute("style"))
            .map(|s| !s.contains("none"))
            .unwrap_or(false);
        if is_visible { fetch_live_stats(); }
    }) as Box<dyn FnMut()>);
    win.set_interval_with_callback_and_timeout_and_arguments_0(
        live_cb.as_ref().unchecked_ref(), 10_000,
    ).ok();
    live_cb.forget();

    // Batch history auto-refresh: every 30 seconds when tribes panel is visible
    let batches_cb = Closure::wrap(Box::new(|| {
        let is_visible = doc().get_element_by_id("panel-tribes")
            .and_then(|e| e.get_attribute("style"))
            .map(|s| !s.contains("none"))
            .unwrap_or(false);
        if is_visible { fetch_batch_history(); }
    }) as Box<dyn FnMut()>);
    win.set_interval_with_callback_and_timeout_and_arguments_0(
        batches_cb.as_ref().unchecked_ref(), 30_000,
    ).ok();
    batches_cb.forget();
    ...
    let tribes_stats_cb = Closure::wrap(Box::new(|| {
        let is_visible = doc().get_element_by_id("panel-tribes")
            ...
            .map(|s| !s.contains("none"))
            .unwrap_or(false);
        if is_visible { fetch_tribe_stats(); }
    }) as Box<dyn FnMut()>);
    win.set_interval_with_callback_and_timeout_and_arguments_0(
        tribes_stats_cb.as_ref().unchecked_ref(), 60_000,
    ).ok();
    tribes_stats_cb.forget();

    // Hubble canvas animation: ~30 fps
    let hubble_cb = Closure::wrap(Box::new(|| {
        if doc().get_element_by_id("panel-hubble")
            .and_then(|e| e.get_attribute("style"))
            .map(|s| s.contains("none"))
            .unwrap_or(false) { return; }
        let _ = try_draw_hubble_canvas();
    }) as Box<dyn FnMut()>);
    win.set_interval_with_callback_and_timeout_and_arguments_0(hubble_cb.as_ref().unchecked_ref(), 33).ok();
    hubble_cb.forget();
}
...
fn render_tribes_overview() {
    ...
    let live = LIVE_TRIBE.with(|lt| {
        lt.borrow().as_ref().map(|s| (
            s.tribe_id.clone(), s.last_batch.clone(),
            s.golden, s.fuzzy, s.dead, s.total, s.batches,
        ))
    });
    ...
    let standalone_live = if let Some((ref lid, ref lbatch, lgold, lfuzz, _ldead, ltotal, lbatches)) = live {
        ...
        if let Some(card) = d.get_element_by_id(&format!("tribe-card-{ti}")) {
            let cb = Closure::wrap(Box::new(move |_: web_sys::Event| {
                TRIBES_STATE.with(|ts| {
                    ...
                    ts.selected = if ts.selected == Some(ti) { None } else { Some(ti) };
                });
                render_tribes_overview();
            }) as Box<dyn FnMut(web_sys::Event)>);
            card.add_event_listener_with_callback("click", cb.as_ref().unchecked_ref()).ok();
            cb.forget();
        }
    }
    ...
    for ti in 0..TRIBES.len() {
        if let Some(btn) = d.get_element_by_id(&format!("cmp-a-{ti}")) {
            let cb = Closure::wrap(Box::new(move |_: web_sys::Event| {
                ...
                let (ca, cb_idx) = TRIBES_STATE.with(|ts| (ts.borrow().compare_a, ts.borrow().compare_b));
...
fn render_tribe_detail(tribe_idx: usize) {
        ...
        lt.borrow().as_ref().and_then(|s| {
            if s.tribe_id == tribe_hex {
                Some(format!(
                    ...
                    g = s.golden, f = s.fuzzy, t = s.total,
                    b = s.batches, lb = s.last_batch
                ))
            } else { None }
        })
    });
    ...
    let rows: String = PARTICLES.iter()
        ...
        .map(|p| {
            ...
            format!(
                ...
                p.name, p.ptype.label(), p.status, p.epoch
            )
        }).collect();
...
fn render_compare_view() {
    ...
    fn tribe_col_html(ti: usize) -> String {
        let tribe = &TRIBES[ti];
        let [r, g, b] = tribe.rgb;
        let tribe_particles: Vec<&tribes::RParticle> = PARTICLES.iter()
            .filter(|p| p.tribe_idx == ti).collect();

        let ptype_counts = |pt: tribes::PType| tribe_particles.iter().filter(|p| p.ptype == pt).count();
    ...
    fn tribe_stats(ti: usize) -> (usize, usize, usize, usize, f32, f32) {
        ...
        let avg_epoch = if ps.is_empty() { 0.0 } else {
...
fn try_draw_tribes_canvas() -> Option<()> {
    ...
    for (ti, tribe) in TRIBES.iter().enumerate() {
        ...
        let tribe_particles: Vec<(usize, &tribes::RParticle)> = PARTICLES.iter().enumerate()
        ...
        for (pi, p) in &tribe_particles {
            ...
            let (pr, pg, pb) = if p.drift > 0.60 {
                lerp_rgb_f([pr_b, pg_b, pb_b], [122, 58, 58], ((p.drift - 0.60) / 0.40).min(1.0))
            } else if p.drift > 0.30 {
                lerp_rgb_f([pr_b, pg_b, pb_b], [255, 184, 108], ((p.drift - 0.30) / 0.30).min(1.0))
            } else { (pr_b, pg_b, pb_b) };
            let dot_r = p.ptype.dot_radius();

            // Glow halo
            let halo_alpha = if selected == Some(ti) { "0.30" } else if selected.is_some() { "0.08" } else { "0.18" };
            ...
            // Dot
            let dot_alpha = if selected.is_some() && selected != Some(ti) { "0.25" } else { "0.90" };
...
fn try_draw_canvas() -> Option<()> {
    ...
    // ── Title bar ─────────────────────────────────────────────────────────────
    ctx.set_fill_style_str(if light { "rgba(30,30,58,0.35)" } else { "rgba(205,214,244,0.22)" });
    ctx.set_font("11px monospace");
    ctx.set_text_align("center");
    ctx.fill_text("EnkiDB  ·  Five-Tier Pipeline Architecture", w / 2.0, 14.0).ok();
    ctx.set_text_align("left");

    // ── Collect pipeline state ────────────────────────────────────────────────
    let (sdb_n, odb_n, qdb_n, jnl_n) = ETL.with(|etl| {
        let e = etl.borrow();
        (e.sdb_pending(), e.odb_active(), e.qdb_count(), e.journal_entries())
    });
    ...
    let sweep_glow   = if sweep_fresh { 1.0_f64 } else if sweep_age < 10 { 0.55 } else { 0.28 };
    ...
    macro_rules! zone_header {
        ($x:expr, $w:expr, $bg:expr, $hdr:expr, $label:expr) => {{
            ...
            ctx.set_fill_style_str(if $hdr == "#50fa7b" || $hdr == "#f1fa8c" { "#1e1e2e" } else { "#cdd6f4" });
    ...
    // ══ SDB Zone ══════════════════════════════════════════════════════════════
    zone_header!(sdb_x, sdb_w,
        if light { "rgba(98,114,164,0.10)" } else { "rgba(68,71,90,0.55)" },
        "#6272a4",
        &format!("SDB  ·  {sdb_n}")
    );

    // Footer label
    ctx.set_fill_style_str(if light { "rgba(30,30,58,0.35)" } else { "rgba(139,147,198,0.45)" });
    ctx.set_font("9px monospace");
    ctx.set_text_align("center");
    ctx.fill_text("Staging Buffer", sdb_x + sdb_w / 2.0, zy + zh - 5.0).ok();
    ctx.set_text_align("left");
    ...
    if sdb_n > 20 {
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text(&format!("+{}", sdb_n - 20), sdb_x + sdb_w / 2.0, zy + zh - 18.0).ok();
        ctx.set_text_align("left");
    }
    if sdb_n == 0 && odb_n == 0 {
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text("empty", sdb_x + sdb_w / 2.0, zy + zh / 2.0 + 5.0).ok();
        ctx.set_text_align("left");
    }
    ...
    let sweep_fill = format!("rgba({},{},{},{sweep_glow:.2})",
    ...
    if sweep_fresh {
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text(&format!("tick {tick_now}"), swp_cx, swp_cy + 30.0).ok();
    }
    ctx.set_text_align("left");

    // ══ ODB Zone ═════════════════════════════════════════════════════════════
    zone_header!(odb_x, odb_w,
        if light { "rgba(80,250,123,0.07)" } else { "rgba(30,55,35,0.55)" },
        "#50fa7b",
        &format!("ODB  ·  {odb_n}")
    );

    ctx.set_fill_style_str(if light { "rgba(30,58,30,0.3)" } else { "rgba(80,250,123,0.28)" });
    ctx.set_font("9px monospace");
    ctx.set_text_align("center");
    ctx.fill_text("Operational Database", odb_x + odb_w / 2.0, zy + zh - 5.0).ok();
    ctx.set_text_align("left");
    ...
    ctx.set_stroke_style_str(if light { "rgba(80,200,100,0.22)" } else { "rgba(80,250,123,0.14)" });
    ctx.set_line_width(1.0);
    for &or in &orbit_rs {
        ctx.begin_path();
    ...
    if odb_n > 36 {
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text(&format!("+{}", odb_n - 36), odb_cx, odb_cy + max_r + 12.0).ok();
        ctx.set_text_align("left");
    }

    // ══ QDB Zone ═════════════════════════════════════════════════════════════
    zone_header!(qdb_x, qdb_w,
        if light { "rgba(255,85,85,0.08)" } else { "rgba(80,15,15,0.62)" },
        "#ff5555",
        &format!("QDB  ·  {qdb_n}")
    );

    ctx.set_fill_style_str(if light { "rgba(180,40,40,0.4)" } else { "rgba(255,85,85,0.28)" });
    ctx.set_font("9px monospace");
    ctx.set_text_align("center");
    ctx.fill_text("Quarantine Buffer", qdb_x + qdb_w / 2.0, zy + zh - 5.0).ok();
    ctx.set_text_align("left");
    ...
    if qdb_n > 30 {
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text(&format!("+{}", qdb_n - 30), qdb_x + qdb_w / 2.0, zy + zh - 18.0).ok();
        ctx.set_text_align("left");
    }
    if qdb_n == 0 {
        ctx.set_fill_style_str(if light { "#9090aa" } else { "#44475a" });
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text("clean", qdb_x + qdb_w / 2.0, zy + zh / 2.0 + 5.0).ok();
        ctx.set_text_align("left");
    }

    // ══ Journal Zone ══════════════════════════════════════════════════════════
    zone_header!(jnl_x, jnl_w,
        if light { "rgba(241,250,140,0.08)" } else { "rgba(45,40,10,0.62)" },
        "#f1fa8c",
        &format!("Journal  ·  {jnl_n}")
    );

    ctx.set_fill_style_str(if light { "rgba(100,80,0,0.35)" } else { "rgba(241,250,140,0.25)" });
    ctx.set_font("9px monospace");
    ctx.set_text_align("center");
    ctx.fill_text("Event Log", jnl_x + jnl_w / 2.0, zy + zh - 5.0).ok();
    ctx.set_text_align("left");
    ...
    for (i, (col, text)) in log_entries.iter().enumerate() {
        ...
        ctx.set_font("9px monospace");
        let show = if text.len() > max_chars { &text[..max_chars] } else { text.as_str() };
        ...
        if text.len() > max_chars && text.len() > max_chars * 2 {
            ...
            ctx.fill_text(&text[max_chars..end2], jnl_x + 6.0, ly + 11.0).ok();
        } else if text.len() > max_chars {
            ctx.set_fill_style_str(&format!("{}88", col));
            ctx.fill_text(&text[max_chars..], jnl_x + 6.0, ly + 11.0).ok();
        }
    }

    // ── Zone connector lines ──────────────────────────────────────────────────
    ctx.set_stroke_style_str(if light { "rgba(30,30,58,0.08)" } else { "rgba(205,214,244,0.06)" });
    ctx.set_line_width(1.0);
...
fn setup_story_engine_events() {
    ...
    attach_click("story-pan-left", || {
        let n = STORY_STATE.with(|ss| {
            ss.borrow().particle_idx.map(|pi| STORIES[pi].len()).unwrap_or(0)
        });
    ...
    let story_mm = Closure::wrap(Box::new(move |e: web_sys::MouseEvent| {
        if !STORY_DRAG.with(|d| d.get()) { return; }
        let start_x = STORY_DRAG_X.with(|x| x.get());
        let delta = e.client_x() as f64 - start_x;
        if delta.abs() > 4.0 { STORY_MOVED.with(|m| m.set(true)); }
    ...
    let story_mu = Closure::wrap(Box::new(move |e: web_sys::MouseEvent| {
        ...
        if STORY_MOVED.with(|m| m.get()) { return; }
        // Short click → hit-test nodes
        if let Some(canvas) = doc().get_element_by_id("story-canvas")
    ...
    if let Some(sc) = doc().get_element_by_id("story-canvas") {
        sc.add_event_listener_with_callback("mousedown", story_md.as_ref().unchecked_ref()).ok();
        sc.add_event_listener_with_callback("mousemove", story_mm.as_ref().unchecked_ref()).ok();
        sc.add_event_listener_with_callback("mouseup",   story_mu.as_ref().unchecked_ref()).ok();
    }
    story_md.forget();
    story_mm.forget();
    story_mu.forget();
    ...
    let tribes_click = Closure::wrap(Box::new(move |e: web_sys::MouseEvent| {
        let canvas = match doc().get_element_by_id("tribes-canvas")
            .and_then(|el| el.dyn_into::<HtmlCanvasElement>().ok())
        {
            Some(c) => c,
            None => return,
    ...
    if let Some(tc) = doc().get_element_by_id("tribes-canvas") {
        tc.add_event_listener_with_callback("click", tribes_click.as_ref().unchecked_ref()).ok();
    }
    tribes_click.forget();
}
...
fn render_story_header() {
    ...
    set_html("story-title", &format!(
        ...
        p.name, tribe.name, p.ptype.css_color(), p.ptype.label()
    ));
...
fn render_story_context_bar(pi: usize) {
    ...
    let eff_status = STEWARD_STATE.with(|ss| {
        ss.borrow().effective_status(pi, p.status).to_string()
    });
    let status_hex = tribes::status_hex_color(&eff_status);
    let drift_pct  = (p.drift * 100.0) as u32;
    let drift_hex  = if p.drift > 0.60 { "#ff5555" }
        else if p.drift > 0.30 { "#ffb86c" }
        else if p.drift > 0.15 { "#f1fa8c" }
        else { "#50fa7b" };

    set_html("story-context-bar", &format!(
        "<div class='scb-tribe'>\
        ...
        tribe.name, orbit.name,
        p.ptype.css_color(), p.ptype.label()
    ));
...
fn render_story_chips() {
    ...
    let pi = match pi { Some(p) => p, None => return };
    let events = STORIES[pi];

    let chips: String = events.iter().enumerate().map(|(i, ev)| {
        let sel = if selected == Some(i) { " se-chip-sel" } else { "" };
        ...
        format!(
            ...
            ev.kind.icon(), ev.kind.label(), ev.tick, ev.state
        )
    }).collect();
...
fn try_draw_story_canvas() -> Option<()> {
    ...
    // Dot grid
    ctx.set_fill_style_str(if light { "rgba(80,80,140,0.12)" } else { "rgba(100,100,180,0.12)" });
    let gs = 32.0_f64;
    let mut gx = 0.0_f64;
    while gx < cw {
        let mut gy = 0.0_f64;
    ...
    // ── Timeline track ────────────────────────────────────────────────────
    let track_y = 117.0;
    ctx.set_stroke_style_str(if light { "rgba(80,80,160,0.18)" } else { "rgba(100,100,200,0.18)" });
    ctx.set_line_width(2.0);
    ctx.begin_path();
    ...
    for i in 0..n.saturating_sub(1) {
        ...
        let fx = x0 + w0;     let fy = y0 + h0 / 2.0;   // output port
        let tx = x1;           let ty = y1 + h1 / 2.0;   // input port
        let cp1x = fx + 28.0; let cp1y = fy;
        let cp2x = tx - 28.0; let cp2y = ty;
    ...
    for (i, ev) in events.iter().enumerate() {
        ...
        ctx.set_text_align("left");
        ctx.fill_text(&format!("{} {}", ev.kind.icon(), ev.kind.label()), nx + 8.0, ny + 15.0).ok();

        // Tick in body
        ctx.set_fill_style_str(if light { "#6b6b88" } else { "#9090aa" });
        ctx.set_font("10px monospace");
        ctx.fill_text(&format!("T{}", ev.tick), nx + 8.0, ny + 34.0).ok();
    ...
    if pan_x < -4.0 {
        ctx.set_fill_style_str(if light { "rgba(240,240,251,0.82)" } else { "rgba(5,5,9,0.82)" });
...
fn setup_steward_events() {
    ...
    if let Some(tc) = doc().get_element_by_id("tribes-canvas") {
        tc.add_event_listener_with_callback("contextmenu", ctx_cb.as_ref().unchecked_ref()).ok();
    }
    ctx_cb.forget();
    ...
    if let Some(d) = web_sys::window().and_then(|w| w.document()) {
        d.add_event_listener_with_callback("click", hide_cb.as_ref().unchecked_ref()).ok();
    }
    hide_cb.forget();
    ...
    if let Some(m) = doc().get_element_by_id("ctxmenu") {
        m.add_event_listener_with_callback("click", stop_cb.as_ref().unchecked_ref()).ok();
    }
    stop_cb.forget();
    ...
    if let Some(el) = doc().get_element_by_id("ctx-open-story") {
        el.add_event_listener_with_callback("mousedown", open_story_cb.as_ref().unchecked_ref()).ok();
    }
    ...
    let bd_cb = Closure::wrap(Box::new(move |e: web_sys::MouseEvent| {
        if let Some(t) = e.target().and_then(|t| t.dyn_into::<web_sys::Element>().ok()) {
            if t.id() == "steward-backdrop" { hide_steward_sidebar(); }
        }
    }) as Box<dyn FnMut(web_sys::MouseEvent)>);
    if let Some(bd) = doc().get_element_by_id("steward-backdrop") {
        bd.add_event_listener_with_callback("click", bd_cb.as_ref().unchecked_ref()).ok();
    }
    bd_cb.forget();
    ...
    attach_click("steward-apply", || {
        ...
        let note = doc().get_element_by_id("steward-note")
        ...
        STEWARD_STATE.with(|ss| {
            ...
            ss.overrides[pi] = Some(story_engine::ParticleOverride { status, ptype, note });
        });
...
fn show_ctxmenu(pi: usize, cx: i32, cy: i32) {
    ...
    set_html("ctx-particle-name", &format!(
        "{} <span style='color:{sc};font-size:11px'>{}</span>", p.name, eff_status
    ));
    // Show/hide Diagnose option only for Dead/Fuzzy
    let diag_display = if eff_status == "Dead" || eff_status == "Fuzzy" { "" } else { "" };
    if let Some(el) = doc().get_element_by_id("ctx-steward") {
        el.set_attribute("style", &format!("display:{diag_display}")).ok();
    }
    if let Some(el) = doc().get_element_by_id("ctxmenu") {
        el.set_attribute("style", &format!("left:{cx}px;top:{cy}px")).ok();
        el.set_attribute("class", "open").ok();
    }
}
...
fn render_steward_header_section(pi: usize) {
    ...
    if let Some(el) = doc().get_element_by_id("steward-tribe-bar") {
        el.set_attribute("style",
            &format!("height:4px;background:rgb({tr},{tg},{tb})")).ok();
    }

    // Mini orbital canvas + text side-by-side
    set_html("steward-header-info", &format!(
...
fn try_draw_steward_mini_canvas(pi: usize) -> Option<()> {
    ...
    for (i, orbit) in tribe.orbits.iter().enumerate() {
        ...
        let (alpha, lw) = if is_active { (0.65_f64, 2.0_f64) } else { (0.18_f64, 1.0_f64) };
...
fn render_steward_journal_section(pi: usize) {
    ...
    let rows: String = events.iter().enumerate().map(|(i, ev)| {
        let is_causal = causal.contains(&i);
        let col = ev.kind.hex_color();
        let (row_cls, badge) = if is_causal {
            let badge_cls = if eff_status == "Dead" { "sd-ev-badge-dead" } else { "sd-ev-badge-fuzzy" };
            let label = if eff_status == "Dead" { "⚠ Root Cause" } else { "~ Contributor" };
            ("sd-ev-row sd-ev-causal", format!("<span class='{badge_cls}'>{label}</span>"))
        } else {
...
fn render_steward_status_pills(_pi: usize) {
    ...
    let html: String = STATUSES.iter().map(|s| {
        let sel = if edit_status == *s { " se-pill-sel" } else { "" };
        let sc  = tribes::status_css_color(s);
        format!("<button class='se-pill{sel}' id='se-st-{s}' style='--pill-c:{sc}'>{s}</button>")
    }).collect();
...
fn render_dashboard() {
    ...
    let at_risk = PARTICLES.iter()
        .filter(|p| matches!(p.status, "Warning" | "Watch" | "Critical"))
        .count();
    let dead_fuzzy = PARTICLES.iter()
        .filter(|p| p.status == "Dead" || p.status == "Fuzzy")
        .count();
    let overrides = STEWARD_STATE.with(|ss| {
        ss.borrow().overrides.iter().filter(|o| o.is_some()).count()
    });

    set_html("dash-metrics", &format!(
...
fn render_alert_queue() {
    let alert_particles: Vec<(usize, String)> = PARTICLES.iter().enumerate()
        .filter_map(|(pi, p)| {
            let eff = STEWARD_STATE.with(|ss| {
                ss.borrow().effective_status(pi, p.status).to_string()
            });
            if eff == "Dead" || eff == "Fuzzy" { Some((pi, eff)) } else { None }
        })
        .collect();

    if alert_particles.is_empty() {
        set_html("dash-alert-queue",
    ...
    let rows: String = alert_particles.iter().map(|(pi, eff)| {
        let p = &PARTICLES[*pi];
        let tribe = &TRIBES[p.tribe_idx];
        let [tr, tg, tb] = tribe.rgb;
...
fn try_draw_drift_canvas() -> Option<()> {
    ...
    let mut indexed: Vec<(usize, &tribes::RParticle)> =
        PARTICLES.iter().enumerate().collect();
    indexed.sort_by(|a, b| {
        b.1.drift.partial_cmp(&a.1.drift).unwrap_or(std::cmp::Ordering::Equal)
    });
    ...
    for &(threshold, col) in &[
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("center");
        ctx.fill_text(&format!("{:.0}%", threshold * 100.0), x, top_pad - 2.0).ok();
    }
    ctx.set_text_align("left");
    ...
    for (row, (pi, p)) in indexed.iter().enumerate() {
        ...
        ctx.set_fill_style_str(if light { "#1e1e3a" } else { "#cdd6f4" });
        ctx.set_font("10px monospace");
        ctx.fill_text(&p.name.replace('_', " "), 0.0, y + bar_h - 2.0).ok();

        // Bar background track
        ctx.set_fill_style_str(
            if light { "rgba(80,80,140,0.08)" } else { "rgba(100,100,200,0.08)" }
        );
        ...
        ctx.set_fill_style_str(if light { "#6b6b88" } else { "#9090aa" });
        ctx.set_font("9px monospace");
        ctx.fill_text(&format!("{pct}%"), chart_x + bar_len + 4.0, y + bar_h - 2.0).ok();
    }
...
fn setup_hubble_events() {
    let hub_click = Closure::wrap(Box::new(move |e: web_sys::MouseEvent| {
        let canvas = match doc().get_element_by_id("hubble-canvas")
            .and_then(|el| el.dyn_into::<HtmlCanvasElement>().ok())
        ...
        match zoom {
            ...
            hubble::ZoomLevel::Tribe(ti) => {
                let max_r = 190.0_f64;
                let pcx = w / 2.0; let pcy = h / 2.0;
                let angles = TRIBES_STATE.with(|ts| ts.borrow().angles.clone());
                for (pi, p) in PARTICLES.iter().enumerate().filter(|(_, p)| p.tribe_idx == ti) {
                    ...
                    if (cx-px)*(cx-px)+(cy-py)*(cy-py) <= hit_r*hit_r {
                        ...
                        STORY_STATE.with(|ss| ss.borrow_mut().open(pi));
                        render_story_header(); render_story_chips(); render_story_detail();
                        switch_tab("tribes");
                        if let Some(el) = doc().get_element_by_id("story-engine-panel") {
                            el.set_attribute("style", "display:block").ok();
                            el.scroll_into_view();
    ...
    if let Some(hc) = doc().get_element_by_id("hubble-canvas") {
        hc.add_event_listener_with_callback("click", hub_click.as_ref().unchecked_ref()).ok();
    }
    hub_click.forget();
}
...
fn render_hubble() {
    ...
    match &zoom {
        hubble::ZoomLevel::Universe => {
            let total = PARTICLES.len();
            let active    = PARTICLES.iter().filter(|p| p.status == "Active").count();
            let at_risk   = PARTICLES.iter().filter(|p| matches!(p.status, "Warning"|"Watch"|"Critical")).count();
            let dead_fuzzy = PARTICLES.iter().filter(|p| p.status == "Dead" || p.status == "Fuzzy").count();
            set_html("hubble-stats", &format!(
        ...
        hubble::ZoomLevel::Tribe(ti) => {
            ...
            let ps: Vec<&tribes::RParticle> = PARTICLES.iter().filter(|p| p.tribe_idx == *ti).collect();
            let total = ps.len();
            let avg_drift_pct = (ps.iter().map(|p| p.drift as f64).sum::<f64>() / total as f64 * 100.0) as u32;
            ...
            set_html("hubble-stats", &format!(
                ...
                tribe.name, tribe.orbits.len()
            ));
            let ti_val = *ti;
            set_html("hubble-breadcrumb", &format!(
...
fn try_draw_hubble_canvas() -> Option<()> {
    let canvas = doc().get_element_by_id("hubble-canvas")?
        .dyn_into::<HtmlCanvasElement>().ok()?;
    let ctx = canvas.get_context("2d").ok()??
        .dyn_into::<CanvasRenderingContext2d>().ok()?;
    let w = canvas.width()  as f64;
...
fn draw_hubble_universe(ctx: &CanvasRenderingContext2d, w: f64, h: f64, light: bool) {
    ...
    // Universe title
    ctx.set_fill_style_str(if light { "rgba(30,30,58,0.35)" } else { "rgba(205,214,244,0.25)" });
    ctx.set_font("11px monospace");
    ctx.set_text_align("center");
    ...
    for (ti, tribe) in TRIBES.iter().enumerate() {
        ...
        let at_risk    = ps.iter().filter(|p| matches!(p.status, "Warning"|"Watch"|"Critical")).count() as f64;
        let dead_fuzzy = ps.iter().filter(|p| p.status == "Dead" || p.status == "Fuzzy").count() as f64;
        let avg_drift  = ps.iter().map(|p| p.drift as f64).sum::<f64>() / total;

        // Color degradation for glow: healthy=tribe color, degraded=dark red
        let (gr, gg, gb) = lerp_rgb_f([tr, tg, tb], [122, 58, 58], (avg_drift * 1.4).min(1.0) as f32);
        ...
        for (pi, p) in PARTICLES.iter().enumerate().filter(|(_, p)| p.tribe_idx == ti && p.status == "Active") {
        ...
        for (i, p) in ps.iter().filter(|p| p.status == "Dead" || p.status == "Fuzzy").enumerate() {
            let orbit_r = tribe.orbits[p.orbit_idx].radius as f64 * 88.0;
        ...
        let hcol = if active / total > 0.7 { "#50fa7b" } else if active / total > 0.4 { "#ffb86c" } else { "#ff5555" };
        ctx.set_fill_style_str(hcol);
        ctx.set_font("10px monospace");
        ctx.fill_text(&format!("{health_pct}% healthy"), cx, cy + 88.0 + 31.0).ok();
        ctx.set_text_align("left");
    }
}
...
fn draw_hubble_tribe_zoom(ctx: &CanvasRenderingContext2d, w: f64, h: f64, ti: usize, light: bool) {
    ...
    for (i, orbit) in tribe.orbits.iter().enumerate() {
        let r = orbit.radius as f64 * max_r;
        let is_active_ring = PARTICLES.iter().filter(|p| p.tribe_idx == ti && p.orbit_idx == i).any(|p| p.status == "Active");
    ...
    for (pi, p) in PARTICLES.iter().enumerate().filter(|(_, p)| p.tribe_idx == ti) {
        ...
        if p.drift > 0.08 {
            ...
            ctx.set_line_width(1.0);
            ctx.begin_path(); ctx.move_to(gx, gy); ctx.line_to(px, py); ctx.stroke();
        ...
        ctx.begin_path(); ctx.arc(px, py, dot_r * 2.6, 0.0, TAU).ok(); ctx.fill();

        // Dot
        ctx.set_fill_style_str(&format!("rgb({dr},{dg},{db})"));
        ctx.begin_path(); ctx.arc(px, py, dot_r, 0.0, TAU).ok(); ctx.fill();

        // Name label
        ctx.set_fill_style_str(if light { "rgba(30,30,58,0.75)" } else { "rgba(205,214,244,0.75)" });
        ctx.set_font("9px monospace");
        ctx.set_text_align("left");
        ctx.fill_text(p.name, px + dot_r + 3.0, py + 3.0).ok();
        ...
        if eff == "Dead" || eff == "Fuzzy" {
            ...
            ctx.set_font("bold 9px monospace");
            ctx.fill_text(&format!("\u{26a0} {eff}"), px + dot_r + 3.0, py + 14.0).ok();
        }
    }
...
fn render_probe() {
    ...
    let steps_html = SC_GA.with(|s| {
        s.borrow().ga.as_ref().map(|ga| {
            ga.steps.iter().enumerate().map(|(i, step)| {
                let col = if i == 0 { "#8be9fd" }
                    else if step.rotor_deg.abs() > 0.1 { "#ff79c6" }
                    else { "#6c7086" };
                let rotor_str = if step.rotor_deg.abs() < 0.1 {
                    String::from("translate")
                } else {
                    format!("R&#8336;: e^({:+.0}&deg;&middot;e&#x2081;&#x2082;)", step.rotor_deg)
                };
                format!(
                    "<div class='prb-step'>\
    ...
    set_html("probe-ga-path", &format!(
        "<div class='prb-section-title'>Cl(2,0) Rotor Navigation Path</div>{steps_html}"
    ));

    // ── Verdict ──────────────────────────────────────────────────────────────
    let colony_word = if beta0 == 1 { "1 unified colony".to_string() } else { format!("{beta0} isolated colonies") };
    let circuit_word = if beta1 == 0 { "no circuit loops".to_string() }
        else if beta1 == 1 { "1 rat circuit loop".to_string() }
        else { format!("{beta1} independent rat circuit loops") };
    let verdict = format!(
...
fn try_draw_probe_canvas() -> Option<()> {
    ...
    let canvas = doc().get_element_by_id("probe-canvas")?
        .dyn_into::<HtmlCanvasElement>().ok()?;
    let ctx = canvas.get_context("2d").ok()??
        .dyn_into::<CanvasRenderingContext2d>().ok()?;
    ...
    // Subtle grid
    ctx.set_stroke_style_str(if light { "rgba(80,200,80,0.06)" } else { "rgba(80,200,80,0.04)" });
    ctx.set_line_width(0.5);
    let step = 60.0_f64;
    let mut gx = 0.0_f64;
    while gx < w { ctx.begin_path(); ctx.move_to(gx,0.0); ctx.line_to(gx,h); ctx.stroke(); gx += step; }
    let mut gy = 0.0_f64;
    while gy < h { ctx.begin_path(); ctx.move_to(0.0,gy); ctx.line_to(w,gy); ctx.stroke(); gy += step; }

    // Title
    ctx.set_fill_style_str("rgba(100,220,100,0.25)");
    ctx.set_font("11px monospace");
    ctx.set_text_align("center");
    ctx.fill_text("Graveyard Necropolis  \u{00b7}  SC+GA PROBE (Demonstration)", w / 2.0, 16.0).ok();
    ctx.set_text_align("left");
    ...
    for (i, &r) in sc_ga::SECTOR_RADII.iter().enumerate() {
        ...
        ctx.set_font("9px monospace");
        ctx.set_text_align("left");
        ctx.fill_text(sector_labels[i], cx + rpx + 4.0, cy + 3.0).ok();
    }
    ...
    let (ran, edges, triangles, ga_pts) = SC_GA.with(|s| {
        ...
        let triangles = s.sc.as_ref().map(|sc| sc.triangles.clone()).unwrap_or_default();
        let ga_pts   = s.ga.as_ref().map(|ga| ga.path_points.clone()).unwrap_or_default();
        (s.ran, edges, triangles, ga_pts)
    });

    // Rat positions in pixel space
    let rat_px: Vec<(f64, f64)> = sc_ga::RATS.iter().map(|r| {
    ...
    if ran && ga_pts.len() > 1 {
        ...
        for (i, &(nx, ny)) in ga_pts.iter().enumerate() {
            ...
            ctx.set_fill_style_str(if is_rotor { "rgba(255,121,198,0.9)" } else { "rgba(139,233,253,0.7)" });
            ctx.begin_path();
            ctx.arc(px, py, if is_rotor { 4.5 } else { 3.0 }, 0.0, TAU).ok();
            ctx.fill();
        }
    }
    ...
    if ran {
        let (e_n, t_n, b0, b1) = SC_GA.with(|s| {
            let s = s.borrow();
            s.sc.as_ref().map(|sc| (sc.edges.len(), sc.triangles.len(), sc.beta0, sc.beta1))
                .unwrap_or_default()
        });
        ...
        for (i, line) in [
            ...
            let col = match i { 2 => "#ffb86c", 3 => "#50fa7b", 4 => "#ff79c6", _ => "#8be9fd" };
            ctx.set_fill_style_str(col);
...
fn setup_schema_events() {
    ...
    if let Some(el) = doc().get_element_by_id("schema-format") {
        el.add_event_listener_with_callback("change", fmt_cb.as_ref().unchecked_ref()).ok();
    }
    fmt_cb.forget();
    ...
    let trust_cb = Closure::wrap(Box::new(move |e: Event| {
        ...
        {
            let idx = sel.selected_index() as usize;
            SCHEMA_STATE.with(|s| s.borrow_mut().source_trust = idx.min(SOURCE_TRUST_LEVELS.len() - 1));
        }
    }) as Box<dyn FnMut(Event)>);
    if let Some(el) = doc().get_element_by_id("schema-trust") {
        el.add_event_listener_with_callback("change", trust_cb.as_ref().unchecked_ref()).ok();
    }
    trust_cb.forget();

    // Gem threshold input
    let gem_cb = Closure::wrap(Box::new(move |e: InputEvent| {
        if let Some(inp) = e.target()
    ...
    if let Some(el) = doc().get_element_by_id("schema-gem") {
        el.add_event_listener_with_callback("input", gem_cb.as_ref().unchecked_ref()).ok();
    }
    gem_cb.forget();
    ...
    if let Some(el) = doc().get_element_by_id("schema-tribe") {
        el.add_event_listener_with_callback("input", tribe_cb.as_ref().unchecked_ref()).ok();
    }
    tribe_cb.forget();
}
...
fn render_schema_view() {
    let html = SCHEMA_STATE.with(|s| {
        schema_config::render_schema_table_html(&s.borrow())
    });
...
fn setup_live_data_events() {
    ...
    attach_click("btn-refresh-batches", || { fetch_batch_history(); fetch_tribe_stats(); });
}
...
fn fetch_live_stats() {
    wasm_bindgen_futures::spawn_local(async {
        ...
        if !resp.ok() {
            set_html("dash-live-status",
                &format!("<span style='color:var(--red)'>HTTP {}</span>", resp.status()));
            return;
        }
...
fn parse_and_render_live_stats(json: &str) {
    ...
    if status == "no_data" {
        ...
        set_html("dash-live-stats", &format!(
            "<p style='color:var(--text-muted);font-size:13px'>{msg}</p>"
        ));
        set_html("dash-live-status", "<span style='color:var(--yellow)'>&#9888; no data yet</span>");
        return;
    }
    ...
    if let Some(el) = doc().get_element_by_id("nb-live-banner") {
        el.set_attribute("style", "display:block;margin-bottom:12px;padding:8px 14px;\
            background:var(--surface);border:1px solid var(--border);\
            border-left:3px solid var(--green);border-radius:6px;\
            font-size:12px;color:var(--text-muted)").ok();
    }
}
...
fn fetch_batch_history() {
    wasm_bindgen_futures::spawn_local(async {
        ...
        let text_promise = match resp.text() { Ok(p) => p, Err(_) => return };
        let text_val = match JsFuture::from(text_promise).await { Ok(v) => v, Err(_) => return };
        let text = text_val.as_string().unwrap_or_default();
...
fn parse_and_render_batch_history(json: &str) {
    ...
    let batches_end = json[batches_start..].rfind('}')
    ...
    for (i, ch) in batches_json.char_indices() {
        match ch {
            ...
            '}' => {
                depth -= 1;
                if depth == 0 { batch_objs.push(&batches_json[obj_start..=i]); }
            }
            _ => {}
        }
    }
    ...
    set_html("tribes-live-status", &format!(
...
fn parse_and_render_tribe_stats(json: &str) {
    ...
    let status = str_val(json, "status");
    if status == "no_data" || status.is_empty() { return; }
    ...
    let tribe_obj_end = json[tribe_obj_start..].find('}')
    ...
    if tribe_id.is_empty() { return; }

    LIVE_TRIBE.with(|lt| {
        *lt.borrow_mut() = Some(LiveTribeStat {
            tribe_id, last_batch, golden, fuzzy, dead, total, batches,
        });
    });
