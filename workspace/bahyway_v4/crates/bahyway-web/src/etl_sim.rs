...
use enkidb_kaki::{KakiRole, IdentityKaki};
...
#[derive(Clone)]
pub enum LogKind { Info, Sweep, Malware }
...
impl EtlSimState {
    ...
    pub fn stage_valid(&mut self) {
        let ik = IdentityKaki::try_from_kaki(
            self.lp.minter.identity(KakiRole::Zikru)
        ).unwrap();
        ...
        self.push_log(format!("Staged valid particle → SDB pending={n}"), LogKind::Info);
    }
    ...
    pub fn stage_malware(&mut self) {
        ...
        let n = self.lp.sdb.stats().pending_count();
        self.push_log(format!("Staged MALWARE particle → SDB pending={n}"), LogKind::Malware);
    }
    ...
    fn rebuild_vis(&mut self) {
        ...
        for rgb in odb_colors {
            ...
            self.vis.push(VisParticle { x, y, z, rgb, vx, vy, vz });
        }
    ...
    pub fn animate_vis(&mut self) {
        for p in &mut self.vis {
            ...
            if p.x.abs() > 1.0 { p.vx *= -1.0; }
            if p.y.abs() > 1.0 { p.vy *= -1.0; }
            if p.z.abs() > 1.0 { p.vz *= -1.0; }
        }
    }

    pub fn sdb_pending(&self)      -> usize { self.lp.sdb.stats().pending_count() }
    pub fn odb_active(&self)       -> usize { self.lp.odb.stats().active_count }
    pub fn qdb_count(&self)        -> usize { self.lp.qdb.len() }
    pub fn journal_entries(&self)  -> usize { self.lp.journal.entry_count() }

    fn push_log(&mut self, text: impl Into<String>, kind: LogKind) {
        self.log.push(LogEntry { text: text.into(), kind });
        if self.log.len() > 40 { self.log.remove(0); }
    }
}
