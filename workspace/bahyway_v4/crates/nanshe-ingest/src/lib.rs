//! nanshe-ingest — Sovereign ingest entry for NANSHE (Diyala River environmental intelligence)
//! Source of truth: BC-ENV-001
//!
...
pub const TRIBE_ID: &str = "nanshe_diyala";
...
/// Ingest a single file (raw bytes) for the NANSHE tribe.
///
...
pub fn ingest(bytes: &[u8], filename: String) -> IngestReport {
    ...
    match outcome {
        naramsin_bridge::NaramsinOutcome::Clean(rows) => IngestReport {
            ...
            nrm_code: "NRM_CLEAN",
            journal_line: format!(
                "NARAMSIN|{archive_id}|{static_filename}|dest=NANSHE|rows={}|{elapsed_ms}ms",
                rows.len()
            ),
            blocked: false,
        },
...
/// Stub MASHSHARU handoff — to be implemented once NANSHE's KAKI schema exists.
pub fn hand_to_mashsharu(report: &IngestReport) {
    if report.blocked {
        tracing::warn!(
            "nanshe-ingest: NARAMSIN blocked ({}), skipping MASHSHARU",
            report.nrm_code
        );
        return;
    }
    tracing::info!(
        "nanshe-ingest: {} rows -> MASHSHARU (stub) | archive_id={}",
        report.row_count,
        report.archive_id
    );
    // TODO: hand row stream to NANSHE-specific KAKI schema builder once it exists.
}
...
mod tests {
    ...
    fn ingest_empty_input_is_rejected_and_blocked() {
        // naramsin-archive's detect_format only errors on truly empty input;
        // any non-empty, non-matching header is treated as a single plain
        // file (ArchiveFormat::None) and passes through as CLEAN.
