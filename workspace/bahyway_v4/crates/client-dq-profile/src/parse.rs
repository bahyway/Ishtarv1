...
pub fn parse_profile(src: &str) -> Result<ClientDqProfile, String> {
    ...
    for raw_line in src.lines() {
        ...
        if line.starts_with("[attr ") && line.ends_with(']') {
            ...
            if name.is_empty() {
                return Err("empty attr name in [attr ...]".into());
            }
            cur_name = Some(name);
            cur_nullable = true;
            cur_threshold = 0.0;
        ...
        if cur_name.is_some() {
            match key {
                ...
                "dim" => cur_dim = DimTag::parse_dim_tag(val),
            ...
            match key {
                ...
                "format_layer" => format_layer = FormatLayerCode::parse_format_layer_code(val),
                "source_trust" => source_trust = SourceTrustLevel::parse_source_trust_level(val),
...
fn parse_u8(s: &str) -> Result<u8, String> {
    s.trim()
        .parse::<u8>()
        .map_err(|e| format!("expected u8: {e}"))
}
...
"#;
    ...
    fn dim_tags_parsed() {
        ...
        let dod = p
            .attrs
            .iter()
            .find(|a| a.attr_name == "date_of_death")
            .unwrap();
