...
pub fn compute_dims(eav: &[EavTriple], profile: &ClientDqProfile) -> FuzzyDimensions {
    let present: std::collections::HashSet<u32> = eav.iter().map(|t| t.attr_hash).collect();

    // D1 — name completeness: fraction of non-nullable Name attrs that are present
    let d1 = {
        let name_mandatory: Vec<u32> = profile
            .attrs
            .iter()
    ...
    let d3 = {
        let id_mandatory: Vec<u32> = profile
            .attrs
            .iter()
            .filter(|a| matches!(a.dim_tag, DimTag::Identity) && !a.nullable)
            .map(|a| a.attr_hash)
            .collect();
        if id_mandatory.is_empty() || id_mandatory.iter().all(|h| present.contains(h)) {
            1.0f32
        } else {
...
fn general_fill_rate(present: &std::collections::HashSet<u32>, profile: &ClientDqProfile) -> f32 {
    let mandatory: Vec<u32> = profile
        .attrs
        .iter()
        .filter(|a| !a.nullable)
        .map(|a| a.attr_hash)
    ...
    mandatory.iter().filter(|h| present.contains(h)).count() as f32 / mandatory.len() as f32
}
...
mod tests {
    ...
    fn full_record_scores_high_d1_and_d3() {
        ...
        let eavs = eav(&[("name", b"Ali Karim"), ("national_id", b"190273649281")]);
        let dims = compute_dims(&eavs, &p);
        assert!(
            dims.d1_name_completeness >= 1.0,
            "name fully filled → d1 = 1.0"
        );
    ...
    fn missing_optional_date_gives_partial_d4() {
        ...
        let dims = compute_dims(&eavs, &p);
        assert!(
            dims.d4_date_validity < 1.0,
            "absent optional date → d4 < 1.0"
        );
        assert!(
            dims.d4_date_validity > 0.0,
            "absent optional date → d4 > 0.0 (nullable)"
        );
    }
    ...
    fn arabic_text_produces_high_d8() {
        let p = profile_cemetery();
        let eavs = eav(&[("name", "علي كريم".as_bytes())]);
        let dims = compute_dims(&eavs, &p);
        assert!(
            dims.d8_arabic_density > 0.5,
            "Arabic name → d8 > 0.5, got {}",
            dims.d8_arabic_density
        );
    }
