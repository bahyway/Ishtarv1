...
use enkimdb::ShakkanakkuRunRecordSpec;
...
/// the SECOND identity signal (see `ShakkanakkuRunRecordSpec`'s own doc
...
fn detect_os_identity() -> OsIdentity {
    ...
    OsIdentity { username, groups_csv, bahyway_role }
}
...
impl RunTally {
    ...
    fn to_spec(&self, outcome: &str, blocked_reason: Option<String>) -> ShakkanakkuRunRecordSpec {
...
/// EnkiMDB may not be reachable from every environment Shakkanakku runs
...
fn emit_run_record(cfg: &Config, spec: &ShakkanakkuRunRecordSpec) -> String {
    ...
    match TcpStream::connect(&addr) {
        Ok(mut s) => {
            ...
            let send_ok = s.write_all(&(payload.len() as u32).to_le_bytes()).and_then(|_| s.write_all(payload.as_bytes()));
            ...
            if resp.starts_with("OK:") {
                format!("  ✓ run-confirmation registry: recorded (outcome={}) at {addr}", spec.outcome)
            } else {
...
fn authenticate_operator(cfg: &Config) -> Result<Option<kupru_vault::AuthedIdentity>, String> {
    ...
    let passphrase = std::env::var("KUPRU_VAULT_PASSPHRASE")
        .map_err(|_| "vault_check_enabled=true but KUPRU_VAULT_PASSPHRASE is not set in the environment".to_string())?;
    let vault_bytes = std::fs::read(vault_path).map_err(|e| format!("cannot read vault file {vault_path}: {e}"))?;
...
fn run(cfg: Config, params: Vec<(String, String)>, tx: Sender<RunnerEvent>, ctl_rx: Receiver<Ctl>) {
    ...
    if let Some(id) = &operator {
        send(RunnerEvent::Log(format!("  ✓ Vault authenticated: realm={} privilege_level={}", id.realm, id.privilege_level)));
    }
    let mut tally = RunTally::new(n as u32, operator);
    match (&tally.os.username, &tally.os.bahyway_role) {
        (Some(user), Some(role)) => send(RunnerEvent::Log(format!("  · OS identity: user={user} bahyway_role={role}"))),
        (Some(user), None) => send(RunnerEvent::Log(format!("  · OS identity: user={user} (not a member of any bahyway-* group)"))),
        (None, _) => send(RunnerEvent::Log("  · OS identity: could not determine (id -un failed)".to_string())),
    }
    ...
    let triage_doc = cfg
        ...
        .unwrap_or_else(|| repo_root.join("docs/PLAYBOOK_EXECUTION_TRIAGE.md"));
    ...
    for i in 0..n {
        ...
        if matches!(ctl_rx.try_recv(), Ok(Ctl::Abort)) {
            let spec = tally.to_spec("Aborted", Some("Architect aborted between playbooks".to_string()));
            send(RunnerEvent::Log(emit_run_record(&cfg, &spec)));
            send(RunnerEvent::Aborted);
            return;
        }
        ...
        loop {
            ...
            match outcome {
                ...
                Outcome::Aborted => {
                    ...
                    let spec = tally.to_spec("Aborted", Some(format!("Architect aborted mid-run during '{pb}'")));
                    send(RunnerEvent::Log(emit_run_record(&cfg, &spec)));
                    send(RunnerEvent::Aborted);
                    return;
                }
                Outcome::Failed(ev) => {
                    ...
                    match ctl_rx.recv() {
                        Ok(Ctl::Abort) | Err(_) => {
                            tally.failed += 1;
                            tally.skipped += (n - i - 1) as u32;
                            let spec = tally.to_spec("Aborted", Some(format!("MAJOR failure at '{}', Architect chose Abort", ev.task)));
                            send(RunnerEvent::Log(emit_run_record(&cfg, &spec)));
                            send(RunnerEvent::Aborted);
                            return;
                        }
...
fn run_one(cfg: &Config, i: usize, pb: &str, params: &[(String, String)], ctl_rx: &Receiver<Ctl>) -> Outcome {
            ...
            if let Some(hosts) = task.get("hosts").and_then(|h| h.as_object()) {
                for (_host, res) in hosts {
                    let failed = res.get("failed").and_then(|f| f.as_bool()).unwrap_or(false)
                        || res.get("unreachable").and_then(|f| f.as_bool()).unwrap_or(false);
                    if failed {
                        let msg = res
                            .get("msg")
  ...
  {"task":{"name":"start enlil-hotindex.service"},
   "hosts":{"h1":{"failed":true,"msg":"port 7101 already bound"}}}
    ...
    fn error_type_inference() {
        assert_eq!(infer_error_type("systemd unit failed"), "ServiceStartFailure");
        assert_eq!(infer_error_type("Permission denied (publickey)"), "PermissionDenied");
    ...
    fn detect_os_identity_reads_a_real_username_on_this_host() {
        ...
        let user = os.username.expect("id -un must succeed on any real Unix host");
    ...
    fn abort_during_real_run_is_responsive_not_blocking() {
        ...
        assert!(start.elapsed() < Duration::from_secs(5), "abort must be responsive, not wait for the 30s sleep");
