//! Shakkanakku — the governor's engine, exposed as a library so both the
//! native egui face (`app`, feature "gui") and the browser-facing HTTP
//! server (`bin/shakkanakku_web.rs`, feature "web") drive the exact same
//! config/runner/remedy/report/chronicle code. Neither face owns the law;
//! they only render it.
...
// docs/BAHYWAY_PHASE2_GLOSSARY.md). Historically driven only from the
