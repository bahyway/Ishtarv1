//! Shakkanakku — the governor. One command executes the corpus;
//! warnings proceed, majors await the Architect; the report is sealed.
//!
//! This binary is the native-window face (feature "gui") or the headless
//! terminal face (no features). Both drive the `shakkanakku` library crate
//! (see src/lib.rs) — the same engine the browser-facing `shakkanakku-web`
//! binary (src/bin/shakkanakku_web.rs) drives.

#[cfg(feature = "gui")]
use shakkanakku::app;
#[cfg(not(feature = "gui"))]
use shakkanakku::{config, report, runner};
...
fn main() -> eframe::Result<()> {
    ...
    eframe::run_native(
        "Shakkanakku 𒑐 — BahyWay Governor",
        options,
        Box::new(|cc| Ok(Box::new(app::ShakkanakkuApp::new(cc)))),
    )
}
...
fn main() -> anyhow::Result<()> {
    ...
    let cfg_path = std::env::args().nth(1).unwrap_or_else(|| "shakkanakku.toml".into());
