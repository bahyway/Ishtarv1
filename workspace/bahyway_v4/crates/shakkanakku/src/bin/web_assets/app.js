// Shakkanakku browser face. No framework: fetches /api/state, renders the
...
function renderIdentity() {
  ...
  if (!currentIdentity) {
    badge.textContent = '(not signed in — Box Resources is free to view; Shakkanakku & Hala need a passport)';
...
// wrote server-side (AkkadianSeal, docs/SHEDU/shakkanakku_reports/). This
...
async function fetchExport(format) {
  ...
  const filename = res.headers.get('X-Suggested-Filename') || `shakkanakku_report.${format}`;
...
function renderRemedy(state) {
  ...
  if (ev.remedy) {
    ...
    html += `<p>No rule in the knowledge base matches this message.</p>
      <p class="dim">Solve it once — then add a [[remedy]] rule so Shakkanakku remembers forever.</p>`;
  }
...
function switchTab(name) {
  ...
  // Tabs 2/3 (Shakkanakku, Hala) still need a real passport --
  ...
  currentTab = name;
  ['resource', 'shakkanakku', 'hala', 'gates'].forEach(t => {
...
function renderGate() {
  ...
  if (!lastResourceCheck || !lastResourceCheck.checked) {
    gateEl.textContent = '⏸ run the resource check before starting a Shakkanakku run or Hala pulse';
    gateEl.className = 'mono gate-status gate-unknown';
  } else if (lastResourceCheck.green) {
    gateEl.textContent = '✓ green — Shakkanakku runs and Hala pulses are allowed';
