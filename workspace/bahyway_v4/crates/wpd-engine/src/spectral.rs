...
impl SpectralScan {
    pub fn new(
        ...
        Ok(Self { segment_id: segment_id.into(), bands, timestamp_ms, altitude_m, gsd_cm })
    }
    ...
    pub fn vegetation_index(&self) -> f32 {
        ...
        let denom = nir + green;
        if denom < 1e-6 { 0.0 } else { (nir - green) / denom }
    }
...
mod tests {
    ...
    fn water_leak_scores_high() {
        ...
        assert!(scan.composite_defect_score() > 0.40,
            "water leak should score > 0.40, got {}", scan.composite_defect_score());
    }
    ...
    fn wavelengths_monotone_increasing() {
        let bands = [
            SpectralBand::B1Blue, SpectralBand::B2Green, SpectralBand::B3Red,
            SpectralBand::B4Nir,  SpectralBand::B5Swir1, SpectralBand::B6Swir2,
            SpectralBand::B7Swir3,SpectralBand::B8Swir4, SpectralBand::B9Tir1,
            SpectralBand::B10Tir2,SpectralBand::B11Tir3, SpectralBand::B12Tir4,
        ];
        let wl: Vec<u32> = bands.iter().map(|b| b.wavelength_nm()).collect();
        for w in wl.windows(2) {
            assert!(w[0] < w[1], "wavelengths not monotone: {} >= {}", w[0], w[1]);
        }
    }
}
