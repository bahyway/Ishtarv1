...
impl RepairScheduler {
    ...
    pub fn schedule(segment: &PipelineSegment, event: &DefectEvent) -> RepairJob {
        ...
        let composite_score   = (severity as f32 / 100.0 * 0.50
            + risk_score        * 0.30
        ...
        RepairJob {
            ...
            description:       format!(
                "{} — {} ({}) composite={:.3}",
                segment.id, event.defect_class.name(), priority.name(), composite_score
            ),
        }
    }
...
mod tests {
    ...
    fn priority_from_severity_boundaries() {
        assert_eq!(RepairPriority::from_severity(100), RepairPriority::Emergency);
