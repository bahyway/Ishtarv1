...
pub enum FormatError {
    ...
    /// Maps to NRM_MALFORMED.
    InvalidJson { byte_offset: usize, reason: &'static str },
...
impl core::fmt::Display for FormatError {
    fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
        match self {
            Self::RaggedCsvRow { row, expected, got } =>
                write!(f, "NARAMSIN NRM_MALFORMED: CSV row {row} has {got} fields, expected {expected}"),
            Self::InvalidJson { byte_offset, reason } =>
                write!(f, "NARAMSIN NRM_MALFORMED: JSON parse error at byte {byte_offset}: {reason}"),
            Self::InvalidXml { line, reason } =>
                write!(f, "NARAMSIN NRM_MALFORMED: XML parse error at line {line}: {reason}"),
            Self::SchemaDrift { source_id } =>
                write!(f, "NARAMSIN NRM_SCHEMA_DRIFT: schema changed for source {source_id}"),
        }
    }
}
