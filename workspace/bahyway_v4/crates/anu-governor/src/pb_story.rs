...
pub fn playbook_story_events(
    ...
    if let Some((minted_at, source_path)) =
        pb_catalog::catalog_event_for(pb_registry_path, kaki_hex)
    {
    ...
    for (gate_name, domain_name, approved_at) in
        domain_review::domain_events_for(domain_registry_path, kaki_hex)
    {
        events.push(StoryEvent {
            ts: approved_at,
    ...
    let dep_events =
        pb_dependency_review::dependency_events_for(dependency_registry_path, kaki_hex);
    if !dep_events.is_empty() {
        let titles: HashMap<String, String> =
            pb_catalog::list_cataloged_playbooks(pb_registry_path)
                .into_iter()
        ...
        for (other_hex, approved_at, is_source) in dep_events {
            let other_title = titles
                .get(&other_hex)
                .cloned()
                .unwrap_or_else(|| other_hex.clone());
            let (kind, title, detail) = if is_source {
                (
                    "DEPENDS_ON_APPROVED",
                    "Dependency confirmed",
                    format!("Confirmed depends-on {other_title}"),
                )
            } else {
                (
                    "DEPENDED_ON_BY_APPROVED",
                    "Dependent confirmed",
                    format!("Confirmed depended-on-by {other_title}"),
                )
            };
            events.push(StoryEvent {
                ts: approved_at,
...
mod tests {
    ...
    fn a_real_chronicle_merges_all_4_registries_in_timestamp_order() {
        ...
        let first = catalogued
            .iter()
            .find(|p| p.source_path.contains("980"))
            .unwrap();
        let second = catalogued
            .iter()
            .find(|p| p.source_path.contains("981"))
            .unwrap();
        ...
        let mut wn = enkiddb::WriteNode::new(
        ...
        let story = playbook_story_events(
            &pb_registry,
            &gate_registry,
            &domain_registry,
            &dependency_registry,
        ...
        assert!(
            story[3].detail.contains(&second.title),
            "the dependency event must name the real other playbook: {story:?}"
        );
        ...
        let second_story = playbook_story_events(
            &pb_registry,
            &gate_registry,
            &domain_registry,
            &dependency_registry,
        ...
        assert!(
            second_story
                .iter()
                .any(|e| e.kind == "DEPENDED_ON_BY_APPROVED" && e.detail.contains(&first.title)),
