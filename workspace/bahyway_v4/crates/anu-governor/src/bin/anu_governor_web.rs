//! AnuGovernor's browser-facing governor. Same engine as the native egui
//! face (`anu_governor::{config,runner,remedy,report,chronicle}`, unchanged)
...
//! TLS (`anu_governor::web_tls`), every mutating action requires a real
//! kupru passport session (`anu_governor::web_auth` — Ed25519-signed,
...
use anu_governor::pb_dependency_review;
...
fn main() -> anyhow::Result<()> {
    ...
    println!("𒀭 AnuGovernor web governor listening on https://{addr}/ (Ctrl-C to stop)");
...
fn export_report(format: &str, state: &Arc<Mutex<ServerState>>) -> Resp {
    ...
    match format {
        ...
        "html" => {
            let html = report::to_html(&md, "AnuGovernor — Infrastructure Deployment Report");
            (
                200,
...
// `anu_governor::gate_review`'s own doc comment) -- nothing in this section
...
/// Rust logic inside the already-launched `anu_governor_web` binary,
...
/// Thin wrapper over `anu_governor::pb_catalog_rebuild::rebuild_full_write_node`
...
/// Thin wrapper over `anu_governor::pb_story::playbook_story_events` (a
/// path-parameterized, fully unit-tested lib function) supplying this
/// server's own env-resolved registry paths.
fn playbook_story_json(kaki_hex: &str) -> Vec<anu_governor::pb_story::StoryEvent> {
    anu_governor::pb_story::playbook_story_events(
        &pb_catalog_registry_path(),
        &gate_registry_path(),
        &domain_registry_path(),
        &dependency_registry_path(),
...
fn search_playbooks_resp(query_str: &str) -> Resp {
    ...
    let all = anu_governor::pb_catalog::list_cataloged_playbooks(&pb_catalog_registry_path());
...
fn auth_requirement(method: &str, path: &str) -> AuthReq {
    match (method, path) {
        ...
        // (AnuGovernor, Hala) -- and every action anywhere -- stay
