...
/// key file on disk — see `docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md`'s "Secrets"
/// section. Until AnuGovernor has a scriptable way to mint this key inside
...
pub fn generate(report_dir: &str, seal_key: &str, run: &RunSummary) -> Result<PathBuf> {
    ...
    md.push_str("# AnuGovernor — Infrastructure Deployment Report\n\n");
    md.push_str(&format!(
        "**Run:** {} → {} UTC  \n**Duration:** {}m {}s  \n**Corpus:** {} playbooks\n\n",
    ...
    let path = PathBuf::from(report_dir).join(format!("anu_governor_report_{ts}.md"));
...
mod tests {
    ...
    fn seal_key_parent_directory_is_created_if_missing() {
        ...
        let keyp = base.join("secrets").join("anu_governor_seal.key");
