...
impl GgufTensor {
    ...
    pub fn dims_2d(&self) -> (usize, usize) {
        match self.shape.len() {
            ...
            _ => (self.shape[self.shape.len()-1] as usize,
...
impl GgufMetadata {
    ...
    pub fn num_heads(&self) -> Option<u32> {
        let arch = self.arch()?;
        self.kv.get(&format!("{arch}.attention.head_count"))?.as_u32()
    }
    /// Number of KV heads (GQA).
    pub fn num_kv_heads(&self) -> Option<u32> {
        let arch = self.arch()?;
        self.kv.get(&format!("{arch}.attention.head_count_kv"))?.as_u32()
    }
    /// Feed-forward dimension.
    pub fn feed_forward_dim(&self) -> Option<u32> {
        let arch = self.arch()?;
        self.kv.get(&format!("{arch}.feed_forward_length"))?.as_u32()
    }
    /// Vocabulary size.
...
impl GgufFile {
    ...
    pub fn parse(data: Vec<u8>) -> Result<Self, GgufError> {
        ...
        let metadata = GgufMetadata { version, tensor_count, kv };
        ...
        for _ in 0..tensor_count {
            ...
            tensors.insert(name.clone(), GgufTensor {
                name, shape, quant_type, data_offset, data_size
            });
        }
    ...
    pub fn tensor_data(&self, name: &str) -> Result<&[u8], GgufError> {
        ...
        if end > self.data.len() {
            return Err(GgufError::AlignmentError(
                format!("tensor {name}: offset {start}+size {} > file size {}", t.data_size, self.data.len())
            ));
        }
...
fn read_u8(data: &[u8], cursor: &mut usize) -> Result<u8, GgufError> {
    if *cursor + 1 > data.len() { return Err(GgufError::Io("EOF reading u8".into())); }
    let v = data[*cursor];
    *cursor += 1;
    Ok(v)
}
...
fn read_u16(data: &[u8], cursor: &mut usize) -> Result<u16, GgufError> {
    if *cursor + 2 > data.len() { return Err(GgufError::Io("EOF reading u16".into())); }
...
fn read_u32(data: &[u8], cursor: &mut usize) -> Result<u32, GgufError> {
    if *cursor + 4 > data.len() { return Err(GgufError::Io("EOF reading u32".into())); }
...
fn read_u64(data: &[u8], cursor: &mut usize) -> Result<u64, GgufError> {
    if *cursor + 8 > data.len() { return Err(GgufError::Io("EOF reading u64".into())); }
...
mod tests {
    ...
    fn invalid_magic_rejected() {
        ...
        assert!(matches!(GgufFile::parse(data), Err(GgufError::InvalidMagic(_))));
    }
