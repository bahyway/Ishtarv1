...
impl BpeTokenizer {
    ...
    pub fn from_vocab(tokens: Vec<String>, merges: Vec<String>, bos_id: u32, eos_id: u32) -> Self {
    ...
    pub fn encode(&self, text: &str, add_bos: bool) -> Vec<u32> {
        ...
        // Map to token IDs
        for ch in &chars {
            let id = self.vocab_index.get(ch).copied().unwrap_or(self.unk_id);
            tokens.push(id);
        }
        tokens
    }
