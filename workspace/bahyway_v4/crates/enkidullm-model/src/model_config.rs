...
impl ModelConfig {
    ...
    pub fn from_metadata(meta: &crate::gguf::GgufMetadata) -> Option<Self> {
        ...
        let arch = ModelArch::parse_arch(arch_str);
        Some(Self {
            arch,
...
mod tests {
    ...
    fn model_arch_from_str() {
        assert_eq!(ModelArch::parse_arch("qwen2"), ModelArch::Qwen2);
        assert_eq!(ModelArch::parse_arch("llama"), ModelArch::LLaMA);
        assert_eq!(ModelArch::parse_arch("unknown_xyz"), ModelArch::Unknown);
    }
    #[test]
    fn head_dim_correct() {
        let cfg = ModelConfig {
