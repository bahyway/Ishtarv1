...
impl DqmReport {
    ...
    pub fn new(scores: [DimensionScore; 6], sla: DqmSla) -> Self {
        ...
        DqmReport { scores, sla, composite, composite_b11, sla_compliant }
    }
    ...
    pub fn summary(&self) -> String {
        ...
        if failing.is_empty() {
            format!("[DQM {status}] composite={:.2} B11={}", self.composite, self.composite_b11)
        } else {
            let names: Vec<&str> = failing.iter().map(|d| d.label()).collect();
            format!(
                "[DQM {status}] composite={:.2} B11={} | failing: {}",
                self.composite, self.composite_b11, names.join(", ")
            )
        }
    }
}
...
impl DqmBatchReport {
    pub fn from_reports(reports: &[DqmReport]) -> Self {
        ...
        for report in reports {
            for (i, score) in report.scores.iter().enumerate() {
                ...
                if score.meets_sla(&report.sla) { dim_pass[i] += 1; }
            }
        }
    ...
    pub fn compliance_rate(&self) -> f32 {
        if self.total_records == 0 { return 1.0; }
    ...
    pub fn dimension_label(i: usize) -> &'static str {
        DqmDimension::ALL.get(i).map(|d| d.label()).unwrap_or("Unknown")
    }
}
