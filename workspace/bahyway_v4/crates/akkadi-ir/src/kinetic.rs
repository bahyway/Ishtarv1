...
impl KineticForce {
    ...
    pub fn memory(staleness: f32, source: &str) -> Self {
        Self {
            kind: ForceKind::Memory { staleness },
            dimension: None,
            source: source.to_string(),
        }
    }
    ...
    pub fn learning(rule_confidence: f32, source: &str) -> Self {
        Self {
            kind: ForceKind::Learning { rule_confidence },
            dimension: None,
            source: source.to_string(),
        }
    }

    pub fn df_dt(&self) -> f32 {
        self.kind.magnitude()
    }
}
...
mod tests {
    ...
    #[test]
    fn physical_is_negative() {
        assert!(KineticForce::physical(0.4, 0.3, "Vault").df_dt() < 0.0);
    }
    #[test]
    fn memory_is_negative() {
        assert!(KineticForce::memory(0.7, "Story").df_dt() < 0.0);
    }
    #[test]
    fn learning_is_positive() {
        assert!(KineticForce::learning(0.9, "AkkadianAOL").df_dt() > 0.0);
    }
