...
use crate::node::{AkkNode, ParticleNode, TribeNode, EmitNode, PipelineNode};
...
impl AkkIr {
    pub fn nodes(&self)    -> &[AkkNode] { &self.nodes }
    ...
    pub fn count_kind(&self, keyword: &str) -> usize {
        self.nodes.iter().filter(|n| n.kind_keyword() == keyword).count()
    }

    pub fn particles(&self) -> impl Iterator<Item = &crate::node::ParticleNode> {
        self.nodes.iter().filter_map(|n| if let AkkNode::Particle(p) = n { Some(p) } else { None })
    }

    pub fn tribes(&self) -> impl Iterator<Item = &crate::node::TribeNode> {
        self.nodes.iter().filter_map(|n| if let AkkNode::Tribe(t) = n { Some(t) } else { None })
    }

    pub fn rules(&self) -> impl Iterator<Item = &crate::node::RuleNode> {
        self.nodes.iter().filter_map(|n| if let AkkNode::Rule(r) = n { Some(r) } else { None })
    }

    pub fn emits(&self) -> impl Iterator<Item = &crate::node::EmitNode> {
        self.nodes.iter().filter_map(|n| if let AkkNode::Emit(e) = n { Some(e) } else { None })
    }

    pub fn is_generative(&self) -> bool { self.nodes.iter().any(|n| n.is_generative()) }

    pub fn summary(&self) -> String {
        format!(
            "AkkIr[{}] particles={} tribes={} rules={} equations={} flows={} observes={} emits={}",
            self.source,
            self.count_kind("PARTICLE"), self.count_kind("TRIBE"),
            self.count_kind("RULE"), self.count_kind("EQUATION"),
            self.count_kind("FLOW"), self.count_kind("OBSERVE"),
...
impl IrBuilder {
    pub fn new(source: &str) -> Self {
        Self { nodes: Vec::new(), by_id: HashMap::new(), by_name: HashMap::new(), source: source.to_string() }
    }
    ...
    pub fn add_node(&mut self, node: AkkNode) -> Result<NodeId, IrError> {
        ...
        if self.by_name.contains_key(&name) {
            return Err(IrError::DuplicateNode { name, span: node.span().clone() });
        }
    ...
    pub fn build(self) -> Result<AkkIr, IrError> {
        if self.nodes.is_empty() { return Err(IrError::EmptyIr); }
        Ok(AkkIr { nodes: self.nodes, by_id: self.by_id, by_name: self.by_name,
                   source: self.source, ir_version: crate::IR_VERSION.to_string() })
    }

    pub fn build_unchecked(self) -> AkkIr {
        AkkIr { nodes: self.nodes, by_id: self.by_id, by_name: self.by_name,
                source: self.source, ir_version: crate::IR_VERSION.to_string() }
    }
    ...
    pub fn emit_self_heal(&mut self, name: &str, template: &str, span: Span) -> Result<NodeId, IrError> {
    ...
    pub fn pipeline(mut self, name: &str, version: &str) -> Result<AkkIr, IrError> {
        ...
        for node in inner { pip = pip.add(node); }
...
mod tests {
    ...
    fn sp() -> Span { Span::generated() }

    #[test]
    fn empty_ir_returns_error() { assert!(matches!(IrBuilder::new("t").build(), Err(IrError::EmptyIr))); }
    ...
    fn duplicate_returns_error() {
        let mut b = IrBuilder::new("t");
        b.particle("citizen", sp()).unwrap();
        assert!(matches!(b.particle("citizen", sp()), Err(IrError::DuplicateNode { .. })));
    }
