...
impl OrbitalDeviationEntry {
    pub fn new(
        ...
        let payload = build_payload(
            particle_id, observed_epoch, reference_epoch,
            cartesian_delta, ring_changed, cause,
        );
    ...
    pub fn verify(&self) -> bool {
        let payload = build_payload(
            self.particle_id, self.observed_epoch, self.reference_epoch,
            self.cartesian_delta, self.ring_changed, self.cause,
        );
    ...
    pub fn to_markdown(&self) -> String {
        format!(
             ...
             **Evidence Seal:** `{:#06x}`\n",
            self.particle_id,
            self.reference_epoch, self.observed_epoch,
            self.cartesian_delta, self.ring_changed,
            self.cause.label(), self.trust_penalty,
            self.checksum,
        )
    }
}
...
impl OrbitalDeviationJournal {
    ...
    pub fn entries(&self)  -> &[OrbitalDeviationEntry] { &self.entries }
...
mod tests {
    ...
    fn make_entry(pid: u64, obs: u64) -> OrbitalDeviationEntry {
        OrbitalDeviationEntry::new(pid, obs, obs - 1, 0.42, true, DeviationCause::Unexplained, 0.3)
    }
    ...
    fn accumulated_penalty_clamps_at_one() {
        ...
        assert!((penalty - 1.0).abs() < 1e-6, "penalty must clamp at 1.0, got {penalty}");
    }
