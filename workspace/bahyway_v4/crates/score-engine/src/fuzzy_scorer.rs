...
use fuzzy_engine::{FuzzyDimensions, FuzzyEngine, FormatLayer, QualityTier};
...
pub fn score(input: &ScoreInput) -> ScoreResult {
    ...
    ScoreResult { b11, tier, state, hps, color, freshness }
}
...
fn encode_f32(v: f32) -> Vec<u8> {
    ...
    let (int_part, frac) = if frac >= 1000 { (int_part + 1, 0) } else { (int_part, frac) };
...
mod tests {
    ...
    fn perfect_particle_scores_gem_golden() {
        ...
        assert!(result.b11 >= 200, "perfect must be Gem, got B11={}", result.b11);
    }
    ...
    fn perfect_particle_color_has_correct_domain_byte() {
        ...
        assert!(result.color.b > 250, "freshness blue should be near max at t=0");
    }
    ...
    fn format_layer_multiplier_affects_b11() {
        ...
        let civil = score(&ScoreInput {
            dims: dims.clone(), format: FormatLayer::CivilRegistry,
            domain_byte: 0, elapsed_seconds: 0.0, decay: FreshnessDecay::civil_registry(),
            orbital_trust_penalty: 0.0,
        });
        let generic = score(&ScoreInput {
            dims, format: FormatLayer::GenericEav,
            domain_byte: 0, elapsed_seconds: 0.0, decay: FreshnessDecay::civil_registry(),
            orbital_trust_penalty: 0.0,
        });
        assert!(civil.b11 >= generic.b11, "CivilRegistry must score >= GenericEav");
    }
    ...
    fn freshness_decays_over_time() {
        ...
        assert!(stale.freshness < fresh.freshness, "freshness must decay over time");
        assert!(stale.color.b < fresh.color.b, "blue byte must decay with freshness");
    }
    ...
    fn orbital_penalty_lowers_b11() {
        ...
        assert!(penalised.b11 <= clean.b11,
            "orbital penalty must not increase b11: clean={} penalised={}",
            clean.b11, penalised.b11);
    }
    ...
    fn zero_orbital_penalty_preserves_score() {
        ...
        assert_eq!(normal.b11, explicit.b11, "zero penalty must leave b11 identical");
    }
    ...
    fn unknown_trust_lowers_score_vs_authoritative() {
        ...
        assert!(auth.b11 >= unkn.b11, "authoritative trust must score >= unknown");
    }
}
