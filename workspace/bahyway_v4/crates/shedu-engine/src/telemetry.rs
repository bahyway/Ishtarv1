...
use crate::hardware::{
    CPU_CRITICAL_PCT, CPU_WARN_PCT, GPU_TEMP_CRITICAL, GPU_TEMP_WARN, NET_LATENCY_CRITICAL_MS,
    NET_LATENCY_WARN_MS, VRAM_CRITICAL_PCT, VRAM_WARN_PCT,
};
...
impl TelemetryFeed {
    ...
    pub fn ingest(&mut self, node_id: u8, m: &HardwareMetrics) {
        ...
        if m.gpu_temp_celsius >= GPU_TEMP_CRITICAL {
            self.events.push(TelemetryEvent::GpuTempCritical {
                node_id,
                temp_celsius: m.gpu_temp_celsius,
                timestamp_ms: t,
            });
    ...
    pub fn node_down(&mut self, node_id: u8, timestamp_ms: u64) {
        self.events.push(TelemetryEvent::NodeDown {
            node_id,
            timestamp_ms,
        });
    }
    ...
    pub fn last_node_failing(&mut self, node_id: u8, b11: u8, timestamp_ms: u64) {
        self.events.push(TelemetryEvent::LastNodeFailing {
            node_id,
            b11,
            timestamp_ms,
        });
