...
impl HardwareMetrics {
    ...
    pub fn vram_utilisation(&self) -> f32 {
        if self.vram_total_mb == 0 { return 1.0; } // treat unknown as saturated
    ...
    pub fn ram_utilisation(&self) -> f32 {
        if self.ram_total_mb == 0 { return 1.0; }
        (self.ram_used_mb as f32 / self.ram_total_mb as f32).clamp(0.0, 1.0)
    }

    pub fn gpu_critical(&self)  -> bool { self.gpu_temp_celsius >= GPU_TEMP_CRITICAL }
    pub fn vram_critical(&self) -> bool { self.vram_utilisation() >= VRAM_CRITICAL_PCT }
    pub fn cpu_critical(&self)  -> bool { self.cpu_load >= CPU_CRITICAL_PCT }
    pub fn net_critical(&self)  -> bool { self.net_latency_ms >= NET_LATENCY_CRITICAL_MS }
...
mod tests {
    ...
    fn gpu_warn_not_critical() {
        let m = HardwareMetrics { gpu_temp_celsius: 87.0, ..HardwareMetrics::simulated_healthy(0) };
        assert!(!m.gpu_critical());
    }
}
