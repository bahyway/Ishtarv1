...
pub fn evaluate_rules(f: &FuzzifiedDimensions) -> AggregatedOutput {
    ...
    out.apply(RuleFiring { tier: QualityTier::Gem, strength: sr01 * 1.0 });
...
mod tests {
    ...
    fn fuzzify(d: &FuzzyDimensions) -> FuzzifiedDimensions { d.fuzzify() }

    #[test]
    fn perfect_record_fires_gem_strongly() {
        let f   = fuzzify(&FuzzyDimensions::perfect());
        let out = evaluate_rules(&f);
        assert!(out.gem > 0.8, "perfect record must fire Gem > 0.8, got {}", out.gem);
    }
    ...
    fn near_duplicate_fires_active() {
        ...
        let f   = fuzzify(&d);
        let out = evaluate_rules(&f);
        assert!(out.active > 0.5, "near-duplicate must fire Active > 0.5, got {}", out.active);
    }
    ...
    fn invalid_id_and_date_fires_non_active() {
        ...
        let f   = fuzzify(&d);
        let out = evaluate_rules(&f);
        assert!(out.non_active > 0.5, "invalid id+date must fire NonActive, got {}", out.non_active);
    }
    ...
    fn empty_record_fires_non_active() {
        ...
        let f   = fuzzify(&d);
        let out = evaluate_rules(&f);
        assert!(out.non_active > 0.5, "empty record must fire NonActive, got {}", out.non_active);
    }
    ...
    fn good_but_not_perfect_fires_tribe() {
        ...
        let f   = fuzzify(&d);
        let out = evaluate_rules(&f);
        assert!(out.tribe > 0.1 || out.gem > 0.1, "good record must fire Tribe or Gem");
    }

    #[test]
    fn aggregated_output_max_aggregation() {
        let mut agg = AggregatedOutput::default();
        agg.apply(RuleFiring { tier: QualityTier::Gem, strength: 0.4 });
        agg.apply(RuleFiring { tier: QualityTier::Gem, strength: 0.9 });
        agg.apply(RuleFiring { tier: QualityTier::Gem, strength: 0.6 });
