...
use ea_agent_core::{ParticleSnapshot, HeptaVector};
...
impl HarmonyResult {
    pub fn is_harmonious(&self) -> bool { self.harmony >= 0.75 }
    pub fn meets_gem_target(&self) -> bool { self.gem_rate >= GEM_RATE_TARGET }
}
...
impl HarmonyEngine {
    ...
    pub fn compute(&self, tribe_id: u16, particles: &[ParticleSnapshot]) -> HarmonyResult {
        ...
        HarmonyResult { tribe_id, particle_count: n, harmony, gem_rate, centroid, avg_b11, assessment }
    }
    ...
    fn assess(&self, harmony: f64, gem_rate: f64, avg_b11: f64) -> String {
        let target_met = if gem_rate >= GEM_RATE_TARGET { "✅" } else { "⚠" };
        format!(
            "Harmony: {:.3} | GEM rate: {:.1}% {target_met} (target {:.1}%) | Avg B11: {:.1}/240",
            harmony, gem_rate * 100.0, GEM_RATE_TARGET * 100.0, avg_b11
        )
    }
}

impl Default for HarmonyEngine { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn identical_particles_max_harmony() {
        ...
        assert!(r.harmony > 0.95, "identical particles → harmony ≈ 1.0, got {}", r.harmony);
    }
