...
pub enum Equation {
    ...
    HeptaQuality { dimensions: [f64; 7], target: [f64; 7], weights: [f64; 7] },
...
impl SolverResult {
    pub fn has_real_roots(&self) -> bool { !self.roots.is_empty() }
    pub fn is_exact(&self) -> bool { self.complex.is_empty() || !self.roots.is_empty() }
}
...
impl AlgebraSolver {
    ...
    pub fn solve(&self, eq: &Equation) -> SolverResult {
        match eq {
            ...
            Equation::HeptaQuality { dimensions, target, weights } =>
                self.solve_hepta_quality(*dimensions, *target, *weights),
            Equation::SpectralRadius2x2 { a, b, c, d } =>
                self.solve_spectral_2x2(*a, *b, *c, *d),
        }
    }
    ...
    fn solve_linear(&self, a: f64, b: f64) -> SolverResult {
        ...
        if a.abs() < ABZU_PRECISION {
            ...
            return SolverResult {
                ...
                roots: vec![], complex: vec![], value: None,
                steps, latex: format!("{a}x + {b} = 0"),
            };
        }
        ...
        SolverResult {
            equation: format!("{a}x + {b} = 0"),
            roots: vec![x], complex: vec![], value: Some(x),
            steps,
    ...
    fn solve_quadratic(&self, a: f64, b: f64, c: f64) -> SolverResult {
        ...
        steps.push(format!("Discriminant Δ = b² - 4ac = {b}² - 4({a})({c}) = {discriminant}"));

        if discriminant > 0.0 {
            let sq = discriminant.sqrt();
            let x1 = (-b + sq) / (2.0 * a);
            let x2 = (-b - sq) / (2.0 * a);
            steps.push(format!("Δ > 0 → two distinct real roots"));
            steps.push(format!("x₁ = (-{b} + {sq:.6}) / {:.6} = {x1:.6}", 2.0*a));
            steps.push(format!("x₂ = (-{b} - {sq:.6}) / {:.6} = {x2:.6}", 2.0*a));
            SolverResult {
                equation: format!("{a}x² + {b}x + {c} = 0"),
                roots: vec![x1, x2], complex: vec![], value: None,
                steps,
                latex: format!("x = \\frac{{-{b} \\pm \\sqrt{{{discriminant:.4}}}}}{{{}}}",
                    2.0*a),
            }
        } else if discriminant.abs() < ABZU_PRECISION {
            let x = -b / (2.0 * a);
            steps.push(format!("Δ = 0 → one repeated real root"));
            ...
            steps.push(format!("Δ < 0 → two complex conjugate roots"));
            steps.push(format!("x₁ = {real:.6} + {imag:.6}i"));
            steps.push(format!("x₂ = {real:.6} - {imag:.6}i"));
            SolverResult {
                equation: format!("{a}x² + {b}x + {c} = 0"),
                roots: vec![], complex: vec![(real, imag), (real, -imag)],
                value: None, steps,
    ...
    fn solve_cubic(&self, a: f64, b: f64, c: f64, d: f64) -> SolverResult {
        ...
        steps.push(format!("Depressed cubic t³ + {pp:.4}t + {qq:.4} = 0 (shift x = t - {shift:.4})"));
        ...
        let roots = if disc > 0.0 {
            ...
            let u = (-qq / 2.0).cbrt();
            steps.push(format!("D = 0: repeated roots"));
            ...
            steps.push(format!("D < 0: three real roots via trigonometric method"));
            vec![t1, t2, t3]
        };

        let latex = roots.iter().map(|r| format!("x={r:.4}")).collect::<Vec<_>>().join(", ");
        SolverResult {
            equation: format!("{a}x³ + {b}x² + {c}x + {d} = 0"),
            roots, complex: vec![], value: None, steps,
    ...
    fn solve_hepta_quality(&self, d: [f64; 7], t: [f64; 7], w: [f64; 7]) -> SolverResult {
        ...
        let weighted_sq: f64 = w.iter().zip(d.iter().zip(t.iter()))
        ...
        steps.push(format!("B11 = round(H(P) × 240) = round({:.4} × 240) = {b11}", hp));

        SolverResult {
            equation: "H(P) quality equation".into(),
            roots: vec![], complex: vec![], value: Some(hp),
            steps,
    ...
    fn solve_spectral_2x2(&self, a: f64, b: f64, c: f64, d: f64) -> SolverResult {
        ...
        let sr = if result.has_real_roots() {
            ...
            result.complex.iter().map(|(r, i)| (r*r + i*i).sqrt()).fold(0.0f64, f64::max)
        };
        ...
        SolverResult {
            ...
            roots: result.roots, complex: result.complex,
            value: Some(sr), steps: final_steps,
            latex: format!("\\rho(A) = {sr:.6}"),
        }
    }
}

impl Default for AlgebraSolver { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn quadratic_two_real_roots() {
        // x² - 5x + 6 = 0 → x=2, x=3
        let r = AlgebraSolver::new().solve(&Equation::Quadratic { a:1.0, b:-5.0, c:6.0 });
    ...
    fn quadratic_complex_roots() {
        // x² + 1 = 0 → ±i
        let r = AlgebraSolver::new().solve(&Equation::Quadratic { a:1.0, b:0.0, c:1.0 });
    ...
    fn hepta_quality_unit_vector() {
        ...
        let r = AlgebraSolver::new().solve(&Equation::HeptaQuality {
            dimensions: d, target: t, weights: HEPTA_WEIGHTS
        });
    ...
    fn steps_contain_chain_of_thought() {
        let r = AlgebraSolver::new().solve(&Equation::Quadratic { a:1.0, b:-2.0, c:1.0 });
