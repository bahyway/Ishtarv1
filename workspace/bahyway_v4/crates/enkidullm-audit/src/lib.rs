...
pub use concept_graph::{
    extract_concepts, concept_graph_overlap, jaccard_similarity, missing_concepts,
};
pub use citation::{
    analyze_citation_coverage, citation_depth, CitationAnalysis, SUSPICIOUS_THRESHOLD,
};
pub use probe::{
    probe_plagiarism, batch_probe, ProbeResult, ProbeInput,
    OVERLAP_SUSPICIOUS, OVERLAP_HIGH,
};
...
pub fn audit_and_journal(
    ...
    for result in &results {
        if result.is_flagged {
            ...
            if journal.append(entry) { written += 1; }
        }
    }
    written
}
...
mod tests {
    ...
    fn full_audit_pipeline() {
        ...
        assert_eq!(written, 1, "one flagged pair should produce one journal entry");
