...
pub fn probe_plagiarism(
    ...
    ProbeResult {
        ...
        source_uuid,
        knowledge_overlap,
        citation_coverage: citation_analysis.coverage_ratio,
...
fn classify_detection_mode(
    ...
    if overlap >= OVERLAP_HIGH && citation.coverage_ratio < SUSPICIOUS_THRESHOLD {
        if !missing.is_empty() {
            format!(
                "HIGH_OVERLAP_WITH_CONCEPT_ERASURE_θ₆ overlap={:.2} citation_gap={:.2}",
                overlap,
                1.0 - citation.coverage_ratio
            )
        } else {
        ...
        format!(
            "MODERATE_OVERLAP_LOW_CITATION overlap={:.2} coverage={:.2}",
...
mod tests {
    ...
    fn probe_identical_books_high_overlap() {
        ...
        assert!(
            result.is_flagged,
            "identical uncited book should be flagged"
        );
    ...
    fn probe_renamed_concepts_detected() {
        ...
        let result = probe_plagiarism(0xBBBB, 0xAAAA, &suspect, &source, &[], IduState::Orange);
        // Should have significant overlap despite renames
        assert!(
            result.knowledge_overlap > 0.5,
            "overlap={}",
    ...
    fn plagiarism_signal_gap_correct() {
        let triples = cap_triples();
        let result = probe_plagiarism(0xBBBB, 0xAAAA, &triples, &triples, &[], IduState::Gold);
