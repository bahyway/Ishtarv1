...
pub fn missing_concepts(
    ...
    if suspect_cited_hashes.contains(&source_uuid_hash) {
        return Vec::new();
    }
...
mod tests {
    ...
    fn concept_overlap_same_triples() {
        let triples = vec![
            t("ch1", "introduces", "CAP"),
            t("ch2", "depends_on", "ACID"),
        ];
    ...
    fn concept_overlap_renamed_concepts() {
        ...
        let source = vec![
            ...
            t("ch1", "discusses", "ACID"),
        ];
        let suspect = vec![
            t("ch1", "introduces", "Consistency_Trilemma"),
            t("ch1", "discusses", "ACID"),
        ];
    ...
    fn no_missing_when_cited() {
        ...
        assert!(
            missing.is_empty(),
            "no missing citations when source is cited"
        );
    }
