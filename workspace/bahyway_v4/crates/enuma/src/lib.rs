...
//!
//! The originating playbook (bw4_playbook_56_enuma.yml) depends on
//! `tiamat::snapshot::ParticleSnapshot`, which doesn't exist anywhere in
//! this repo -- only `tiamat-engine4` exists, with no `snapshot` module or
//! `ParticleSnapshot` type. ENUMA's actual pattern-matching algorithms
//! only need four fields (kaki_hex, tribe_id, eav_quality, an ordering
//! key), so [`ParticleSnapshot`] is defined locally here instead of
//! depending on a crate that doesn't exist.

#![forbid(unsafe_code)]

/// Minimal snapshot ENUMA's pattern matchers need: one particle's EAV
/// quality at one point in its history. Deliberately small and local (see
