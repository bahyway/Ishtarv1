...
pub use plan::{QueryPlan, PlanNode, OpKind, JoinType, ScanType};
pub use bottleneck::{Bottleneck, BottleneckKind, BottleneckDetector, Severity};
pub use journey::{SevenLevelJourney, JourneyLevelId, LevelScore, DmwState};
pub use oracle::{TheOracle, Recommendation, RotationType};
pub use fragmentation::{StorageFragment, FragmentationMap, FragmentationState};
pub use colorid::{ColorId, ColorIdBuilder, LevelColorContribution};
pub use particle::{QueryParticle, MotionState};
pub use narrative::{NarrativeBuilder, Narrative, gate_summary};
pub use cardinality::{CardinalityAnalyser, CardinalityAnalysis, CardinalityError, StatisticsHealth};
pub use cpu_analysis::{CpuAnalyser, CpuAnalysis, CpuPattern, CpuSmell};
pub use cache_analysis::{CacheAnalyser, CacheAnalysis, SniffingRisk, CacheFixStrategy};
pub use heptagon::{HeptaWeights, HeptaDimension};
pub use gray_rot::{GrayRotLevel, GrayRotReport};
pub use execution_plan::{PlanOperator, PlanGraph, PlanNode as GraphNode, PlanComparator};
pub use data_simulator::{DataSimulator, ExecutionMetrics, SalesOrderRow, RowStatus};
pub use io_analysis::{IoAnalyser, IoAnalysis, IoJoinType, JoinCostMetrics};
pub use index_health::{IndexCoverageStatus, IndexHealthAnalyser, IndexHealthAnalysis,
                        IndexRecommendation};
pub use level_result::{LevelResult, AnalysisJourney};
pub use particle::LeakType;
pub use sargability::{NonSargableKind, PredicateAnalysis, SargabilityAnalyser, SargabilityAnalysis};
pub use set_theory::{SetOperator, SetSmell, SetSmellKind, SmellSeverity, SetTheoryAnalyser,
                     SetTheoryAnalysis, SetExprNode};
...
impl DmwReport {
    pub fn is_golden(&self)    -> bool { self.state == DmwState::Golden    }
    pub fn is_critical(&self)  -> bool { self.state == DmwState::Critical  }
    pub fn problem_count(&self)-> usize { self.bottlenecks.len() }
    ...
    pub fn technical_summary(&self) -> String {
        ...
        format!(
            "Query: {} | {:.1}% ({}) | Bottlenecks: {} | Oracle fixes: {} | \
             Projected: {:.1}% | {}",
            self.query_id, self.alignment_pct, self.state.label(),
            self.bottlenecks.len(), self.recommendations.len(),
            self.projected_pct, weights,
        )
    }
}
...
mod tests {
    ...
    #[test] fn version_non_empty() {
        assert!(!DMW_ENGINE_VERSION.is_empty());
    }
    #[test] fn journey_levels_is_seven() {
        assert_eq!(DMW_JOURNEY_LEVELS, 7);
    }
    #[test] fn golden_above_critical() {
        assert!(GOLDEN_THRESHOLD > CRITICAL_THRESHOLD);
    }
    #[test] fn sales_order_report_structure() {
        let plan   = sales_order_plan();
        let report = DmwAnalyzer::analyze(&plan);
    ...
    #[test] fn report_problem_count_matches_bottlenecks() {
        let plan   = simple_nested_loop_plan();
        let report = DmwAnalyzer::analyze(&plan);
        assert_eq!(report.problem_count(), report.bottlenecks.len());
    }
    #[test] fn simple_plan_not_golden() {
        let plan   = simple_nested_loop_plan();
        let report = DmwAnalyzer::analyze(&plan);
        assert!(!report.is_golden(), "NestedLoop + TableScan cannot be Golden");
    }
}
