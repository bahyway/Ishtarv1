...
impl PlanNode {
    ...
    pub fn count_where<F: Fn(&PlanNode) -> bool>(&self, f: &F) -> usize {
        ...
        self_count
            + self
                .children
                .iter()
                .map(|c| c.count_where(f))
                .sum::<usize>()
    }

    /// Walk the tree and collect all nodes matching a predicate (depth-first).
    pub fn collect_where<'a, F: Fn(&PlanNode) -> bool>(
        &'a self,
        f: &F,
        out: &mut Vec<&'a PlanNode>,
    ) {
        if f(self) {
            out.push(self);
        }
        for child in &self.children {
            child.collect_where(f, out);
        }
    }
    ...
    pub fn highest_cost_node(&self) -> &PlanNode {
        ...
        for child in &self.children {
            ...
            if cand.cost_pct > best.cost_pct {
                best = cand;
            }
        }
        best
...
impl QueryPlan {
    ...
    pub fn with_parallelism(mut self, p: bool) -> Self {
        self.parallelism = p;
        self
    }
    pub fn with_actual_rows(mut self, r: u32) -> Self {
        self.actual_rows = r;
        self
    }
    ...
    pub fn join_count(&self) -> usize {
        self.root.count_where(&|n| {
            matches!(
                n.op,
                OpKind::HashMatch(_) | OpKind::NestedLoopJoin(_) | OpKind::MergeJoin(_)
            )
        })
    }
    ...
    pub fn nested_loop_count(&self) -> usize {
        self.root
            .count_where(&|n| matches!(n.op, OpKind::NestedLoopJoin(_)))
    }

    pub fn hash_match_count(&self) -> usize {
        self.root
            .count_where(&|n| matches!(n.op, OpKind::HashMatch(_)))
    }
    ...
    pub fn sort_count(&self) -> usize {
        self.root
            .count_where(&|n| matches!(n.op, OpKind::Sort { .. }))
    }
    ...
    pub fn has_key_lookups(&self) -> bool {
        self.key_lookup_count() > 0
    }
    pub fn has_table_scans(&self) -> bool {
        self.table_scan_count() > 0
    }
    pub fn has_sorts(&self) -> bool {
        self.sort_count() > 0
    }
    pub fn has_spools(&self) -> bool {
        self.spool_count() > 0
    }
    ...
    pub fn cardinality_ratio(&self) -> Option<f32> {
        if self.root.estimated_rows == 0 || self.actual_rows == 0 {
            return None;
        }
...
pub fn sales_order_plan() -> QueryPlan {
    ...
    let idx_phone = PlanNode::new(
        IndexScan {
            name: "IX_PersonPhone_PhoneNumber".into(),
            scan_type: NonClustered,
        },
        3.0,
        19972,
        291,
    ...
    let _idx_cl = PlanNode::new(
        IndexScan {
            name: "IX_Person_LastName".into(),
            scan_type: Clustered,
        },
        1.0,
        291,
    ...
    let hm_inner =
        PlanNode::new(HashMatch(Inner), 15.0, 291, 808).with_children(vec![cs2, cs3.clone()]);

    let cs4 = PlanNode::new(ComputeScalar, 0.0, 808, 0);
    let idx_country = PlanNode::new(
        IndexScan {
            name: "IX_PersonPhone_CountryRegion".into(),
            scan_type: NonClustered,
        },
        0.0,
        1,
    ...
    let hm_outer = PlanNode::new(HashMatch(LeftOuter), 18.0, 808, 808).with_children(vec![
        cs4,
        PlanNode::new(ComputeScalar, 0.0, 1, 0).with_children(vec![idx_country]),
    ]);

    let root = PlanNode::new(HashMatch(LeftOuter), 63.0, 6069, 19972).with_children(vec![
        PlanNode::new(ComputeScalar, 0.0, 6069, 0).with_children(vec![PlanNode::new(
            IndexSeek {
                name: "PK_BusinessEntity".into(),
                scan_type: Clustered,
            },
            0.0,
            6069,
    ...
    QueryPlan::new("SalesOrder Correlated Subquery", root, 100.0).with_actual_rows(6069)
}
...
pub fn simple_nested_loop_plan() -> QueryPlan {
    ...
    let root =
        PlanNode::new(NestedLoopJoin(Inner), 95.0, 100, 50_000).with_children(vec![outer, inner]);
...
mod tests {
    ...
    #[test]
    fn cardinality_ratio_none_when_zero() {
        let plan = QueryPlan::new(
            "empty",
            PlanNode::new(OpKind::ComputeScalar, 0.0, 0, 0),
            0.0,
        );
