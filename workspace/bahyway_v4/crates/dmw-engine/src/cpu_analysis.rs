...
impl CpuAnalyser {
    pub fn analyse(
        ...
        if has_scalar_udf {
            ...
            smells.push(CpuSmell {
                ...
                fix_description:      "Replace scalar UDF with inline table-valued \
                    ...
                    Scalar UDFs prevent parallelism.".into(),
        ...
        if has_cursor {
            ...
            smells.push(CpuSmell {
                ...
                fix_description:      "Replace cursor with set-based UPDATE/INSERT. \
                    Cursors operate one row at a time and cannot be parallelised.".into(),
                fix_sql:              "UPDATE target SET col = src.val\n\
                    FROM source src WHERE target.id = src.id".into(),
        ...
        if has_unnecessary_order_by {
            ...
            smells.push(CpuSmell {
                ...
                fix_description:      "Remove ORDER BY from inner queries / CTEs. \
                    Only sort at the final output when required by the application.".into(),
                fix_sql:              "-- Remove ORDER BY from CTE/subquery\n\
                    -- Add only to outermost SELECT if needed".into(),
        ...
        let reduction_pct = if total_ms > 0.0 { (total_savings / total_ms) * 100.0 } else { 0.0 };

        let can_parallelise = !has_scalar_udf && !has_cursor;

        let green: u8 = if reduction_pct > 80.0 { 50 } else if reduction_pct > 40.0 { 120 } else { 200 };
    ...
    pub fn color_contribution(analysis: &CpuAnalysis) -> LevelColorContribution {
        LevelColorContribution::new(
            ...
            &format!("{} CPU pattern(s) — {:.0}ms → {:.0}ms ({:.0}% reduction)",
                analysis.smells.len(),
...
mod tests {
    ...
    fn scalar_subquery_detected() {
        ...
        assert!(a.smells.iter().any(|s| s.pattern == CpuPattern::ScalarSubqueryInSelect));
    }

    #[test]
    fn scalar_subquery_executes_per_row() {
        let a = CpuAnalyser::demo_analysis(6069);
        let smell = a.smells.iter()
    ...
    fn cpu_reduction_over_90_percent() {
        let a = CpuAnalyser::demo_analysis(6069);
        assert!(a.cpu_reduction_pct > 90.0,
            "scalar subquery fix must save >90% CPU, got {:.1}%", a.cpu_reduction_pct);
    }
    ...
    fn cursor_has_highest_red_penalty() {
        assert!(CpuPattern::CursorOrWhileLoop.red_penalty()
    ...
    fn fix_sql_non_empty_for_all_smells() {
        ...
        for s in &a.smells { assert!(!s.fix_sql.is_empty()); }
    }

    #[test]
    fn akkadi_rule_non_empty_for_all_smells() {
        let a = CpuAnalyser::analyse(1000, true, true, true, true);
        for s in &a.smells { assert!(!s.akkadi_rule.is_empty()); }
    }
