...
impl FragmentationState {
    ...
    pub fn from_pct(pct: f32) -> Self {
        if      pct < 10.0  { FragmentationState::Healthy }
        else if pct < 20.0  { FragmentationState::Minor }
        else if pct < 30.0  { FragmentationState::Moderate }
        else if pct < 40.0  { FragmentationState::Severe }
        else                { FragmentationState::Critical }
    }
}
...
impl StorageFragment {
    ...
    pub fn is_critical(&self) -> bool { self.fragmentation_pct >= 30.0 }
...
impl FragmentationMap {
    ...
    pub fn total_indexes(&self) -> usize { self.fragments.len() }
    ...
    pub fn moderate_or_above_count(&self) -> usize {
        self.fragments.iter().filter(|f| f.fragmentation_pct >= 20.0).count()
    }

    pub fn rebuild_targets(&self) -> Vec<&StorageFragment> {
        self.fragments.iter().filter(|f| f.fragmentation_pct >= 30.0).collect()
    }
    ...
    pub fn worst_fragment(&self) -> Option<&StorageFragment> {
        self.fragments.iter().max_by(|a, b| a.fragmentation_pct.partial_cmp(&b.fragmentation_pct).unwrap())
    }

    pub fn healthy_count(&self) -> usize {
        self.fragments.iter().filter(|f| f.state() == FragmentationState::Healthy).count()
    }

    /// Overall storage health score: 100 − weighted_fragmentation (0–100).
    pub fn overall_health_score(&self) -> f32 {
        if self.fragments.is_empty() { return 100.0; }
        let total_pages: u32 = self.fragments.iter().map(|f| f.page_count).sum();
        if total_pages == 0 { return 100.0; }
    ...
    pub fn iter(&self) -> impl Iterator<Item = &StorageFragment> { self.fragments.iter() }
}
...
pub fn sales_order_frag_map() -> FragmentationMap {
    FragmentationMap::new(vec![
        ...
        StorageFragment::new("IX_PersonPhone_PhoneNumber", "Person.PersonPhone", 12.3, 450, 80),
        StorageFragment::new("IX_PersonPhone_CountryRegion", "Person.PersonPhone", 22.1, 110, 80),
        StorageFragment::new("PK_SalesOrder",          "Sales.SalesOrderHeader", 8.7,  900, 80),
        StorageFragment::new("IX_SalesOrder_CustomerID","Sales.SalesOrderHeader",31.0, 1800, 90),
        StorageFragment::new("IX_OrderDetail_ProductID","Sales.SalesOrderDetail", 7.1, 600, 80),
    ])
}
...
mod tests {
    ...
    #[test] fn sales_frag_has_seven_indexes() {
        assert_eq!(sales_order_frag_map().total_indexes(), 7);
    }
    #[test] fn critical_count_two() {
        // IX_Person_LastName (38.4%) + IX_SalesOrder_CustomerID (31.0%)
        assert_eq!(sales_order_frag_map().critical_count(), 2);
    }
    #[test] fn rebuild_targets_are_critical_indexes() {
        let map = sales_order_frag_map();
        for t in map.rebuild_targets() {
            assert!(t.fragmentation_pct >= 30.0);
        }
    }
    #[test] fn reorganize_targets_in_correct_range() {
        let map = sales_order_frag_map();
        for t in map.reorganize_targets() {
            assert!(t.fragmentation_pct >= 20.0 && t.fragmentation_pct < 30.0,
                "{} at {:.1}%", t.index_name, t.fragmentation_pct);
        }
    }
    #[test] fn worst_fragment_is_person_lastname() {
        let map = sales_order_frag_map();
        let worst = map.worst_fragment().unwrap();
        assert_eq!(worst.index_name, "IX_Person_LastName");
    }
    #[test] fn health_score_below_100_when_fragmented() {
        assert!(sales_order_frag_map().overall_health_score() < 100.0);
    }
    #[test] fn health_score_100_for_empty_map() {
        assert_eq!(FragmentationMap::new(vec![]).overall_health_score(), 100.0);
    }
    #[test] fn fragmentation_state_thresholds() {
        assert_eq!(FragmentationState::from_pct(5.0),  FragmentationState::Healthy);
        assert_eq!(FragmentationState::from_pct(15.0), FragmentationState::Minor);
        assert_eq!(FragmentationState::from_pct(25.0), FragmentationState::Moderate);
        assert_eq!(FragmentationState::from_pct(35.0), FragmentationState::Severe);
        assert_eq!(FragmentationState::from_pct(45.0), FragmentationState::Critical);
    }
    #[test] fn recommended_action_matches_state() {
    ...
    #[test] fn state_labels_non_empty() {
        let states = [
            FragmentationState::Healthy, FragmentationState::Minor,
            FragmentationState::Moderate, FragmentationState::Severe,
            FragmentationState::Critical,
        ];
        for s in &states { assert!(!s.label().is_empty()); }
    }
}
