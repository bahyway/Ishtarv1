...
impl ColorId {
    ...
    pub fn from_journey(plan: &QueryPlan, journey: &SevenLevelJourney) -> Self {
        ...
        Self { red, green, blue, level_bitmask: bitmask }
    }
    ...
    pub fn rgb_tuple(self) -> (u8, u8, u8) { (self.red, self.green, self.blue) }

    /// Returns the three colour bytes for KAKI PK embedding.
    pub fn as_bytes(self) -> [u8; 3] { [self.red, self.green, self.blue] }

    /// Number of Journey levels that passed (0–7). Only meaningful from `from_journey()`.
    pub fn passed_count(self) -> u32 { self.level_bitmask.count_ones() }

    /// `true` when all 7 Journey levels passed.
    pub fn all_passed(self) -> bool { self.level_bitmask == 0x7F }
...
impl std::fmt::Display for ColorId {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "RGB({}, {}, {}) [{}]", self.red, self.green, self.blue, self.status_label())
    }
}
...
impl ColorIdBuilder {
    pub fn new() -> Self { Self { contributions: Vec::new() } }
...
mod tests {
    ...
    fn simple_plan_not_all_passed() {
        let c = color_for(&simple_nested_loop_plan());
        assert!(!c.all_passed(), "simple plan cannot have all 7 gates passing");
    }
    ...
    fn perfect_plan_is_healthy() {
        ...
        assert!(c.is_healthy(), "red={} green={} blue={}", c.red, c.green, c.blue);
    }
    ...
    fn builder_merges_worst() {
        ...
        b.add(LevelColorContribution::new("L1", ColorId::new(50, 200, 200), "ok"));
        b.add(LevelColorContribution::new("L4", ColorId::new(220, 80, 30), "loop"));
        let r = b.build();
        assert_eq!(r.red, 220);
