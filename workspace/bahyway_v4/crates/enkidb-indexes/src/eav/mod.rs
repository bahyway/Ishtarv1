...
impl EavIndex {
    ...
    pub fn insert(&mut self, tribe_id: TribeId, attr_hash: u32, value: Vec<u8>, uuid_hash: u32) {
        let key = EavKey { tribe_id, attr_hash, value };
...
impl Default for EavIndex { fn default() -> Self { Self::new() } }
...
mod tests {
    ...
    fn unknown_attr_is_empty() {
        ...
        assert!(idx.lookup_exact(TribeId::from_u16(0x0001), 0xFFFF, b"x").is_empty());
    }
}
