...
pub fn detect(data: &[u8]) -> MediaType {
    if data.is_empty() {
        return MediaType::UnknownBinary;
    }
    ...
    if starts_with(data, &[0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C]) {
        return MediaType::SevenZip;
    }
    // RAR 5.x: Rar!\x1A\x07\x01\x00
    if starts_with(data, &[0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x01, 0x00]) {
        return MediaType::Rar5;
    }
    // RAR 4.x: Rar!\x1A\x07\x00
    if starts_with(data, &[0x52, 0x61, 0x72, 0x21, 0x1A, 0x07, 0x00]) {
        return MediaType::Rar4;
    }
    // GZIP / tar.gz: \x1F\x8B
    ...
    if data.len() > 32_774 && &data[32_769..32_774] == b"CD001" {
        return MediaType::Iso9660;
    }
    // Microsoft CAB: MSCF
    if starts_with(data, &[0x4D, 0x53, 0x43, 0x46]) {
        return MediaType::Cab;
    }
    ...
    // GIF: GIF8
    if starts_with(data, b"GIF8") {
        return MediaType::Gif;
    }
    // BMP: BM
    if starts_with(data, &[0x42, 0x4D]) {
    ...
    if starts_with(data, &[0x4D, 0x4D, 0x00, 0x2A]) {
        return MediaType::Tiff;
    }
    // ICO: \x00\x00\x01\x00
    ...
    if starts_with(data, &[0x00, 0x00, 0x00, 0x0C, 0x6A, 0x50, 0x20, 0x20]) {
        return MediaType::Jpeg2000;
    }
    // HDF5: \x89HDF\r\n\x1A\n
    if starts_with(data, &[0x89, 0x48, 0x44, 0x46, 0x0D, 0x0A, 0x1A, 0x0A]) {
        return MediaType::Hdf5;
    }
    if data.len() >= 2 && is_cbor_heuristic(data) {
    ...
    // LAS: LASF
    if starts_with(data, b"LASF") {
        return MediaType::Las;
    }
    ...
    if data.len() > 132 && &data[128..132] == b"DICM" {
        return MediaType::Dicom;
    }
    // NITF: NITF02
    if starts_with(data, b"NITF02") {
        return MediaType::Nitf;
    }
    // MrSID: msid (MrSID generation 3)
    if starts_with(data, b"msid") {
        return MediaType::MrSid;
    }
    // ECW: NCS Ecw v1 or ECWP: starts with 0x0155 (little-endian)
    if starts_with(data, &[0x01, 0x55]) || starts_with(data, b"ECWP") {
        return MediaType::Ecw;
    }
    ...
    if starts_with(data, b"ID3") || (data.len() >= 2 && data[0] == 0xFF && (data[1] & 0xE0) == 0xE0)
    {
...
fn detect_zip_based(data: &[u8]) -> MediaType {
    ...
    if fname == b"[Content_Types].xml" || fname == b"_rels/.rels" {
        ...
        if contains(search, b"xl/") || contains(search, b"xl\\") {
            return MediaType::Xlsx;
        }
        if contains(search, b"word/") || contains(search, b"word\\") {
    ...
    if fname == b"mimetype" {
        ...
        if contains(search, b"spreadsheet") {
            return MediaType::Ods;
        }
    }
    MediaType::Zip
}
...
fn detect_ole2(data: &[u8]) -> MediaType {
    ...
    if contains(search, b"W\x00o\x00r\x00k\x00b\x00o\x00o\x00k") {
        return MediaType::Xls;
    }
    // Word document stream: "W\x00o\x00r\x00d\x00D\x00o\x00c\x00u\x00m\x00e\x00n\x00t"
    if contains(search, b"W\x00o\x00r\x00d\x00D\x00o\x00c") {
...
fn detect_text(data: &[u8]) -> MediaType {
    ...
    if s.starts_with('[') || s.starts_with('{') {
        return detect_json_vs_ndjson(data);
    }
    // XML / HTML
    if s.starts_with('<') || s.starts_with("<?xml") {
        return MediaType::Xml;
    }
    // ENVI header: starts with "ENVI" as first token
    if s.starts_with("ENVI") {
        return MediaType::EnviHdr;
    }

    // CSV heuristic: lines contain commas or tabs, first line looks like a header
    if is_csv_heuristic(s) {
...
mod tests {
    ...
    fn detects_zip() {
        assert_eq!(
            detect(&[0x50, 0x4B, 0x03, 0x04, 0, 0, 0, 0]),
            MediaType::Zip
        );
    }
    ...
    fn detects_7zip() {
        assert_eq!(
            detect(&[0x37, 0x7A, 0xBC, 0xAF, 0x27, 0x1C, 0, 0]),
            MediaType::SevenZip
        );
    }
    ...
    fn detects_hdf5() {
        assert_eq!(
            detect(&[0x89, 0x48, 0x44, 0x46, 0x0D, 0x0A, 0x1A, 0x0A]),
            MediaType::Hdf5
        );
    }
