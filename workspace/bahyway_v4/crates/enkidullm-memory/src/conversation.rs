...
impl Turn {
    ...
    pub fn display(&self) -> String {
        ...
        let kaki = self.kaki.bytes()[..4]
            .iter()
            .map(|b| format!("{b:02x}"))
            .collect::<String>();
        format!(
            "[{role}|κ:{kaki}|ep:{:5}|tok:{:4}] {}",
            self.epoch,
            self.token_count,
...
impl ConversationParticle {
    pub fn from_turn(turn: Turn) -> Self {
        ...
        let attributes = vec![
            (attr_hash("role"), turn.role.as_str().to_string()),
            (attr_hash("session_id"), turn.session_id.to_string()),
            (attr_hash("turn_index"), turn.turn_index.to_string()),
            (attr_hash("epoch"), turn.epoch.to_string()),
            (attr_hash("token_count"), turn.token_count.to_string()),
            (attr_hash("content"), turn.content.clone()),
        ];
...
mod tests {
    ...
    fn token_count_heuristic() {
        ...
        assert!(
            turn.token_count >= 90 && turn.token_count <= 110,
            "token_count = {}",
            turn.token_count
        );
    }
