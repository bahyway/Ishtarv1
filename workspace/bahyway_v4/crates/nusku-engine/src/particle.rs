use crate::types::{KakiPK, Hepta, ZoneId, BodyType};
...
impl ParticleSignal {
    ...
    pub(crate) fn with_timestamp(
        ...
        ParticleSignal {
            kaki, zone, body_type, temperature, baseline,
            deviation, hepta, hps, timestamp_ms, frame_id,
        }
    }
}
    ...
    pub fn from_temperatures(
        ...
        let zones = ZoneId::ALL.map(|z| {
            ParticleSignal::with_timestamp(
                z, body_type, temperatures[z as usize], frame_id, timestamp_ms,
            )
        });
        let scan_id = derive_kaki(&zones[0].hepta, frame_id as u8, frame_id);
        BodyScan { scan_id, body_type, frame_id, zones, timestamp_ms }
    }
...
mod tests {
    use super::*;
    use crate::types::{ZoneId, BodyType};
    ...
    fn uniform_scan_is_perfectly_symmetric() {
        let scan = BodyScan::uniform(BodyType::Man, 0);
        assert!((scan.symmetry_score() - 1.0).abs() < 1e-4,
            "symmetry={}", scan.symmetry_score());
    }
