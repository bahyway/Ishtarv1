...
impl PipelineContext {
    ...
    pub fn timing_report(&self) -> String {
        self.timings.iter()
            .map(|t| format!("  [{:4}ms] {:30} budget={}ms {}",
                t.elapsed_ms, t.stage_name, t.budget_ms,
                if t.within_budget { "✓" } else { "⚠ EXCEEDED" }))
            .collect::<Vec<_>>()
            .join("\n")
    }
}
    ...
    fn nusku_alert_color_hex_for_all_levels() {
        for level in 0u8..=5 {
            let alert = NuskuAlert {
                frame_id: 0, alert_level: level, score: 0.0,
                message_en: String::new(), message_ar: String::new(),
                kaki_hex: String::new(), elapsed_ms: 0,
                sound_alarm: false, vibrate: false,
            };
