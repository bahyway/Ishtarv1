...
mod tests {
    ...
    fn unit_hepta() -> Hepta {
        Hepta { d1_identity: 1.0, d2_belonging: 1.0, d3_domain: 1.0,
                d4_quality: 1.0, d5_freshness: 1.0, d6_temporal: 1.0, d7_integrity: 1.0 }
    }
    ...
    fn different_heptas_produce_different_kakis() {
        let h1 = unit_hepta();
        let h2 = Hepta { d1_identity: 0.5, ..h1 };
    ...
    fn d1_hash_different_zones_differ() {
        ...
        assert!((a - b).abs() > 1e-5, "zone 0 and zone 1 should produce different d1");
    }
    ...
    fn d1_hash_in_range() {
        for z in 0u8..16 {
            ...
            assert!(v >= 0.0 && v <= 1.0, "d1_hash out of range for zone {z}: {v}");
        }
    }
}
