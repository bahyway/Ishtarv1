...
impl OhlcBar {
    ...
    pub fn wick_size(&self) -> i64 {
        (self.high - self.low) - self.body_size()
    }

    /// Real volume (raw_volume / 1000).
    pub fn real_volume(&self) -> f64 {
        self.raw_volume as f64 / 1000.0
    }
...
impl OhlcBarBuilder {
    ...
    fn update(&mut self, tick: &TickRecord) {
        if tick.raw_price > self.high {
            self.high = tick.raw_price;
        }
        if tick.raw_price < self.low {
            self.low = tick.raw_price;
        }
...
impl OhlcAggregator {
    ...
    pub fn completed_bars(&self) -> &[OhlcBar] {
        &self.completed
    }
