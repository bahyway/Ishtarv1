...
impl RiskReport {
    ...
    pub fn with_risk_free(returns: &[f64], confidence: f64, risk_free: f64) -> Self {
        ...
        let sharpe = if vol.abs() > 1e-15 {
            (mean - risk_free) / vol
        } else {
            0.0
        };
        let sortino = if downside_std.abs() > 1e-15 {
            (mean - risk_free) / downside_std
        } else {
            0.0
        };
        ...
        let calmar = if max_dd.abs() > 1e-15 {
            total / max_dd.abs()
        } else {
            0.0
        };
...
mod tests {
    ...
    fn returns() -> Vec<f64> {
        vec![
            0.02, -0.01, 0.03, -0.02, 0.01, -0.03, 0.04, -0.015, 0.02, 0.01,
        ]
    }
    ...
    fn all_negative_returns_high_var() {
        ...
        assert!(
            r.var > 0.0,
            "var should be positive for all-negative returns"
        );
    }
