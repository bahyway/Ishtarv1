...
impl ReturnSeries {
    ...
    pub fn from_prices_with_kind(
        ...
        Self { asset_hash, kind, returns, mean, std_dev }
    }
    ...
    pub fn log_returns(&self) -> &[f64] { &self.returns }
    ...
    pub fn max_return(&self) -> f64 {
        self.returns.iter().cloned().fold(f64::NEG_INFINITY, f64::max)
    }
    ...
    pub fn up_count(&self) -> usize { self.returns.iter().filter(|&&r| r > 0.0).count() }

    /// Negative return count.
    pub fn down_count(&self) -> usize { self.returns.iter().filter(|&&r| r < 0.0).count() }
    ...
    pub fn kind(&self) -> ReturnKind { self.kind }
}

fn compute_returns(reals: &[f64], kind: ReturnKind) -> Vec<f64> {
    if reals.len() < 2 { return Vec::new(); }
    reals.windows(2).map(|w| {
        let (prev, curr) = (w[0], w[1]);
        match kind {
            ReturnKind::Log    => if prev > 0.0 { (curr / prev).ln() } else { 0.0 },
            ReturnKind::Simple => if prev > 0.0 { (curr - prev) / prev } else { 0.0 },
        }
    }).collect()
}
...
mod tests {
    ...
    fn log_returns_sum_to_cumulative() {
        ...
        assert!((sum - expected).abs() < 1e-9, "sum={sum} expected={expected}");
    }
    ...
    fn simple_cumulative_return_correct() {
        ...
        assert!((cum - expected).abs() < 1e-9, "cum={cum} expected={expected}");
    }
