...
pub fn emit_late_quarantine(
    ...
    for p in affected {
        ...
        {
            Some(ik) => ik,
            None     => { report.invalid_kaki += 1; continue; }
        };
...
mod tests {
    ...
    fn valid_particle(minter: &KakiMinter) -> AffectedParticle {
        let ik = enkidb_kaki::IdentityKaki::try_from_kaki(
            minter.identity(KakiRole::Zikru)
        ).unwrap();
        AffectedParticle { kaki_bytes: *ik.bytes(), epoch: 1, color_rgb: [100, 200, 255] }
    }
    ...
    fn journal_entry_has_late_quarantine_cause() {
        ...
        let mut jnl = Journal::new(64);
        let p = AffectedParticle { kaki_bytes: *ik.bytes(), epoch: 1, color_rgb: [50, 100, 200] };

        emit_late_quarantine(&[p.clone()], &mut jnl, &minter, "test.zip");

        let history = jnl.read_particle_history(&ik);
        assert_eq!(history.len(), 1);
        assert_eq!(history[0].event_cause(), Some(EventCause::MusaruLateQuarantine));
