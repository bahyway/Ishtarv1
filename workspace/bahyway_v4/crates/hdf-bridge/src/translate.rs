...
use dfg_engine::{DfgEdge, DfgNode, DfgPlan, EdgeKind, EdgeMeta, FeatureAttrib, NodeId, NodeKind};
use hepta_spatial::{AnchorBlock, AttribValue, ChordType, HeptaFile, NodeBlock, SectorBlock};
...
fn attribs_from_sector(sector: &SectorBlock) -> Vec<FeatureAttrib> {
    ...
    if let Some(ref cl) = sector.chord_link {
        v.push(FeatureAttrib {
            domain: "routing".into(),
            key: "chord_link_target".into(),
            value: cl.target.label.clone(),
        });
        v.push(FeatureAttrib {
            domain: "routing".into(),
            key: "chord_link_type".into(),
            value: cl.link_type.clone(),
        });
    }
    v
}
...
fn attribs_from_node(node_block: &NodeBlock) -> Vec<FeatureAttrib> {
    let mut v: Vec<FeatureAttrib> = node_block
        .attribs
        .iter()
        .map(attrib_stmt_to_feature)
        .collect();
...
fn attrib_stmt_to_feature(a: &hepta_spatial::AttribStmt) -> FeatureAttrib {
    FeatureAttrib {
...
fn props_to_meta(p: &hepta_spatial::BeamProperties) -> EdgeMeta {
    EdgeMeta {
