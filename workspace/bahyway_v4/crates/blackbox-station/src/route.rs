...
impl BlackBoxStation {
    ...
    // Same too_many_arguments reasoning as enkiodb/odb_store.rs and
    // naramsin-bridge/lib.rs earlier -- each param is a distinct real
    // store/dep this routing decision needs, not something to bundle
    // into a params struct unilaterally.
...
mod tests {
    ...
    fn harmful_particle_routes_to_storage_sector_not_qdb() {
        ...
        let report =
            station.route_from_sdb(&sdb, &mut qdb, &mut sector, &mut jnl, &minter, tid, 900);
