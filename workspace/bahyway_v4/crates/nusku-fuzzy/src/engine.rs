...
use crate::membership::{
    LinguisticVar,
    var_torso_dev, var_head_dev, var_arm_dev,
    var_symmetry, var_torso_arm_inversion, var_state_output,
};
use crate::rules::{FuzzyRule, Connective, nusku_rule_base};
...
impl FuzzyEngine {
    ...
    pub fn infer(&self, scan: &BodyScan, symmetry_score: f32, inversion: f32) -> FuzzyVerdict {
        // ── Fuzzify inputs ─────────────────────────────────────────────────────
        let torso_dev = (scan.zone(ZoneId::Chest).deviation
                       + scan.zone(ZoneId::Abdomen).deviation) / 2.0;
        let head_dev  =  scan.zone(ZoneId::Head).deviation;
        let arm_dev   = (scan.zone(ZoneId::LUpperArm).deviation
                       + scan.zone(ZoneId::RUpperArm).deviation) / 2.0;
        ...
        input_memberships.insert("symmetry".into(),            self.symmetry_var.eval_all(symmetry_score));
        input_memberships.insert("torso_arm_inversion".into(), self.inversion_var.eval_all(inversion));
        ...
        for rule in &self.rules {
            ...
            let final_strength = if self.iraq_mode
                && rule.consequent.term == "ambiguous"
                && weighted > 0.4 {
            ...
            if final_strength > 0.01 {
                ...
                let entry = output_clips.entry(rule.consequent.term.clone()).or_insert(0.0);
        ...
        reasoning.push(format!("Symmetry={:.3}  Inversion={:.3}", symmetry_score, inversion));
        for (id, strength, desc) in rule_activations.iter().take(3) {
            reasoning.push(format!("Rule {:02}: strength={:.3} — {}", id, strength, desc));
        }
        reasoning.push(format!("Crisp={:.4} → {}", crisp_score, state.display()));

        FuzzyVerdict {
            state, crisp_score, rule_activations, input_memberships,
            reasoning, confidence, iraq_protocol: self.iraq_mode,
        }
    }
    ...
    fn fire_rule(
        ...
        let values: Vec<f32> = rule.antecedents.iter().map(|ant| {
            let mu = memberships.get(&ant.variable)
                .and_then(|terms| terms.iter().find(|(t, _)| t == &ant.term))
                .map(|(_, v)| *v)
                .unwrap_or(0.0);
            if ant.negated { 1.0 - mu } else { mu }
        }).collect();
...
mod tests {
    ...
    fn engine()      -> FuzzyEngine { FuzzyEngine::new(false) }
    fn iraq_engine() -> FuzzyEngine { FuzzyEngine::new(true)  }

    #[test]
    fn normal_scan_is_clear() {
        let scan   = BodyScan::uniform(BodyType::Man, 0);
        let result = engine().infer(&scan, 1.0, 0.0);
        assert_eq!(result.state, FuzzyState::Clear,
            "got {:?} crisp={:.4}", result.state, result.crisp_score);
    }
    ...
    fn security_threat_pattern_detected() {
        ...
        assert_eq!(result.state, FuzzyState::SecurityThreat,
            "got {:?} crisp={:.4}", result.state, result.crisp_score);
    ...
    fn fever_pattern_detected() {
        ...
        assert_eq!(result.state, FuzzyState::Fever,
            "got {:?} crisp={:.4}", result.state, result.crisp_score);
    }
    ...
    fn stress_pattern_detected() {
        ...
        let result = engine().infer(&scan, 1.0, 0.0);
        assert_eq!(result.state, FuzzyState::Stress,
            "got {:?} crisp={:.4}", result.state, result.crisp_score);
    }
    ...
    fn display_and_arabic_non_empty_for_all_states() {
        let states = [
            FuzzyState::Clear, FuzzyState::Fever, FuzzyState::Stress,
            FuzzyState::MedicalOther, FuzzyState::Ambiguous, FuzzyState::SecurityThreat,
        ];
        for s in &states {
