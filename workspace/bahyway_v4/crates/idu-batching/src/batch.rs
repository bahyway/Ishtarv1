    ...
    pub fn probe_batch(&self, requests: &[LinkProbeRequest]) -> BatchResult {
        ...
        for req in requests {
            for anchor in &req.anchors {
                unique.insert(*anchor.bytes(), *anchor);
            }
        }
        ...
        let results: Vec<LinkResult> = requests
            .iter()
            .map(|req| {
                let states: Vec<ParticleState> = req
                    .anchors
                    .iter()
                    .map(|a| {
                        *anchor_states
...
mod tests {
    ...
    fn push_state(jnl: &mut Journal, m: &KakiMinter, p: &IdentityKaki, state: ParticleState) {
        ...
        jnl.append(JournalEntry::new(ek, p.clone(), 1, eav))
            .unwrap();
    }
    ...
    fn shared_anchor_projected_once() {
        ...
        let requests: Vec<LinkProbeRequest> = (0..3)
            .map(|i| LinkProbeRequest {
                link_id: i,
                anchors: vec![p1.clone(), p2.clone()],
            })
            .collect();
        ...
        assert!(res
            .results
            .iter()
            .all(|r| r.link_state == CrossTribeLinkState::Gold));
    }
    ...
    fn batch_result_reflects_dead_anchor() {
        ...
        let batch = BatchProbe::new(&jnl, &[]);
        let res = batch.probe_batch(&[LinkProbeRequest {
            link_id: 0,
            anchors: vec![p1, p2],
        }]);
