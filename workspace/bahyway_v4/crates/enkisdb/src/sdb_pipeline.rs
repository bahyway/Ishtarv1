...
use enkidw::{
    LandingZone, LandingFileKind,
    zip_extract,
    kaki_generator::{parse_csv, parse_tsv, generate, RawRecord},
};
...
pub enum SdbAlertSeverity { Info, Warning, Error }
...
impl SdbAlert {
    fn info(msg: impl Into<String>)    -> Self { SdbAlert { severity: SdbAlertSeverity::Info,    message: msg.into() } }
    fn warning(msg: impl Into<String>) -> Self { SdbAlert { severity: SdbAlertSeverity::Warning, message: msg.into() } }
    fn error(msg: impl Into<String>)   -> Self { SdbAlert { severity: SdbAlertSeverity::Error,   message: msg.into() } }
}
...
impl SdbPipeline {
    ...
    pub fn run_once(&mut self, store: &mut SdbStore, journal: &mut Journal) -> &SdbPipelineStats {
        ...
        self.emit(SdbAlert::info(format!("{} file(s) arrived in SDB landing zone", new_files.len())));
        ...
        for lf in new_files {
            ...
            let archived_path = match self.landing.archive(&lf) {
                Ok(p)  => p,
                Err(e) => {
                    self.emit(SdbAlert::error(format!("{fname}: failed to archive source — {e}")));
                    continue;
                }
            };
            ...
            match lf.kind {
                ...
                LandingFileKind::Csv => self.process_records(&archived_path, &fname, true,  store, journal),
                LandingFileKind::Tsv => self.process_records(&archived_path, &fname, false, store, journal),
                LandingFileKind::Way | LandingFileKind::Other => {
                    self.emit(SdbAlert::warning(format!("{fname}: SDB does not process WAY/other files")));
                }
            }
        }
    ...
    fn process_zip(&mut self, path: &Path, fname: &str, store: &mut SdbStore, journal: &mut Journal) {
        let data = match fs::read(path) {
            Ok(d)  => d,
            Err(e) => { self.emit(SdbAlert::error(format!("{fname}: read error — {e}"))); return; }
        };

        let malware_in_zip = zip_scan(&data).malware_detected;
        if malware_in_zip {
            self.stats.malware_hits += 1;
            self.emit(SdbAlert::error(format!("{fname}: Musarû pre-scan flagged malware signature")));
        }

        self.stats.zips_processed += 1;
        let entries = zip_extract(&data);
        self.emit(SdbAlert::info(format!("{fname}: extracted {} ZIP entries", entries.len())));
            ...
            match result {
                Ok(entry) => {
                    ...
                    if is_csv || is_tsv {
                        ...
                        self.emit(SdbAlert::warning(format!("{}/{}: skipped (not CSV/TSV)", fname, entry.name)));
                    }
                }
                Err(e) => {
    ...
    fn process_records(&mut self, path: &Path, fname: &str, is_csv: bool, store: &mut SdbStore, journal: &mut Journal) {
        let data = match fs::read(path) {
            Ok(d)  => d,
            Err(e) => { self.emit(SdbAlert::error(format!("{fname}: read error — {e}"))); return; }
        };
        let records = if is_csv { parse_csv(&data) } else { parse_tsv(&data) };
    ...
    fn stage_records(
        ...
        for record in records {
            ...
            let event_kaki = match EventKaki::try_from_kaki(self.minter.event(KakiRole::Zikru)) {
                Ok(ek) => ek,
                Err(_) => { self.stats.records_skipped += 1; continue; }
            };
            ...
            let entry = JournalEntry::new(event_kaki, ge.particle.clone(), ge.epoch, ge.eav.clone())
        ...
        if ok > 0 {
            self.emit(SdbAlert::info(format!("{source}: staged {ok}/{count} particles")));
        }
    }
    ...
    pub fn take_alerts(&mut self) -> Vec<SdbAlert>  { std::mem::take(&mut self.alerts) }
    pub fn alerts(&self)          -> &[SdbAlert]     { &self.alerts }
    pub fn stats(&self)           -> &SdbPipelineStats { &self.stats }
}
...
mod tests {
    ...
    fn no_duplicate_staging_after_restart() {
        ...
        assert_eq!(pipe2.stats().records_staged, 0, "restart must not re-stage an already-archived file");
