...
use crate::kaki_generator::{fnv1a_32, RawRecord};
use media_type_detector::{detect as detect_media_type, MediaCategory, MediaType};
...
pub fn records_from_entry(name: &str, data: &[u8]) -> Vec<RawRecord> {
    ...
    match mt {
        ...
        _ if mt.category() == MediaCategory::Archive => {
            eprintln!(
                "[record_router] WARN: nested archive detected inside entry '{}' ({}), \
...
pub fn parse_json_records(data: &[u8]) -> Vec<RawRecord> {
    ...
    if s.starts_with('[') {
        ...
        // Single object
        parse_json_object(s).map(|r| vec![r]).unwrap_or_default()
    } else {
        Vec::new()
    }
}
...
fn extract_json_pairs(s: &str) -> Vec<(String, String)> {
    ...
    loop {
        ...
        let Some(key) = read_json_string(bytes, &mut pos) else {
            break;
        };
        ...
        // Read value
        let Some(val) = read_json_value(bytes, &mut pos) else {
            break;
        };
        pairs.push((key, val));
    }
...
fn extract_xml_children(segment: &str) -> Vec<(String, String)> {
    ...
    while i < bytes.len() {
        if bytes[i] == b'<' {
            ...
            let tag = std::str::from_utf8(&bytes[name_start..i])
                .unwrap_or("")
                .to_string();
...
mod tests {
    ...
    fn json_array_routes_correctly() {
        let data =
            b"[{\"name\":\"Ali\",\"city\":\"Baghdad\"},{\"name\":\"Fatima\",\"city\":\"Basra\"}]";
