...
fn pseudo_timeout(node_id: u64, term: u64) -> u64 {
    let seed = node_id.wrapping_mul(0x9e3779b97f4a7c15).wrapping_add(term.wrapping_mul(6364136223846793005));
...
impl RaftNode {
    ...
    pub fn commit_index(&self) -> usize { self.commit_index }
    ...
    pub fn handle(&mut self, from: u64, msg: RaftMessage) -> Vec<RaftEffect> {
        ...
        match msg {
            RaftMessage::Vote { term, candidate_id, last_log_index, last_log_term } => {
                self.handle_vote(from, term, candidate_id, last_log_index, last_log_term, &mut effects);
            }
            RaftMessage::VoteOk { term, vote_granted } => {
                self.handle_vote_ok(term, vote_granted, &mut effects);
            }
            RaftMessage::Append { term, leader_id, prev_log_index, prev_log_term, entries, leader_commit } => {
                self.handle_append(from, term, leader_id, prev_log_index, prev_log_term, entries, leader_commit, &mut effects);
            }
            RaftMessage::AppendOk { term: _, match_index } => {
                self.handle_append_ok(from, match_index, &mut effects);
            }
            RaftMessage::AppendFail { term: _ } => {
                self.handle_append_fail(from);
            }
            RaftMessage::Heartbeat { term, leader_id, leader_commit } => {
    ...
    pub fn propose(&mut self, command: RaftEav) -> Option<Vec<RaftEffect>> {
        ...
        let mut effects = vec![
            RaftEffect::WriteWal { entries: vec![entry] },
        ];
    ...
    fn handle_append(
        ...
        if !LogView(&self.log).matches(prev_log_index, prev_log_term) {
            effects.push(RaftEffect::Send {
                to:  from,
                msg: RaftMessage::AppendFail { term: self.current_term },
            });
            return;
        }
        ...
        if !entries.is_empty() {
            effects.push(RaftEffect::WriteWal { entries: entries.clone() });
    ...
    fn try_advance_commit(&mut self, _effects: &mut Vec<RaftEffect>) {
        ...
        for n in ((self.commit_index + 1)..=self.log.len()).rev() {
            if self.log.get(n - 1).map_or(false, |e| e.term == self.current_term) {
    ...
    fn replicate_to_peers(&mut self, effects: &mut Vec<RaftEffect>) {
        for (i, &peer) in self.peers.iter().enumerate() {
            ...
            let prev_term  = if prev_index == 0 { 0 } else {
    ...
    fn advance_state_machine(&mut self, effects: &mut Vec<RaftEffect>) {
        if self.last_applied < self.commit_index {
            ...
            let apply_entries = self.log
                .get(self.last_applied..new_applied)
                .unwrap_or(&[])
                .to_vec();
            if !apply_entries.is_empty() {
                effects.push(RaftEffect::Apply { entries: apply_entries });
            }
...
mod tests {
    ...
    fn election_timeout_triggers_candidacy() {
        ...
        assert!(effects.iter().any(|e| matches!(e, RaftEffect::Broadcast { .. })));
    }
    ...
    fn vote_granted_when_log_ok() {
        ...
        let effects = node.handle(2, RaftMessage::Vote {
            term: 1, candidate_id: 2, last_log_index: 0, last_log_term: 0,
        });
        assert!(effects.iter().any(|e| {
            matches!(e, RaftEffect::Send { msg: RaftMessage::VoteOk { vote_granted: true, .. }, .. })
        }));
    }
    ...
    fn becomes_leader_after_quorum_votes() {
        ...
        let effects = node.handle(2, RaftMessage::VoteOk { term: 1, vote_granted: true });
        assert_eq!(node.role, Role::Leader);
        // Should send heartbeats immediately.
        assert!(effects.iter().any(|e| matches!(e, RaftEffect::Send { msg: RaftMessage::Heartbeat { .. }, .. })));
    }

    #[test]
    fn follower_advances_commit_on_heartbeat() {
        let mut follower = RaftNode::new(2, vec![1, 3]);
        let effects = follower.handle(1, RaftMessage::Heartbeat {
            term: 1, leader_id: 1, leader_commit: 0,
        });
    ...
    fn leader_can_propose_entry() {
        ...
        node.handle(2, RaftMessage::VoteOk { term: 1, vote_granted: true });
        assert_eq!(node.role, Role::Leader);

        let effects = node.propose(eav(99)).expect("must be leader");
        assert!(effects.iter().any(|e| matches!(e, RaftEffect::WriteWal { .. })));
    ...
    fn append_rejected_when_prev_log_mismatch() {
        ...
        let effects = follower.handle(1, RaftMessage::Append {
            term:           1,
            leader_id:      1,
            prev_log_index: 5,
            prev_log_term:  1,
        ...
        assert!(effects.iter().any(|e| matches!(e, RaftEffect::Send { msg: RaftMessage::AppendFail { .. }, .. })));
    }
    ...
    fn higher_term_in_message_demotes_leader() {
        ...
        node.handle(3, RaftMessage::Heartbeat { term: 5, leader_id: 3, leader_commit: 0 });
        assert_eq!(node.role, Role::Follower);
