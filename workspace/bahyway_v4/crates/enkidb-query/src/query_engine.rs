...
use heptascript::{execute, parse_query, QueryResult};
...
pub fn query(db: &EnkiDb, hepta_source: &str) -> Result<QueryResult, BahywayError> {
    let q = parse_query(hepta_source).map_err(|e| BahywayError::Internal(e.to_string()))?;
...
mod tests {
    use super::*;
    use akkvalue::{codec::encode as codec_encode, AkkValue};
