...
impl AkkadianRoot {
    ...
    pub fn sargon() -> Self {
        Self::new('Š', 'R', 'K', VowelPattern::Ui)
    }
    ...
    pub fn from_phrase(phrase: &str) -> KupruResult<Self> {
        ...
        let consonants: Vec<char> = normalized
            .chars()
        ...
        let vowels: Vec<char> = normalized
            .chars()
            .filter(|c| matches!(c, 'a' | 'e' | 'i' | 'o' | 'u'))
            .collect();
    ...
    pub fn to_key_material(&self) -> [u8; 32] {
        ...
        if let Some(p) = &self.affixes.prefix {
            h.update(p.as_bytes());
        }
        if let Some(s) = &self.affixes.suffix {
            h.update(s.as_bytes());
        }
        h.finalize().into()
    }
...
impl LinguisticProof {
    pub fn create(phrase: &str) -> KupruResult<Self> {
        ...
        Ok(Self {
            ...
            commitment,
            nonce,
            mudu: Self::score_morphology(phrase),
        })
    }
    ...
    fn score_morphology(phrase: &str) -> u8 {
        ...
        if p.contains("um") || p.contains("im") || p.contains("am") {
            score += 1;
        }
        if p.len() > 16 {
            score += 1;
