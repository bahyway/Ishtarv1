...
impl PersistedDb {
    ...
    pub fn commit(
        ...
        let entry = JournalEntry::new(event_kaki.clone(), target.clone(), epoch, eav.clone());
    ...
    pub fn db(&self)     -> &EnkiDb     { &self.db }
    pub fn db_mut(&mut self) -> &mut EnkiDb { &mut self.db }
    pub fn stats(&self)  -> &PersistStats { &self.stats }
    pub fn tribe_id(&self) -> TribeId    { self.db.tribe_id() }
}
