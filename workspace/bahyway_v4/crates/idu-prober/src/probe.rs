...
impl<'a> IduProbe<'a> {
    ...
    pub fn probe(&self, anchors: &[IdentityKaki]) -> IduProbeResult {
        ...
        let anchor_states: Vec<_> = anchors.iter()
            .map(|a| engine.project(a).state)
            .collect();
        let projections_run = anchor_states.len();
        let link_state = compose_n_anchors(&anchor_states);
        IduProbeResult { link_state, anchor_states, projections_run }
    }
...
mod tests {
    ...
    fn push_event(jnl: &mut Journal, m: &KakiMinter, target: &IdentityKaki, state: ParticleState, epoch: u32) {
    ...
    fn time_travel_probe() {
        ...
        assert_eq!(probe.probe_at(&[p1.clone(), p2.clone()], 3).link_state, LinkState::Gold);
