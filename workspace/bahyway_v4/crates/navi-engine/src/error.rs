...
mod tests {
    ...
    #[test] fn route_not_found_display()      { assert!(NaviError::RouteNotFound.to_string().contains("route")); }
    #[test] fn map_not_loaded_display()       { assert!(NaviError::MapNotLoaded.to_string().contains("map")); }
    #[test] fn spoof_detected_display()       { assert!(NaviError::SpoofDetected.to_string().contains("spoof")); }
    #[test] fn invalid_particle_display()     { assert!(NaviError::InvalidParticle("bad crc".into()).to_string().contains("bad crc")); }
    #[test] fn sovereign_violation_display()  { assert!(NaviError::SovereignViolation("tribe 0x0099".into()).to_string().contains("sovereignty")); }
    #[test] fn map_parse_error_display()      { assert!(NaviError::MapParseError("line 7".into()).to_string().contains("map parse")); }
    #[test] fn empty_graph_display()          { assert!(NaviError::EmptyGraph.to_string().contains("empty")); }
    #[test] fn error_is_clone()               { assert_eq!(NaviError::RouteNotFound.clone(), NaviError::RouteNotFound); }
    #[test] fn error_is_partial_eq()          { assert_eq!(NaviError::MapNotLoaded, NaviError::MapNotLoaded); assert_ne!(NaviError::MapNotLoaded, NaviError::EmptyGraph); }
}
