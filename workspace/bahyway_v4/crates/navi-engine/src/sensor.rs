...
pub enum SensorEvent {
    ...
    WeatherChange { node_id: NaviNodeId, surface: SurfaceQuality },
        ...
        match event {
            SensorEvent::Congestion { node_id, level } => {
                let n = graph.node_mut(node_id)
                    .ok_or_else(|| NaviError::InvalidParticle(format!("node {node_id} not found")))?;
            ...
            SensorEvent::SovereignAlert { tribe } => {
                ...
                let affected: Vec<NaviNodeId> = graph
                    .all_node_ids()
                    .into_iter()
                    .filter(|&id| graph.node(id).map(|n| n.particle.tribe == tribe).unwrap_or(false))
                    .collect();
                for id in affected {
...
mod tests {
    ...
    fn graph() -> NaviGraph { NaviGraph::from_navimap(&seven_node_map()).unwrap() }

    #[test]
    fn congestion_event_updates_node() {
        let mut g = graph();
        SensorFeed::apply(&mut g, SensorEvent::Congestion { node_id: 2, level: 200 }).unwrap();
    ...
    fn weather_change_updates_surface() {
        ...
        SensorFeed::apply(&mut g, SensorEvent::WeatherChange {
            node_id: 4, surface: SurfaceQuality::Poor
        }).unwrap();
    ...
    fn speed_change_updates_node() {
        let mut g = graph();
        SensorFeed::apply(&mut g, SensorEvent::SpeedChange { node_id: 5, speed_kmh: 30 }).unwrap();
    ...
    fn event_on_nonexistent_node_errors() {
        let mut g = graph();
        let r = SensorFeed::apply(&mut g, SensorEvent::Congestion { node_id: 99, level: 50 });
        assert!(r.is_err());
    }
    ...
    fn sensor_event_names_non_empty() {
        let events = [
            SensorEvent::Congestion    { node_id: 1, level: 0 },
            SensorEvent::RoadClosure   { node_id: 1 },
            SensorEvent::RoadOpen      { node_id: 1 },
            SensorEvent::WeatherChange { node_id: 1, surface: SurfaceQuality::Good },
            SensorEvent::SovereignAlert { tribe: TribeId::from_u16(0x0001) },
            SensorEvent::SpeedChange   { node_id: 1, speed_kmh: 60 },
        ];
        for e in &events {
