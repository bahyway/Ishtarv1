...
impl NaviMap {
    ...
    pub fn add_beam(&mut self, beam: MapBeam) {
    ...
    pub fn parse(src: &str) -> NaviResult<Self> {
        ...
        for (line_no, raw) in src.lines().enumerate() {
            ...
            match parts.first().copied() {
                Some("NODE") => {
                    if parts.len() < 7 {
                        return Err(NaviError::MapParseError(format!(
                            "line {}: NODE needs 6 fields",
                            line_no + 1
                        )));
                    }
                    let id = parts[1].parse::<u32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad node id", line_no + 1))
                    })?;
                    let lat = parts[2].parse::<f32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad lat", line_no + 1))
                    })?;
                    let lon = parts[3].parse::<f32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad lon", line_no + 1))
                    })?;
                    let alt = parts[4].parse::<f32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad alt", line_no + 1))
                    })?;
                    let sector = parts[5].parse::<u8>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad sector", line_no + 1))
                    })?;
                    let tribe = u16::from_str_radix(parts[6].trim_start_matches("0x"), 16)
                        .map_err(|_| {
                            NaviError::MapParseError(format!("line {}: bad tribe hex", line_no + 1))
                        })?;
                    if sector > 6 {
                        return Err(NaviError::MapParseError(format!(
                            "line {}: sector must be 0-6",
                            line_no + 1
                        )));
                    }
                    map.add_node(MapNode {
                        id,
                        lat,
                        lon,
                        alt,
                ...
                Some("BEAM") => {
                    if parts.len() < 4 {
                        return Err(NaviError::MapParseError(format!(
                            "line {}: BEAM needs from, to, distance",
                            line_no + 1
                        )));
                    }
                    let from = parts[1].parse::<u32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad beam from", line_no + 1))
                    })?;
                    let to = parts[2].parse::<u32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad beam to", line_no + 1))
                    })?;
                    let distance = parts[3].parse::<f32>().map_err(|_| {
                        NaviError::MapParseError(format!("line {}: bad distance", line_no + 1))
                    })?;
                    let one_way = parts.get(4).copied() == Some("ONE_WAY");
                    map.add_beam(MapBeam {
                        from,
                        to,
                        distance,
                        one_way,
