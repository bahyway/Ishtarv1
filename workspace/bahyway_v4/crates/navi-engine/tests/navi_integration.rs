...
use navi_engine::{
    NaviGraph, NaviMap, NaviCommand, NaviCodeExecutor, RouteConstraints,
    RouteRequest, RouteEngine, SensorEvent, SensorFeed, SurfaceQuality,
    seven_node_map, haversine_m,
};
use bahyway_core::TribeId;

fn najaf_tribe()  -> TribeId { TribeId::from_u16(0x0001) }
fn build_graph()  -> NaviGraph { NaviGraph::from_navimap(&seven_node_map()).unwrap() }
...
fn pilgrimage_centre_to_all_sectors() {
    ...
    for dest in 2u32..=7 {
        let plan = e.plan_default(1, dest)
...
fn road_closure_forces_alternate_path() {
    ...
    assert!(!plan.waypoints.contains(&3), "closed node must not appear in route");
}
...
fn congestion_increases_cost_but_route_still_found() {
    ...
    SensorFeed::apply(&mut g, SensorEvent::Congestion { node_id: 2, level: 255 }).unwrap();
    let e    = RouteEngine::new(&g);
    let plan = e.plan_default(1, 2).expect("congestion must not block routing");
    assert!(plan.is_valid());
}
...
fn sovereign_alert_blocks_tribe_but_leaves_others_passable() {
    ...
    for id in 1u32..=7 { assert!(g.node(id).unwrap().is_passable()); }
}
...
fn poor_surface_on_all_nodes_still_routes() {
    let mut g = build_graph();
    for id in 1u32..=7 {
        SensorFeed::apply(&mut g, SensorEvent::WeatherChange {
            node_id: id, surface: SurfaceQuality::Poor,
        }).unwrap();
    }
    let e = RouteEngine::new(&g);
    let plan = e.plan_default(1, 5).expect("must route despite poor surface");
    assert!(plan.is_valid());
...
fn wadi_al_salam_seven_sector_full_scan() {
    ...
    for &zone in &zones {
        match e.plan_default(1, zone) {
            ...
            Err(e) => { all_valid = false; eprintln!("zone {zone} failed: {e}"); }
        }
    }
    assert!(all_valid, "all 6 cemetery zones must be reachable from entry");
...
fn haversine_wadi_al_salam_north_south_distance() {
    ...
    // ~2.5 km expected
    assert!(d > 2_000.0 && d < 3_500.0, "N-S distance should be ~2.5 km, got {d}");
}
...
fn strict_constraints_still_find_route() {
    let g = build_graph();
    let e = RouteEngine::new(&g);
    let c = RouteConstraints { max_risk: 0, avoid_restricted: true, prefer_quality: true };
    let plan = e.plan(1, 5, c).expect("strict constraints must not break routing");
    assert!(plan.is_valid());
}
