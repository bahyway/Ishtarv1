...
use enkidb_indexes::{
    colorid::ColorIdIndex, eav::EavIndex, identity::IdentityIndex, snapshot_idx::SnapshotIndex,
    sovereignty::SovereigntyIndex, temporal::TemporalIndex, typerole::TypeRoleIndex,
};
use enkidb_journal::{
...
impl EnkiDb {
    ...
    pub fn register_particle(&mut self, particle: &IdentityKaki) -> Result<()> {
        ...
        self.idx_sovereignty
            .insert(particle.tribe_id(), particle.uuid_hash());
    ...
    pub fn append_event(
        ...
        self.journal
            .append(JournalEntry::new(event_kaki, target, epoch, eav))?;
        Ok(())
    }
...
mod tests {
    ...
    fn time_travel_via_engine() {
        ...
        db.append_event(
            ...
            vec![EavTriple::new(
                ATTR_STATE,
                encode_state(ParticleState::Dead).to_vec(),
            )],
        )
        .unwrap();
