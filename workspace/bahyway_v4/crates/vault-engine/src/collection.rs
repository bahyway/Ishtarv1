...
impl Collection {
    pub fn from_files(id: String, name: String, path: PathBuf, files: Vec<FileRecord>) -> Self {
        ...
        Self { id, name, path, files, total_bytes, file_count, zero_byte_cnt, avg_age_days, dominant_ext }
    }
}
...
impl CollectionSet {
    pub fn total_files(&self) -> u64  { self.collections.iter().map(|c| c.file_count).sum() }
    pub fn total_bytes(&self) -> u64  { self.collections.iter().map(|c| c.total_bytes).sum() }
...
pub fn build_collections(root: &std::path::Path, files: Vec<FileRecord>) -> CollectionSet {
    ...
    let mut collections: Vec<Collection> = groups.into_iter()
        .enumerate()
        .map(|(i, (path, files))| {
    ...
    collections.sort_by(|a, b| b.file_count.cmp(&a.file_count));

    CollectionSet { collections, root: root.to_path_buf() }
}
...
mod tests {
    ...
    fn make_temp_dir() -> PathBuf {
        ...
        p.push(format!("vault_col_test_{}", std::time::SystemTime::now()
    ...
    fn collections_sorted_by_file_count_descending() {
        ...
        for i in 0..5 { std::fs::write(big.join(format!("f{}.txt", i)), b"x").unwrap(); }
        std::fs::write(small.join("only.txt"), b"y").unwrap();

        let (files, _) = walk_folder(&dir);
        let set = build_collections(&dir, files);
        assert_eq!(set.len(), 2);
        assert!(set.collections[0].file_count >= set.collections[1].file_count,
