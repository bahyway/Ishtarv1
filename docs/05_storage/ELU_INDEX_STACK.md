# The Elu Index Stack — W5H2 Reference
...
**Amendment, 2026-08-15 (second rename, Anu → Elu):** the Architect
reclaimed "Anu" to name the AnuGovernor (formerly Shakkanakku), so this
index stack's sovereign name moves again — to **Elu** (elû, Akkadian
"high, upper, superior"), per NL-001's Orthography Clause (§6a: plain
Latin "Elu" in identifiers, the diacritic "elû" reserved for prose). The
"Why Anu, not ENLIL" history below is preserved unrewritten (Concordance
Doctrine, Clause U-3): it explains why ENLIL was rejected, which still
holds; only the final chosen name changed a second time, for an
unrelated reason (freeing "Anu" elsewhere, not a defect in "Anu" itself).
`enkidb_indexes::SOVEREIGN_NAME` now returns `"elu"`, was `"anu"`.

## Why "Anu," not "ENLIL" (original naming, 2026-07-13 — see amendment above for the 2026-08-15 rename to "Elu")
...
`enkidb_indexes::SOVEREIGN_NAME` returned `"anu"` at the time (now `"elu"`
— see the 2026-08-15 amendment above). Nothing about the crate's build,
its dependents, or its public API changed to make either name true.
...
Each Elu index accelerates one or more of these dimensions by turning an
