...
playbook tracked in `docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md`, in the order the
...
1. Reads `docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md` directly — parses its own
...
- A range (`A..B`) always expands **ascending**, filtered to numbers that
  actually exist as real rows in `docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md` —
...
speculatively. This is the same discipline `docs/08_pipeline_alaktu/TRANSPARENCY_STANDARD.md`
...
## 13. AnuGovernor — the interactive governor (PB-263, `crates/anu-governor`)
...
**AnuGovernor** (`playbook_263_deploy_anu_governor.yml`, crate at
`crates/anu-governor`) is the answer — an eframe/egui GUI (or headless CLI,
...
**Same source of truth, not a second one.** AnuGovernor's decree
(`anu-governor.toml`, repo root) sets `triage_doc =
"docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md"` — at load time it parses this file's
own tables (`crates/anu-governor::config::playbooks_from_triage`, logic
...
PB-263 — "two competing sources of truth" — closed by having AnuGovernor
...
per-playbook granularity AnuGovernor's halt-and-diagnose UI needs).

**Chronicle, not just a report.** Every run also appends an immutable
JSONL event log (`docs/SHEDU/anu_governor_chronicle/`, gitignored like
...
is not automated by AnuGovernor itself, same "sealing is a separate,
explicit act" principle as §12.

**Known limits, honestly scoped:**
- The seal key (`secrets/anu_governor_seal.key`, gitignored) is a bare
  ...
  AnuGovernor has no scriptable path into that vault yet. Treat this key
...
- `simulate = true` is the checked-in decree's default — an Architect
  flips it to `false` deliberately before AnuGovernor touches real
  infrastructure. It never defaults to live execution.

**A third face: the browser.** The native egui window (`anu-governor`)
...
it, `anu-governor-web` (same `--bin`, `web` feature) puts the identical
engine — `config`/`runner`/`remedy`/`report`/`chronicle`, byte-for-byte
unchanged, now `pub` in a real `crates/anu-governor` lib target both faces
link against — behind a small hand-rolled HTTP/1.1 server
(`src/bin/anu_governor_web.rs`, `std::net` only, no async runtime: one
...
Run `anu-governor-web` from the repo root (same relative-path rule as the
...
`crates/anu-governor/src/{app,main}.rs`), not a one-off; the web face is the
recommended default on eriduous-vdi and similar VDI environments.

**Real auth, not a label — `crates/anu-governor/src/web_auth.rs` +
...
- **TLS** (`web_tls.rs`): `rustls` with the `ring` crypto provider
  ...
  to `secrets/anu_governor_web_{cert,key}.pem` (gitignored, `0600`, same
