# GL-VIZ-009 — The Nasaru Default Visualizing Style
## One Interaction Grammar for Every Šala Prototype

**Ecosystem:** BahyWay.Ecosystem v4.0 — visualization standard
**Consumes:** GL-STY-001 (StoryWay), GL-BRT-001 (Birth Gate / Two-Branch), GL-ONT-003 (Non-Substitution), GL-NSR-001 (Nasaru τ)
**Reference implementation:** `nasaru_style.js` (shared core, `shala-prototypes/nasaru_style.js`) + `shala_nasaru_style_reference.html` (template court)
**Status:** SEALED — CSR-08 confirmed by the Architect

**Landing note (2026-08-21):** this tablet arrived in the Mon20260817 delivery bearing the ID GL-VIZ-001 and was renumbered to GL-VIZ-009 on landing — GL-VIZ-001 is already sealed in this repo as *Bivector Orbit Encoding and the BUZU Chunk* (`docs/07_file_formats/GL-VIZ-001.md`), an unrelated document; GL-VIZ-000 and GL-VIZ-002 through GL-VIZ-008 are also already claimed. See `docs/mon20260817-incoming/README.md` for the original, unmodified draft and the full collision record.
...
- each Particle is an individual with its own journal, never a substitutable instance — GL-ONT-003 upheld in the UI;
