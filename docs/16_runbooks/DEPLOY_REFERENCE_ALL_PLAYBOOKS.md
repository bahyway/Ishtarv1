...
authenticate`, the same function AnuGovernor's own `vault_check_enabled`
...
never covers (it only runs inside AnuGovernor's own corpus loop).
...
- `playbook_263_deploy_anu_governor.yml` — the standard
  `import_tasks` gate added (installs the governor system-wide).
- `playbook_265_anu_governor_type1_infra_cqrs_nodes.yml` — **not** the
...
## The 3-tab web dashboard — Resource Check / AnuGovernor / Uruinimgina
...
tools into headless automation, `anu-governor-web` (already real — TLS,
Sargon-Passport auth, previously a single-tab corpus-runner dashboard)
now has three tabs behind the same login:

1. **Resource Check** — `crates/anu-governor/src/resource_check.rs`
   ...
   result.
2. **AnuGovernor** — the existing corpus runner, unchanged, now just one
   of three tabs.
...
Build/run exactly as before: `cargo build --release -p anu-governor --bin
anu-governor-web --no-default-features --features web`, then
`anu-governor-web` from the repo root (or via `playbook_263...yml -e
anu_governor_features=web`). No new binary, no new port, no new auth
...
`docs/08_pipeline_alaktu/BEEMDM_ETL_PIPELINE.md`). No playbook in this repo
...
- **Container recreate ≠ data destroy.** The named Podman volume holding
  ...
  their own KNOWN ISSUE header notes and `docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md`.)
...
**CEO / audit-trail reporting:** `docs/16_runbooks/PLAYBOOK_EXECUTION_TRIAGE.md` is
...
(see `docs/11_tooling/ISIMUD_ENGINE_MANUAL.md`) will already include PB-259 in
...
one-line ledger entry per run appended to `docs/16_runbooks/NARU_AUDIT_JOURNAL.md`
...
`ansible-playbook` by hand each time. Shala2 (AnuGovernor's own web tab)
already runs any config you point it at — it doesn't have to be the big,
whole-corpus `anu-governor.toml`. A launcher config is just a minimal
`anu-governor.toml`-shaped file whose `playbooks = [...]` list has exactly
...
that fix, `anu-governor-web`'s working directory depended on wherever
