...
Scheduling: all validation flows run via sovereign Rust job scheduler on UrOS v4.0, configurable per administrator (default: every 15 minutes).
...
| **L10** | eridu-runtime · scheduler | OS Runtime layer — UrOS v4.0 |
...
# **6\. UrOS v4.0 — The Sovereign Algebraic Kernel**

## **6.1 What UrOS Is**

UrOS v4.0 is the sovereign algebraic operating system — the runtime that manages CPU/GPU resources to solve Jordan Normal Form equations for the city's data. It is named after Eridu — the first city ever built in ancient Mesopotamia.

The best analogy: UrOS is to BahyWay what the JVM is to Java — a sovereign sandbox runtime. But with three critical differences:

| Property | Java JVM | UrOS v4.0 |
| ----- | ----- | ----- |
...
| Garbage collection | Objects are collected and destroyed | UrOS never forgets (Axiom 8\) |
...
## **6.2 The UrOS Sandbox Model — App Isolation**

Each BahyWay Enterprise Application runs in its own UrOS sandbox:
...
UrOS is the sovereign equivalent of a Linux Flatpak runtime — a self-contained bundle with all dependencies. But unlike Flatpak, UrOS is built entirely from sovereign primitives: no external database, no file system mutation, no third-party storage substrate.
