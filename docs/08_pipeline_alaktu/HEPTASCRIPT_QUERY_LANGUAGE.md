...
work, `enkidb-query-server`, and the Elu Index Stack documents into one
production-level reference.

**Status tags** follow this ecosystem's own `docs/08_pipeline_alaktu/TRANSPARENCY_STANDARD.md`
convention, used identically to `docs/00_codex/BAHYWAY_ECOSYSTEM_V4_GLOSSARY.md`:
...
| **`enkidb-query-server`** (port 7001 — real code, but **not part of the currently deployed EnkiDDB/EnkiMDB fleet**; see `docs/05_storage/EnkiDDB_MANUAL.md` §1) | `execute_indexed` | Same as Write Node | Reads exactly one field of `.plan` — `across`, only to append a `"[BIGRING]"` tag to its own stderr log line. No federation actually happens. |
...
`docs/08_pipeline_alaktu/BEEMDM_ETL_PIPELINE.md`.
...
from `docs/05_storage/EnkiMDB_MANUAL.md`, not this clause's `port()` values, for
...
Full detail: `docs/05_storage/ELU_INDEX_STACK.md` (thirteen modules,
...
- `docs/09_observatory/HEPTASCRIPT_GLOSSARY.md` — every term this manual
  uses without re-defining, HeptaScript-specific terms first.
- `docs/05_storage/ELU_INDEX_STACK.md` — the thirteen-index W5H2
  reference this manual's §8 summarizes.
- `docs/02_identity/KAKI_V4.md` — the Identity-Kaki/Event-Kaki byte
  layout WHERE/WHAT conditions ultimately resolve against.
- `docs/05_storage/ENKIDB_7_TYPES.md` — the seven EnkiDB node types
  `NODE`'s `DbTarget` enum names (with the real-vs-aspirational port
  caveat repeated in §5).
- `docs/01_mathematics/GEOENGINE_ALGEBRA_ARSENAL.md` — `bahyway-algebra`'s
...
- `docs/08_pipeline_alaktu/BEEMDM_ETL_PIPELINE.md` — CompareEngine, including the
...
- `docs/05_storage/EnkiDDB_MANUAL.md`, `docs/05_storage/EnkiMDB_MANUAL.md` — the real deployed
...
- `docs/00_codex/BAHYWAY_ECOSYSTEM_V4_GLOSSARY.md` — Tribe, BIGRING, Triple-O, and
