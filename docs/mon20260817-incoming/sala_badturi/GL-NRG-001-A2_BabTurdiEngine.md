...
- **PB-348** — BabTurdiEngine: semantic clustering of a client's refused stream, spot-concentration measure, defense-cost attribution with τ, reward-tier gate. Law tests **L61** (no Particle KAKI ever appears in a cost-attribution output — the No-Contamination boundary), **L62** (cost attributed to client with τ, never a bare number), **L63** (above-median cost → reward tier not earned; the client can lower it by cleaner data), **L64** (a client's quiet-vs-siege costs are never averaged).
